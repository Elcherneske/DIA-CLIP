...
#define Throw(x)                                                                 \
	{                                                                            \
		dsout << "ERROR: " << __FILE__ << ": " << __LINE__ << ": " << x << "\n"; \
		exit(-1);                                                                \
	}
#define Warning(x)                             \
	{                                          \
...
struct MOD
{
	...
	MOD(){};
	MOD(const std::string &_name, float _mass, int _label = 0)
	{
		name = _name, mass = _mass;
		label = _label;
	}
	void init(const std::string &_name, const std::string &_aas, float _mass, int _label = 0)
	{
		name = _name, aas = _aas, mass = _mass;
		label = _label;
	}
...
{ // precision ~17 sig figures, locale-independent
	...
	while (std::getline(in, line))
		if (line.size() >= 4)
		{
			...
			else if (look)
			{
				...
				{
					def.clear();
					for (pos = 6; line[pos] != '.'; pos++)
						def += line[pos];
				}
				else if (!memcmp(&line[0], "name: ", 6))
					name = trim(line.substr(6));
				else if (!memcmp(&line[0], "xref: delta_mono_mass \"", 23))
					mass = to_double(&line[23]);
				...
				else if (balance == 0 && !memcmp(&line[0], "xref: spec_1_classification \"Isotopic label", 43))
					label = 1;
			}
		}
	in.close();
...
void init_unimod()
{
	...
	for (int i = 0; i < mods.size(); i++)
	{
		...
		if (!std::memcmp(&(mods[i].name[0]), "UniMod", 6))
			UniMod[i].second = mods[i].name, indices.insert(UniModIndex[i].second = i);
		else
		{
			double min = INF;
			int min_index = 0;
			if (!OriginalModNames)
				for (int j = 0; j < mods.size(); j++)
					if (j != i)
					{
						if (std::memcmp(&(mods[j].name[0]), "UniMod", 6))
							continue;
						double delta = Abs(((double)mods[i].mass) - (double)mods[j].mass);
						if (delta < min)
							min = delta, min_index = j;
					}
			if (min > 0.00001)
			{
...
{
	for (int i = 0; i < UniMod.size(); i++)
		if (mod == UniMod[i].first)
			return UniMod[i].second;
...
{
	for (int i = 0; i < UniModIndex.size(); i++)
		if (mod == UniModIndex[i].first)
			return UniModIndex[i].second;
	Warning("cannot convert to UniMod: unknown modification");
	return 0;
}
...
inline double get_mobility(float intensity)
{ // 1/K0 encoded as part of the intensity 32-bit float
...
inline unsigned int hashA(int *data, int n)
{
	unsigned int res = 0;
	for (int i = 0; i < n; i++)
		res ^= hash_map[(unsigned int)(data[i] & 0xFFFF)] ^ hash_map[(unsigned int)((data[i] >> 16) & 0xFFFF)];
	return res;
}
...
template <class T>
inline double sum(std::vector<T> &x) { return sum(&(x[0]), x.size()); }

template <class T>
inline double mean(T *x, int n)
{
	assert(n >= 1);
...
template <class T>
inline double var(std::vector<T> &x) { return var(&(x[0]), x.size()); }

template <class Tx, class Ty>
inline double corr(Tx *x, Ty *y, int n)
{
	assert(n >= 1);
...
template <class T>
inline void get_corr_matrix(double *mat, T *x, int p, int imul, int n)
{
...
template <class Tx, class Ty>
inline double grad_corr(Tx *x, Ty *y, int n)
{
	assert(n >= 1);
...
template <class T>
inline void get_grad_corr_matrix(double *mat, T *x, int p, int imul, int n)
{
...
template <class Tx, class Ty>
inline double cos(Tx *x, Ty *y, int n)
{
	double s2x = 0.0, s2y = 0.0, r = 0.0;
	for (int i = 0; i < n; i++)
		s2x += Sqr(x[i]), s2y += Sqr(y[i]), r += x[i] * y[i];
	if (s2x < E || s2y < E)
		return 0.0;
...
template <class Tx, class Ty>
inline double scalar(Tx *x, Ty *y, int n)
{
	double r = 0.0;
	for (int i = 0; i < n; i++)
		r += x[i] * y[i];
...
template <class Tx, class Ty>
inline double scalar(Tx *x, Ty *y, bool *mask, int n)
{
	double r = 0.0;
	for (int i = 0; i < n; i++)
		if (mask[i])
...
template <class T>
inline double signal_level(T *x, int center, int n)
{
	int i;
...
template <class T>
inline void smooth(T *dst, T *src, int n)
{
	assert(n >= 2);
...
inline double centroid_coo(Tw *weights, Tx *x, int n)
{
	int i;
	double u, t;
	for (i = 0, u = t = 0.0; i < n; i++)
		u += weights[i] * x[i], t += weights[i];
	return u / Max(E, t);
}

template <class T>
void solve(T *x, Eigen::MatrixXd &A, Eigen::VectorXd &b, int n)
{
...
template <class Tx, class Tfunc>
std::vector<std::pair<Tx, Tfunc>> PAVA(std::vector<std::pair<Tx, Tfunc>> &data)
{
	...
	for (i = 1; i < n; i++)
	{
		...
		{
			pava_a[j - 1] = (pava_w[j] * pava_a[j] + pava_w[j - 1] * pava_a[j - 1]) / (pava_w[j] + pava_w[j - 1]);
...
template <class Tx, class Tfunc>
void spline(std::vector<double> &coeff, std::vector<double> &points, std::vector<std::pair<Tx, Tfunc>> &data, int segments)
{
	...
	for (i = 0; i < n; i++)
		rt_p[i] = data[i].second, rt_e[i] = data[i].first;

	double split = ((double)(n - 1)) / (double)m;
	for (i = 0; i < m; i++)
		rt_s[i] = rt_e[Min(split * (double)i, n - 1)];
	rt_s[m] = rt_e[n - 1];
	for (i = 0; i < m; i++)
		points[i] = 0.5 * (rt_s[i] + rt_s[i + 1]);
	for (i = 1; i < m; i++)
		if (points[i] - points[i - 1] < E)
		{
			for (k = i - 1; k < m - 1; k++)
				points[k] = points[k + 1];
			m--;
...
template <class Tx, class Tfunc>
void map_RT(std::vector<double> &coeff, std::vector<double> &points, std::vector<std::pair<Tx, Tfunc>> &data, int segments)
{
	...
	int pos = 0;
	for (auto &p : data)
		rt_diff.push_back(Abs(p.second - calc_spline(coeff, points, p.first)));
...
template <class Tx, class Ty>
inline void flip_map(std::vector<std::pair<Tx, Ty>> &data)
{
	int n = data.size();
	for (int i = 0; i < n; i++)
		data[i] = std::pair<Ty, Tx>(data[i].second, data[i].first);
}
...
std::vector<Group> bipartite_set_cover(std::vector<std::vector<int>> &sets, int N)
{ // greedy algorithm
			...
			if (!removed[el])
			{
				...
				for (auto &g : gr)
					if (!added[g->index])
					{
						...
						if (g->size >= 1)
							size_index[g->size].push_back(g);
						auto end = size_index[g->size].end();
						index[g->index] = --end;
					}
			}
		while (!size_index[size_index.size() - 1].size() && size_index.size())
			size_index.resize(size_index.size() - 1);
		if (!size_index.size())
			break;
	}
...
inline double y_ratio(int i, int charge, const std::string &aas)
{
	...
	for (j = 0; j < n; j++)
		v += yDelta[y_delta_composition_index(aas[j])];
...
inline std::vector<double> get_sequence(const std::string &name, int *no_cal = NULL)
{
	...
	for (i = 0; i < name.size(); i++)
	{
		...
		if (symbol < 'A' || symbol > 'Z')
		{
			...
			if (end >= name.size())
				Throw(std::string("incorrect peptide name format: ") + name);

			std::string mod = name.substr(i, end - i);
			for (j = 0; j < Modifications.size(); j++)
				if (Modifications[j].name == mod)
				{
					if (!result.size())
						add += Modifications[j].mass;
					else
						result[result.size() - 1] += Modifications[j].mass;
					if (no_cal != NULL && Modifications[j].mass < 4.5 && Modifications[j].mass > 0.0 && Abs(Modifications[j].mass - (double)floor(Modifications[j].mass + 0.5)) < 0.1)
						*no_cal = 1;
					break;
				}
...
inline std::string to_canonical(const std::string &name)
{
	...
	for (i = 0; i < name.size(); i++)
	{
		...
		if (symbol < 'A' || symbol > 'Z')
		{
			...
			std::string mod = name.substr(i, end - i);
			for (j = 0; j < UniMod.size(); j++)
				if (UniMod[j].first == mod)
				{
					result += std::string("(") + UniMod[j].second + std::string(")");
					break;
				}
			if (j == Modifications.size())
				result += std::string("(") + mod + std::string(")"); // no warning here
			i = end;
			continue;
		}
		else
	...
	for (i = 0; i < name.size(); i++)
	{
		...
		if (symbol < 'A' || symbol > 'Z')
		{
			...
			for (j = 0; j < Modifications.size(); j++)
				if (Modifications[j].name == mod)
				{
					if (!Modifications[j].label)
						eg += std::string("(") + UniMod[j].second + std::string(")");
					break;
				}
			if (j == Modifications.size())
				eg += std::string("(") + mod + std::string(")"); // no warning here
			i = end;
			continue;
		}
		else
...
inline std::string to_prosit(const std::string &name)
{
	...
	for (i = 0; i < name.size(); i++)
	{
		...
		if (symbol < 'A' || symbol > 'Z')
		{
			...
			if (i + 9 < name.size())
				if (!memcmp(&(name[i]), "UniMod:35", 9))
					result += "(ox)"; // methionine oxidation
			i = end;
			continue;
		}
		else
...
void arguments(int argc, char *argv[])
{
	...
	if (args.find("--clear-mods") != std::string::npos)
		Modifications.clear(), cleared_mods = true,
	...
	if (args.find("--full-unimod") != std::string::npos)
	{
		...
		if (!loaded)
			dsout << "WARNING: failed to open unimod.obo - place http://www.unimod.org/obo/unimod.obo to the DIA-NN installation folder\n";
		else
		{
	...
	while (start != std::string::npos)
	{
		...
		{ // parse the contents of the file
			...
			if (in.is_open())
			{
				...
				stream << in.rdbuf();
				args = stream.str() + std::string(" ") + args.substr(end, std::string::npos);
				in.close();
				...
				dsout << "WARNING: failed to open cfg file\n";
		}
		...
		else if (!memcmp(&(args[start]), "original-mods ", 14))
			OriginalModNames = true, dsout << "DIA-NN will not attempt to convert library modifications to the UniMod format\n";
		else if (!memcmp(&(args[start]), "threads ", 8))
			Threads = std::stoi(args.substr(start + 8, std::string::npos)), dsout << "Thread number set to " << Threads << "\n";
		else if (!memcmp(&(args[start]), "fasta-search ", 13))
			FastaSearch = true, dsout << "Library-free search enabled\n";
		else if (!memcmp(&(args[start]), "pg-level ", 9))
			PGLevelSet = PGLevel = std::stoi(args.substr(start + 9, std::string::npos)),
		...
		else if (!memcmp(&(args[start]), "no-batch-mode ", 14))
			BatchMode = false, dsout << "Batch mode disabled\n";
		else if (!memcmp(&(args[start]), "verbose ", 8))
			Verbose = std::stoi(args.substr(start + 8, std::string::npos));
		else if (!memcmp(&(args[start]), "export-windows ", 15))
			ExportWindows = true;
		else if (!memcmp(&(args[start]), "export-library ", 15))
			ExportLibrary = true;
		else if (!memcmp(&(args[start]), "gen-fr-restriction ", 19))
			GenFrExclusionInfo = true,
		...
		else if (!memcmp(&(args[start]), "export-decoys ", 14))
			ExportDecoys = true;
		else if (!memcmp(&(args[start]), "prosit ", 7))
			ExportProsit = true;
		else if (!memcmp(&(args[start]), "xic ", 4))
		{
			if (args[start + 4] >= '1' && args[start + 4] <= '9')
				XICRadius = std::stoi(args.substr(start + 4, std::string::npos)) / 2;
		...
		else if (!memcmp(&(args[start]), "extract ", 8))
		{
			...
			for (i = 0; std::getline(list, word, ','); i++)
				Extract.push_back(to_double(word));
		...
		else if (!memcmp(&(args[start]), "vis ", 4))
		{
			...
			for (i = 0; std::getline(list, word, ','); i++)
			{
				...
				{
					if (i == 0)
						VisWindowRadius = Max(1, std::stoi(word) / 2);
				}
		...
		else if (!memcmp(&(args[start]), "cal-info ", 9))
			SaveCalInfo = true;
		else if (!memcmp(&(args[start]), "compact-report ", 15))
			ExtendedReport = false;
		else if (!memcmp(&(args[start]), "report-lib-info ", 16))
			ReportLibraryInfo = true;
		else if (!memcmp(&(args[start]), "decoy-report ", 13))
			DecoyReport = true;
		else if (!memcmp(&(args[start]), "candidate-psms ", 15))
			SaveCandidatePSMs = true;
		else if (!memcmp(&(args[start]), "matrices ", 9))
			SaveMatrices = true, dsout << "Precursor/protein x samples expression level matrices will be saved along with the main report\n";
		else if (!memcmp(&(args[start]), "no-isotopes ", 12))
			UseIsotopes = false, dsout << "Isotopologue chromatograms will not be used\n";
		else if (!memcmp(&(args[start]), "no-ms2-range ", 13))
			MS2Range = false, dsout << "MS2 range inference will not be performed\n";
		else if (!memcmp(&(args[start]), "min-peak ", 9))
			MinPeakHeight = to_double(args.substr(start + 9, std::string::npos)), dsout << "Minimum peak height set to " << MinPeakHeight << "\n";
		else if (!memcmp(&(args[start]), "no-cal-filter ", 14))
			MassCalFilter = false,
		...
		else if (!memcmp(&(args[start]), "no-nn-filter ", 13))
			nnFilter = false,
		...
		else if (!memcmp(&(args[start]), "nn-cross-val ", 13))
			nnCrossVal = true,
			dsout << "Neural network cross-validation will be used to tackle potential overfitting\n";
		else if (!memcmp(&(args[start]), "guide-classifier ", 17))
			GuideClassifier = true, dsout << "A separate classifier for the guide library will be used\n";
		else if (!memcmp(&(args[start]), "int-removal ", 12))
			IDsInterference = std::stoi(args.substr(start + 12, std::string::npos)),
			dsout << "Number of interference removal iterations set to " << IDsInterference << "\n";
		else if (!memcmp(&(args[start]), "int-margin ", 11))
			InterferenceCorrMargin = to_double(args.substr(start + 11, std::string::npos)),
			dsout << "Interference correlation margin set to " << InterferenceCorrMargin << "\n";
		else if (!memcmp(&(args[start]), "strict-int-removal ", 19))
			StrictIntRemoval = true, dsout << "Potentially interfering peptides with close (but not the same) elution times will also be discarded\n";
		else if (!memcmp(&(args[start]), "reverse-decoys ", 15))
			ReverseDecoys = true, dsout << "Decoys will be generated using the pseudo-reverse method\n";
		else if (!memcmp(&(args[start]), "force-frag-rec ", 15))
			ForceFragRec = true, dsout << "Decoys will be generated only for precursors with all library fragments recognised\n";
		else if (!memcmp(&(args[start]), "max-rec-charge ", 15))
			MaxRecCharge = Max(2, std::stoi(args.substr(start + 15, std::string::npos))),
			dsout << "Fragment recognition module will consider charges up to " << MaxRecCharge << "\n";
		else if (!memcmp(&(args[start]), "max-rec-loss ", 13))
			MaxRecLoss = Max(0, std::stoi(args.substr(start + 13, std::string::npos))),
			dsout << "Fragment recognition module will consider losses with the index up to " << MaxRecLoss << "\n";
		else if (!memcmp(&(args[start]), "gen-spec-lib ", 13))
			GenSpecLib = true, dsout << "A spectral library will be generated\n";
		else if (!memcmp(&(args[start]), "lib-gen-direct-q ", 17))
			ProfileQValue = false, dsout << "When generating a spectral library, run-specific q-values will be used instead of profile q-values\n";
		else if (!memcmp(&(args[start]), "save-original-lib ", 18))
			SaveOriginalLib = true, dsout << "All entries from the library provided will be saved to the newly generated library\n";
#ifdef WIFFREADER
		else if (!memcmp(&(args[start]), "fast-wiff ", 10))
			fast_wiff = true, dsout << "Custom fast centroiding will be used when processing .wiff files (WARNING: experimental).\n";
#endif
		else if (!memcmp(&(args[start]), "f ", 2))
			files.push_back(trim(args.substr(start + 2, end - start - 2)));
#ifdef CPP17
...
#endif
		else if (!memcmp(&(args[start]), "lib ", 4))
			lib_file = trim(args.substr(start + 4, end - start - 4));
		else if (!memcmp(&(args[start]), "temp ", 5))
			temp_folder = trim(args.substr(start + 5, end - start - 5));
		else if (!memcmp(&(args[start]), "fasta ", 6))
			fasta_files.push_back(trim(args.substr(start + 6, end - start - 6)));
		else if (!memcmp(&(args[start]), "fasta-filter ", 13))
			fasta_filter_files.push_back(trim(args.substr(start + 13, end - start - 13)));
		else if (!memcmp(&(args[start]), "ref ", 4))
			ref_file = trim(args.substr(start + 4, end - start - 4));
		else if (!memcmp(&(args[start]), "out ", 4))
			out_file = trim(args.substr(start + 4, end - start - 4));
		else if (!memcmp(&(args[start]), "out-gene ", 9))
			out_gene_file = trim(args.substr(start + 9, end - start - 9));
		else if (!memcmp(&(args[start]), "qvalue ", 7))
			ReportQValue = to_double(args.substr(start + 7, std::string::npos)), dsout << "Output will be filtered at " << ReportQValue << " FDR\n";
		else if (!memcmp(&(args[start]), "protein-qvalue ", 15))
			ReportProteinQValue = to_double(args.substr(start + 15, std::string::npos)),
			dsout << "Output will be filtered at " << ReportProteinQValue << " protein-level FDR\n";
		else if (!memcmp(&(args[start]), "matrix-qvalue ", 14))
			MatrixQValue = Max(0.001, Min(0.05, to_double(args.substr(start + 14, std::string::npos)))),
			dsout << "Precursor/protein x sample matrices will be filtered at " << MatrixQValue << " precursor & protein-level FDR\n";
		else if (!memcmp(&(args[start]), "no-prot-inf ", 12))
			InferPGs = false, dsout << "Protein inference will not be performed\n";
		else if (!memcmp(&(args[start]), "no-swissprot ", 13))
			SwissProtPriority = false, dsout << "SwissProt proteins will not be prioritised for protein inference\n";
		else if (!memcmp(&(args[start]), "force-swissprot ", 16))
			ForceSwissProt = true, dsout << "Only SwissProt proteins will be considered in library-free search\n";
		else if (!memcmp(&(args[start]), "species-genes ", 14))
			SpeciesGenes = true, dsout << "Species suffix will be added to gene annotation; this affects proteotypicity definition\n";
		else if (!memcmp(&(args[start]), "duplicate-proteins ", 19))
			FastaProtDuplicates = true, dsout << "Duplicate proteins in FASTA files will not be skipped\n";
		else if (!memcmp(&(args[start]), "out-lib ", 8))
			out_lib_file = trim(args.substr(start + 8, end - start - 8));
		else if (!memcmp(&(args[start]), "learn-lib ", 10))
			learn_lib_file = trim(args.substr(start + 10, end - start - 10));
		...
		else if (!memcmp(&(args[start]), "out-measured-rt ", 16))
			iRTOutputLibrary = false,
		...
		else if (!memcmp(&(args[start]), "mod ", 4))
		{
			...
			std::stringstream list(trim(args.substr(start + 4, end - start - 4)));
			if (!std::getline(list, name, ','))
				dsout << "WARNING: no modification name, modification ignored\n";
			else if (!std::getline(list, mass, ','))
				dsout << "WARNING: no modification mass, modification ignored\n";
			else
			{
				if (std::getline(list, l, ','))
					if (l.find("label") != std::string::npos)
						label = 1;
		...
		else if (!memcmp(&(args[start]), "fixed-mod ", 10))
		{
			...
			if (!std::getline(list, name, ','))
				...
				dsout << "WARNING: no modification mass, modification ignored\n";
			else if (!std::getline(list, aas, ','))
				dsout << "WARNING: no amino acids to be modified, modification ignored\n";
			else
			{
				if (std::getline(list, l, ','))
					if (l.size())
		...
		else if (!memcmp(&(args[start]), "ref-cal ", 8))
			RefCal = true, dsout << "Reference peptides will be used for calibration\n";
		else if (!memcmp(&(args[start]), "gen-ref ", 8))
			GenRef = true, gen_ref_file = trim(args.substr(start + 8, end - start - 8)),
		...
		else if (!memcmp(&(args[start]), "window ", 7))
		{
			...
			if (WindowRadius <= 0)
				dsout << "WARNING: scan window radius should be a positive integer\n";
			else
				InferWindow = false, dsout << "Scan window radius set to " << WindowRadius << "\n";
		}
		else if (!memcmp(&(args[start]), "cut ", 4))
		{
			...
			if (Cut.size())
			{
				...
				for (int s = 1; s < Cut.size(); s++)
					dsout << ',' << char(Cut[s] >> 8) << char(Cut[s] & 0xFF);
				dsout << '\n';
			}
			if (NoCut.size())
			{
				...
				for (int s = 1; s < NoCut.size(); s++)
					dsout << ',' << char(NoCut[s] & 0xFF) << char(NoCut[s] >> 8);
				dsout << '\n';
			}
		}
		else if (!memcmp(&(args[start]), "min-pep-len ", 12))
			MinPeptideLength = std::stoi(args.substr(start + 12, std::string::npos)),
			dsout << "Min peptide length set to " << MinPeptideLength << "\n";
		else if (!memcmp(&(args[start]), "max-pep-len ", 12))
			MaxPeptideLength = std::stoi(args.substr(start + 12, std::string::npos)),
			dsout << "Max peptide length set to " << MaxPeptideLength << "\n";
		else if (!memcmp(&(args[start]), "min-fr-corr ", 12))
			MinGenFrCorr = to_double(args.substr(start + 12, std::string::npos)),
			dsout << "Minimum fragment profile correlation for the inclusion into the spectral library set to " << MinGenFrCorr << "\n";
		else if (!memcmp(&(args[start]), "min-gen-fr ", 11))
			MinOutFrNum = std::stoi(args.substr(start + 11, std::string::npos)),
			dsout << "Minimum number of fragments for library generation set to " << MinOutFrNum << "\n";
		else if (!memcmp(&(args[start]), "min-pr-mz ", 10))
			MinPrMz = to_double(args.substr(start + 10, std::string::npos)),
			dsout << "Min precursor m/z set to " << MinPrMz << "\n";
		else if (!memcmp(&(args[start]), "max-pr-mz ", 10))
			MaxPrMz = to_double(args.substr(start + 10, std::string::npos)),
			dsout << "Max precursor m/z set to " << MaxPrMz << "\n";
		else if (!memcmp(&(args[start]), "min-fr-mz ", 10))
			MinFrMz = to_double(args.substr(start + 10, std::string::npos)),
			dsout << "Min fragment m/z set to " << MinFrMz << "\n";
		else if (!memcmp(&(args[start]), "max-fr-mz ", 10))
			MaxFrMz = to_double(args.substr(start + 10, std::string::npos)),
			dsout << "Max fragment m/z set to " << MaxFrMz << "\n";
		else if (!memcmp(&(args[start]), "min-pr-charge ", 14))
			MinPrCharge = std::stoi(args.substr(start + 14, std::string::npos)),
			dsout << "Min precursor charge set to " << MinPrCharge << "\n";
		else if (!memcmp(&(args[start]), "max-pr-charge ", 14))
			MaxPrCharge = std::stoi(args.substr(start + 14, std::string::npos)),
			dsout << "Max precursor charge set to " << MaxPrCharge << "\n";
		else if (!memcmp(&(args[start]), "max-fr ", 7))
			MaxF = std::stoi(args.substr(start + 7, std::string::npos)),
			dsout << "Maximum number of fragments set to " << MaxF << "\n";
		else if (!memcmp(&(args[start]), "min-fr ", 7))
			MinF = std::stoi(args.substr(start + 7, std::string::npos)),
			dsout << "Minimum number of fragments for library export set to " << MinF << "\n";
		else if (!memcmp(&(args[start]), "min-search-fr ", 14))
			MinSearchFrNum = std::stoi(args.substr(start + 14, std::string::npos)),
			dsout << "Minimum number of fragments required for a precursor to be searched set to " << MinSearchFrNum << "\n";
		else if (!memcmp(&(args[start]), "missed-cleavages ", 17))
			MissedCleavages = std::stoi(args.substr(start + 17, std::string::npos)),
		...
		else if (!memcmp(&(args[start]), "var-mods ", 9))
			MaxVarMods = std::stoi(args.substr(start + 9, std::string::npos)),
			dsout << "Maximum number of variable modifications set to " << MaxVarMods << "\n";
		else if (!memcmp(&(args[start]), "met-excision ", 6))
			NMetExcision = true, dsout << "N-terminal methionine excision enabled\n";
		else if (!memcmp(&(args[start]), "no-rt-window ", 13))
			RTWindowedSearch = false, dsout << "Full range of retention times will be considered\n";
		else if (!memcmp(&(args[start]), "disable-rt ", 11))
			DisableRT = true, dsout << "All RT-related scores disabled\n";
		else if (!memcmp(&(args[start]), "min-rt-win ", 11))
			MinRTWinFactor = to_double(args.substr(start + 11, std::string::npos)),
			dsout << "Minimum acceptable RT window scale set to " << MinRTWinFactor << "\n";
		else if (!memcmp(&(args[start]), "no-window-inference ", 20))
			InferWindow = false, dsout << "Scan window inference disabled\n";
		else if (!memcmp(&(args[start]), "individual-windows ", 19))
			IndividualWindows = true, dsout << "Scan windows will be inferred separately for different runs\n";
		else if (!memcmp(&(args[start]), "individual-mass-acc ", 20))
			IndividualMassAcc = true, dsout << "Mass accuracy will be determined separately for different runs\n";
		else if (!memcmp(&(args[start]), "individual-reports ", 19))
			IndividualReports = true, dsout << "Reports will be generated separately for different runs (in the respective folders)\n";
		else if (!memcmp(&(args[start]), "no-stats ", 9))
			SaveRunStats = false;
		else if (!memcmp(&(args[start]), "convert ", 8))
			Convert = true, dsout << "MS data files will be converted to .dia format\n";
		else if (!memcmp(&(args[start]), "mgf ", 4))
			MGF = true, std::cout << "Spectra will be extracted from the DIA data and exported in the .mgf format\n";
		else if (!memcmp(&(args[start]), "reannotate ", 11))
			Reannotate = true, dsout << "Library precursors will be reannotated using the FASTA database\n";
		else if (!memcmp(&(args[start]), "out-dir ", 8))
			out_dir = trim(args.substr(start + 8, end - start - 8));
#ifdef CPP17
		else if (!memcmp(&(args[start]), "remove-quant ", 13))
			RemoveQuant = true, dsout << ".quant files will be removed when the analysis is finished\n";
#endif
		else if (!memcmp(&(args[start]), "no-quant-files ", 15))
			QuantInMem = true, dsout << ".quant files will not be saved to the disk\n";
		else if (!memcmp(&(args[start]), "use-rt ", 7))
			UseRTInfo = true, dsout << "Existing .quant files will be used for RT profiling\n";
		else if (!memcmp(&(args[start]), "use-quant ", 10))
			UseQuant = true, dsout << "Existing .quant files will be used\n";
		else if (!memcmp(&(args[start]), "quant-only ", 11))
			QuantOnly = true, dsout << "Quantification will be performed anew using existing identification info\n";
		else if (!memcmp(&(args[start]), "report-only ", 12))
			ReportOnly = true, dsout << "Report will be generated using .quant files\n";
		else
			flag = true;
		...
		else if (!memcmp(&(args[start]), "iter ", 5))
			iN = Max(CalibrationIter + 4, std::stoi(args.substr(start + 5, std::string::npos))),
			dsout << "Number of iterations set to " << iN << "\n";
		else if (!memcmp(&(args[start]), "profiling-qvalue ", 17))
			MaxProfilingQvalue = to_double(args.substr(start + 17, std::string::npos)),
			dsout << "RT profiling q-value threshold set to " << MaxProfilingQvalue << "\n";
		else if (!memcmp(&(args[start]), "quant-qvalue ", 13))
			MaxQuantQvalue = to_double(args.substr(start + 13, std::string::npos)),
			dsout << "Q-value threshold for cross-run quantification set to " << MaxQuantQvalue << "\n";
		else if (!memcmp(&(args[start]), "protein-quant-qvalue ", 21))
			ProteinQuantQvalue = to_double(args.substr(start + 21, std::string::npos)),
			dsout << "Precursor Q-value threshold for protein quantification set to " << ProteinQuantQvalue << "\n";
		else if (!memcmp(&(args[start]), "no-maxlfq ", 10))
			MaxLFQ = false, dsout << "MaxLFQ-based gene quantification disabled\n";
		else if (!memcmp(&(args[start]), "top ", 4))
			TopN = std::stoi(args.substr(start + 4, std::string::npos)),
			dsout << "Top " << TopN << " precursors will be used for protein quantification in each run\n";
		else if (!memcmp(&(args[start]), "out-lib-qvalue ", 15))
			ReportQValue = to_double(args.substr(start + 15, std::string::npos)),
			dsout << "Q-value threshold for spectral library generation set to " << ReportQValue << "\n";
		else if (!memcmp(&(args[start]), "rt-profiling ", 13))
			RTProfiling = true, dsout << "RT profiling enabled\n";
		else if (!memcmp(&(args[start]), "prefix ", 7))
			prefix = trim(args.substr(start + 7, end - start - 7)); // prefix added to input file names
		else if (!memcmp(&(args[start]), "ext ", 4))
			ext = trim(args.substr(start + 4, end - start - 4)); // extension added to input file names
		else if (!memcmp(&(args[start]), "lc-all-scores ", 14))
			LCAllScores = true, dsout << "All scores will be used by the linear classifier (not recommended)\n";
		else if (!memcmp(&(args[start]), "peak-center ", 12))
			QuantMode = 1, dsout << "Fixed-width center of each elution peak will be used for quantification\n";
		else if (!memcmp(&(args[start]), "peak-boundary ", 14))
			PeakBoundary = to_double(args.substr(start + 14, std::string::npos)),
			dsout << "Peak boundary intensity factor set to " << PeakBoundary << "\n";
		else if (!memcmp(&(args[start]), "standardisation-scale ", 22))
			StandardisationScale = to_double(args.substr(start + 22, std::string::npos)),
			dsout << "Standardisation scale set to " << StandardisationScale << "\n";
		else if (!memcmp(&(args[start]), "no-ifs-removal ", 15))
			NoIfsRemoval = true, dsout << "Interference removal from fragment elution curves disabled\n";
		else if (!memcmp(&(args[start]), "no-fr-selection ", 16))
			NoFragmentSelectionForQuant = true, dsout << "Cross-run selection of fragments for quantification disabled (not recommended)\n";
		else if (!memcmp(&(args[start]), "restrict-fr ", 12))
			RestrictFragments = true, dsout << "Certain fragments (based on the library annotation) will not be used when quantifying peptides\n";
		else if (!memcmp(&(args[start]), "no-fr-exclusion ", 16))
			ExcludeSharedFragments = false, dsout << "Exclusion of fragments shared between heavy and light labelled peptides from quantification disabled\n";
		else if (!memcmp(&(args[start]), "central-bins ", 13))
			QuantAdjacentBins = false, dsout << "Only the central scanning bin will be used for quantification\n";
		else if (!memcmp(&(args[start]), "peak-translation ", 17))
			TranslatePeaks = true, dsout << "Translation of retention times between peptides within the same elution group enabled\n";
		else if (!memcmp(&(args[start]), "no-standardisation ", 19))
			Standardise = false, dsout << "Scores will not be standardised for neural network training\n";
		else if (!memcmp(&(args[start]), "no-nn ", 6))
			nnIter = INF, dsout << "Neural network classifier disabled\n";
		else if (!memcmp(&(args[start]), "nn-iter ", 8))
			nnIter = Max(CalibrationIter + 3, std::stoi(args.substr(start + 8, std::string::npos))),
			dsout << "Neural network classifier will be used starting from the interation number " << nnIter << "\n";
		else if (!memcmp(&(args[start]), "nn-bagging ", 11))
			nnBagging = std::stoi(args.substr(start + 11, std::string::npos)),
			dsout << "Neural network bagging set to " << nnBagging << "\n";
		else if (!memcmp(&(args[start]), "nn-epochs ", 10))
			nnEpochs = std::stoi(args.substr(start + 10, std::string::npos)),
			dsout << "Neural network epochs number set to " << nnEpochs << "\n";
		else if (!memcmp(&(args[start]), "nn-learning-rate ", 17))
			nnLearning = to_double(args.substr(start + 17, std::string::npos)),
			dsout << "Neural network learning rate set to " << nnLearning << "\n";
		else if (!memcmp(&(args[start]), "nn-reg ", 7))
			Regularisation = to_double(args.substr(start + 7, std::string::npos)),
			dsout << "Neural network regularisation set to " << Regularisation << "\n";
		else if (!memcmp(&(args[start]), "nn-hidden ", 10))
			nnHidden = Max(1, std::stoi(args.substr(start + 10, std::string::npos))),
			dsout << "Number of hidden layers set to " << nnHidden << "\n";
		else if (!memcmp(&(args[start]), "mass-acc-cal ", 13))
			CalibrationMassAccuracy = to_double(args.substr(start + 13, std::string::npos)) / 1000000.0,
			dsout << "Calibration mass accuracy set to " << CalibrationMassAccuracy << "\n";
		else if (!memcmp(&(args[start]), "fix-mass-acc ", 13))
			ForceMassAcc = true;
		else if (!memcmp(&(args[start]), "mass-acc ", 9))
			GlobalMassAccuracy = to_double(args.substr(start + 9, std::string::npos)) / 1000000.0, ForceMassAcc = true;
		else if (!memcmp(&(args[start]), "mass-acc-ms1 ", 13))
			GlobalMassAccuracyMs1 = to_double(args.substr(start + 13, std::string::npos)) / 1000000.0, ForceMassAcc = true;
		else if (!memcmp(&(args[start]), "gen-acc ", 8))
			GeneratorAccuracy = to_double(args.substr(start + 8, std::string::npos)) / 1000000.0,
			dsout << "Fragmentation spectrum generator accuracy set to " << GeneratorAccuracy << "\n";
		else if (!memcmp(&(args[start]), "sptxt-acc ", 10))
			SptxtMassAccuracy = to_double(args.substr(start + 10, std::string::npos)) / 1000000.0,
			dsout << "Fragment filtering for .sptxt/.msp spectral libraries set to " << SptxtMassAccuracy * 1000000.0 << " ppm\n";
		else if (!memcmp(&(args[start]), "min-corr ", 9))
			MinCorrScore = Min(to_double(args.substr(start + 9, std::string::npos)), -1.1 + (double)TopF),
			dsout << "Only peaks with correlation sum exceeding " << MinCorrScore << " will be considered\n";
		else if (!memcmp(&(args[start]), "min-ms1-corr ", 13))
			MinMs1Corr = Min(to_double(args.substr(start + 13, std::string::npos)), 0.99),
			dsout << "Only peaks with MS1 profile correlation exceeding " << MinMs1Corr << " will be considered\n";
		else if (!memcmp(&(args[start]), "corr-diff ", 10))
			MaxCorrDiff = Max(to_double(args.substr(start + 10, std::string::npos)), E),
			dsout << "Peaks with correlation sum below " << MaxCorrDiff << " from maximum will not be considered\n";
		else if (!memcmp(&(args[start]), "peak-apex ", 10))
			PeakApexEvidence = Min(to_double(args.substr(start + 10, std::string::npos)), 0.99),
			dsout << "Peaks must have apex height which is at least " << PeakApexEvidence << " of the maximum within the m/z scan window\n";
		else if (!memcmp(&(args[start]), "norm-qvalue ", 12))
			NormalisationQvalue = to_double(args.substr(start + 11, std::string::npos)),
			dsout << "Q-value threshold for cross-run normalisation set to " << NormalisationQvalue << "\n";
		else if (!memcmp(&(args[start]), "norm-fraction ", 14))
			NormalisationPeptidesFraction = to_double(args.substr(start + 14, std::string::npos)),
			dsout << "Global normalisation peptides fraction set to " << NormalisationPeptidesFraction << "\n";
		else if (!memcmp(&(args[start]), "norm-radius ", 12))
			LocNormRadius = std::stoi(args.substr(start + 12, std::string::npos)),
			dsout << "Local normalisation radius set to " << LocNormRadius << "\n";
		else if (!memcmp(&(args[start]), "global-norm ", 12))
			LocalNormalisation = false, dsout << "Median-based local normalisation disabled\n";
		else if (!memcmp(&(args[start]), "no-rt-norm ", 11))
			NoRTDepNorm = true, dsout << "Median-based RT-dependent local normalisation disabled\n";
		else if (!memcmp(&(args[start]), "sig-norm ", 9))
			NoSigDepNorm = false, dsout << "Median-based signal-dependent local normalisation enabled\n";
#if Q1
		else if (!memcmp(&(args[start]), "q1-cal ", 7))
			Q1Cal = true, dsout << "Q1 calibration enabled\n";
#endif
		else if (!memcmp(&(args[start]), "no-calibration ", 15))
			Calibrate = false, dsout << "Mass calibration disabled\n";
		else if (!memcmp(&(args[start]), "mass-cal-bins ", 14))
			MassCalBinsMax = std::stoi(args.substr(start + 14, std::string::npos)),
			dsout << "Maximum number of mass calibration bins set to " << MassCalBinsMax << "\n";
		else if (!memcmp(&(args[start]), "min-cal ", 8))
			MinCal = std::stoi(args.substr(start + 8, std::string::npos)),
			dsout << "Minimum number of precursors identified at 10% FDR used for calibration set to " << MinCal << "\n";
		else if (!memcmp(&(args[start]), "min-class ", 10))
			MinClassifier = std::stoi(args.substr(start + 10, std::string::npos)),
			dsout << "Minimum number of precursors identified at 10% FDR used for linear classifier training set to " << MinClassifier << "\n";
		else if (!memcmp(&(args[start]), "scanning-swath ", 15))
			ForceScanningSWATH = true, dsout << "All runs will be analysed as Scanning SWATH runs\n";
		else if (!memcmp(&(args[start]), "regular-swath ", 14))
			ForceNormalSWATH = true, dsout << "All runs will be analysed as regular SWATH runs\n";
#if Q1
		else if (!memcmp(&(args[start]), "no-q1 ", 6))
			UseQ1 = false, dsout << "Q1 scores disabled\n";
		else if (!memcmp(&(args[start]), "use-q1 ", 8))
			ForceQ1 = true, dsout << "Q1 scores will be used for regular SWATH runs\n";
#endif
		else
			dsout << "WARNING: unrecognised option [--" << trim(args.substr(start, end - start)) << "]\n";
	...
	if (!ms_files.size() && GenSpecLib)
		ExportLibrary = true, GenSpecLib = false;
	if (ms_files.size() < 2)
		RTProfiling = false;
	if (Predictor || ExportProsit)
		ExportLibrary = false;

	if (DisableRT)
		RTWindowedSearch = false;
	if (!ExportLibrary && !GenSpecLib && !Predictor && !ExportProsit)
		out_lib_file.clear();
	...
	if (nnCrossVal)
	{
		...
		if (bagging != nnBagging)
			nnBagging = bagging, dsout << "Bagging factor set to " << bagging << " (required for NN cross validation)\n";
	}
	if (BatchMode)
		nnIter = Max(iN - 1, nnIter);
	...
	else if (!FastaSearch && !lib_file.size() && !Convert)
		dsout << "WARNING: no spectral library provided, FASTA digest not enabled: nothing to do\n";
	...
	if (Reannotate && !Convert)
	{
		if (!lib_file.size())
			Reannotate = false;
		if (!fasta_files.size() && lib_file.size())
			Reannotate = false, dsout << "WARNING: no FASTA, cannot reannotate the library\n";
	}
...
void write_vector(F &out, std::vector<T> &v)
{
	int size = v.size();
...
template <class F, class T>
void read_vector(F &in, std::vector<T> &v)
{
	int size = 0;
...
template <class F>
void write_string(F &out, std::string &s)
{
	int size = s.size();
	out.write((char *)&size, sizeof(int));
...
template <class F>
void read_string(F &in, std::string &s)
{
	int size = 0;
	in.read((char *)&size, sizeof(int));
	if (size)
...
template <class F, class T>
void write_array(F &out, std::vector<T> &a)
{
	int size = a.size();
	out.write((char *)&size, sizeof(int));
...
template <class F, class T>
void read_array(F &in, std::vector<T> &a)
{
	int size = 0;
	in.read((char *)&size, sizeof(int));
	if (size)
...
template <class F>
void write_strings(F &out, std::vector<std::string> &strs)
{
	int size = strs.size();
	out.write((char *)&size, sizeof(int));
	for (int i = 0; i < size; i++)
		write_string(out, strs[i]);
}

template <class F>
void read_strings(F &in, std::vector<std::string> &strs)
{
	int size = 0;
	in.read((char *)&size, sizeof(int));
	if (size)
...
start:
	...
	if (charge == 1)
		v.push_back(Ion(fTypeY, sequence.size(), loss, pr_charge, (s + OH + (1.0 + double(pr_charge)) * proton - Loss[loss]) / double(pr_charge))); // non-fragmented
	...
	loss++;
	if (loss == loss_cap)
		loss = loss_none, charge++;
	if (charge <= max_charge && tot < fragments.size())
		goto start;
...
std::vector<Ion> recognise_fragments(const std::string &name, const std::vector<Product> &fragments, float &gen_acc, int pr_charge, bool full_spectrum = false, int max_charge = 19, int loss_cap = loss_N)
{
	int i;
	if (fragments.size())
		if (fragments[0].type & 3)
		{
			std::vector<Ion> result(fragments.size());
			for (i = 0; i < result.size(); i++)
				if (fragments[i].type & 3)
					result[i].init(fragments[i]);
				else
					break;
			if (i == result.size())
				return result;
...
struct Feature
{
	...
	friend inline bool operator<(const Feature &left, const Feature &right)
	{
		if (left.RT < right.RT)
			return true;
		if (left.RT <= right.RT && left.mz < right.mz)
			return true;
		return false;
	}

	template <bool open>
	void find(bool _decoy, std::vector<Mass> &fragments, std::vector<Mass> &fragments_shift,
			  std::vector<std::pair<float, int>> &precursors, double acc_ms1, double acc_ms2, std::vector<Match> &temp)
	{
		...
		for (int iter = 0; iter <= (int)open; iter++)
			for (auto &p : peaks)
			{
				...
				if (high <= frm[0].mz || query.mz >= frm[frs - 1].mz)
					continue;
				...
				while (pos->mz < high)
				{
					...
					{
						if (size <= pos->pr)
							size = ((pos->pr * 5) / 4) + 1, temp.resize(size);
						...
						for (i = 0; i < SCSFr - 1; i++)
						{
							...
							else if (open && h.fr == fr && p.score <= h.score)
								break;
						}
						if (i == SCSFr - 1)
						{
...
struct Scan
{
	...
	template <bool get_mz>
	inline double level(int &low, int &high, float mz, float accuracy, float *peak_mz = NULL)
	{
		int i;
		...
		while (high > low)
		{
			...
			if (v < max)
			{
				if (v > min)
				{
					...
					if (get_mz)
						*peak_mz = peaks[middle].mz;
					for (i = middle + 1; i < high && peaks[i].mz < max; i++)
						if (peaks[i].height > s)
						{
							s = peaks[i].height;
							if (get_mz)
					...
					if (i < high)
						high = i;
					for (i = middle - 1; i >= low && peaks[i].mz > min; i--)
						if (peaks[i].height > s)
						{
							s = peaks[i].height;
							if (get_mz)
	...
	template <bool get_mz>
	inline double level(float mz, float accuracy, float *peak_mz = NULL)
	{
		int low = 0, high = size();
		return level<get_mz>(low, high, mz, accuracy, peak_mz);
	}

	inline double level(float mz, float accuracy, float ref_height)
	{ // finds peak with height closest to the ref_height
		...
		while (high > low)
		{
			...
			if (v < max)
			{
				if (v > min)
				{
					...
					for (i = middle + 1; i < high && peaks[i].mz < max; i++)
						if ((u = Abs(peaks[i].height - ref_height)) < delta)
						{
							s = peaks[i].height;
							delta = u;
						}
	...
	void bin_peaks(FI *binned, int bins_per_Da)
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			if (p.height > fi->value)
				fi->value = p.height, fi->index = i;
		}
	}
	...
	void top_peaks(Peak *top, int N, std::vector<Peak> &temp, float DR = 10000.0)
	{
		...
		std::sort(temp.begin(), temp.end(), [&](const Peak &left, const Peak &right)
				  { return left.height > right.height; });

		margin = temp[0].height / DR;
		for (k = 0; k < Min(n, N); k++)
			if (temp[k].height < margin)
				break;
	...
	finish:
		if (N > k)
			memset(top + k, 0, (N - k) * sizeof(Peak));
	}
	...
	void top_peaks(Peak *top, int N, float low, float high, std::vector<Peak> &temp, float DR = 10000.0)
	{
		...
		margin = temp[0].height / DR;
		for (i = k = 0; i < n && k < N; i++)
			if (temp[i].mz > low && temp[i].mz < high)
			{
				if (temp[k].height < margin)
					break;
				top[k++] = temp[i];
...
public:
	...
	void annotate(std::vector<Isoform> &_proteins, std::vector<std::string> &_names, std::vector<std::string> &_genes)
	{
		...
		{
			for (auto &p : proteins)
				if (_names[_proteins[p].name_index].size())
			...
			names.clear();
			name_indices.clear();
			name_indices.insert(name_indices.begin(), name.begin(), name.end());
		...
		{
			for (auto &p : proteins)
				if (_genes[_proteins[p].gene_index].size())
			...
			if (genes.size() && !proteins.size())
			{
				...
				while (std::getline(list, word, ';'))
				{
					...
					if (pos != _genes.end())
						if (*pos == word)
							gene.insert(std::distance(_genes.begin(), pos));
				}
			}
			genes.clear();
			gene_indices.clear();
			gene_indices.insert(gene_indices.begin(), gene.begin(), gene.end());
			if (gene_indices.size())
				for (int i = 0; i < gene_indices.size(); i++)
					genes += (genes.size() ? std::string(";") : std::string("")) + _genes[gene_indices[i]];
		}
	}
...
#endif
	friend inline bool operator<(const PrecursorEntry &left, const PrecursorEntry &right)
	{
...
public:
	...
	QuantEntry()
	{
		memset(&pr, 0, sizeof(PrecursorEntry));
		memset(fr, 0, TopF * sizeof(Fragment));
	}

	template <class F>
	void write(F &out) { out.write((char *)this, sizeof(QuantEntry)); }

	template <class F>
	void read(F &in) { in.read((char *)this, sizeof(QuantEntry)); }
...
public:
	...
	DecoyEntry() {}
	DecoyEntry(int _index, float _qvalue, float _lib_qvalue, float _dratio, float _cscore)
	{
		index = _index;
		qvalue = _qvalue;
		lib_qvalue = _lib_qvalue;
		dratio = _dratio;
		cscore = _cscore;
	}
...
public:
	...
	template <class F>
	void read_meta(F &in, int _lib_size = 0)
	{
		if (in.fail())
		{
	...
	void read_meta(const std::string &file, int _lib_size = 0)
	{
		...
		if (in.fail() || temp_folder.size())
		{
			...
			if (pos >= 1)
				in = std::ifstream(location_to_file_name(file.substr(0, pos - 1)) + std::string(".quant"), std::ifstream::binary);
		}
	...
	template <class F>
	void read(F &in, int _lib_size = 0)
	{
		read_meta(in, _lib_size);
...
public:
	...
	void annotation_sgd(std::string &header, std::string &id, std::string &name, std::string &gene, std::string &description, bool *swissprot)
	{
		*swissprot = (header.find("Verified ORF") != std::string::npos);
		...
		if (end == std::string::npos)
			id = fast_trim(header.substr(start + 1)), name = description = "";
		else
		{
			...
			else
			{
				...
				if (end2 == std::string::npos)
					...
					description = header.substr(end + 1, end2 - end - 1);
			}
		}
		gene = name;
	}
	...
	void annotation(std::string &header, std::string &id, std::string &name, std::string &gene, std::string &description, bool *swissprot)
	{
		...
		if (end == std::string::npos)
			end = header.find(" ", start + 1), name = description = "";
		else
		{
			...
			if (end2 != std::string::npos)
			{
				...
				{
					while (header[end3] != ' ' && end3 > end2 + 2)
						end3--;
					if (end3 > end2 + 2)
						description = fast_trim(header.substr(end2 + 1, end3 - end2 - 1));
					else
						description = "";
				}
				else
					description = fast_trim(header.substr(end2 + 1));
			}
			else
				name = fast_trim(header.substr(end + 1)), description = "";
		}
		if (end != std::string::npos)
			id = fast_trim(header.substr(start + 1, end - start - 1));
		else
			id = fast_trim(header.substr(start + 1));
		...
		else
		{
			...
			if (end != std::string::npos)
				gene = fast_trim(header.substr(start + 3, end - start - 3));
			else
				gene = fast_trim(header.substr(start + 3));
		}
	...
	bool load_proteins(std::vector<std::string> &files)
	{
		...
		{
			if (Verbose >= 1)
				Time(), dsout << "Loading protein annotations from FASTA " << file << "\n";
			...
			while (getline(in, line))
				if (line[0] == '>')
				{
					...
					if (ins.second)
						proteins.push_back(Isoform(id, name, gene, description, swissprot));
				}
			in.close();
		}
	...
	bool load(std::vector<std::string> &files, std::vector<std::string> &filter_files)
	{
		...
		for (auto &filter : filter_files)
		{
			...
			if (!in.fail())
			{
				...
				in.close();
				if (Verbose >= 1)
					Time(), dsout << "Loaded FASTA filter " << filter << "\n";
			}
			else
				dsout << "WARNING: couldn't open the FASTA filter file " << filter << "\n";
		...
		for (auto file : files)
		{
				...
				while (true)
				{
					...
					if (new_prot || eof)
					{
						if (new_prot)
						{
							...
							if (!skip)
								proteins.push_back(Isoform(next_id, next_name, next_gene, description, swissprot));
							if (FastaProtDuplicates)
								skip = false;
							if (skip)
								skipped++;
							if (ForceSwissProt)
						...
						for (int i = 0; i < sequence.length(); i++)
						{
							...
							if (i < sequence.length() - 4)
							{
								...
								if (!ins.second)
									ins.first->second.push_back(vec[0]);
							}
						}
						...
						if (max_miss >= cuts.size())
							max_miss = cuts.size() - 1;
						for (int miss = 0; miss <= max_miss; miss++)
						{
							for (int start = 0; start < cuts.size() - miss - 1; start++)
							{
								...
								if (len < MinPeptideLength || len > MaxPeptideLength)
									continue;
								...
								for (auto s : stripped)
									if (s < 'A' || s > 'Z')
										skip_peptide = true;
								if (skip_peptide)
									continue; // handle undefined amino acids in SGD FASTA
								if (filters.size())
								{
							...
							insert:
								...
								if (!ins.second)
								{
									if (!name_included(ins.first->ids, id))
										ins.first->ids += std::string(";") + id;
									if (!name_included(ins.first->names, name))
										ins.first->names += std::string(";") + name;
									if (!name_included(ins.first->genes, gene))
										ins.first->genes += std::string(";") + gene;
								}
			...
			in.close();
			if (skipped)
				dsout << "WARNING: " << skipped << " sequences skipped due to duplicate protein ids; use --duplicate-proteins to disable skipping duplicates\n";
		}
		dict.insert(dict.begin(), dic.begin(), dic.end());
		dic.clear();
		...
		for (auto &seq : unique)
		{
			...
			else
			{
				...
				for (auto &fmod : FixedMods)
					for (int x = 0; x < fmod.aas.size(); x++)
						if (fmod.aas[x] == lc)
						{ // N-terminal modification: encoded with a lower-case letter for the amino acid
							peptide += '(' + fmod.name + ')';
							break;
						}
				for (auto &aa : s)
...
struct NormInfo
{
	...
	NormInfo() {}
	NormInfo(float _par, int _index, float _signal)
	{
		index = _index;
		signal = _signal;
		par = _par;
	...
	inline friend bool operator<(const XIC &left, const XIC &right)
	{
		...
		if (left.pr == right.pr)
		{
			if (left.run < right.run)
				return true;
			if (left.run == right.run && left.fr < right.fr)
				return true;
		}
		return false;
	}
...
public:
	...
	public:
		...
		void init()
		{
			std::sort(target.fragments.begin(), target.fragments.end(), [](const Product &left, const Product &right)
					  { return left.height > right.height; });
			std::sort(decoy.fragments.begin(), decoy.fragments.end(), [](const Product &left, const Product &right)
					  { return left.height > right.height; });
			if (target.fragments.size() > MaxF)
				target.fragments.resize(MaxF);
			if (decoy.fragments.size() > MaxF)
				decoy.fragments.resize(MaxF);
		...
		inline void generate_decoy()
		{
			...
			for (auto &f : decoy.fragments)
				if (!(f.type & 3) || f.charge <= 0)
				{
					recognise = true;
					break;
				}
			...
			if (!ReverseDecoys)
			{
				...
				if (!mod_state[m])
					pos = m;
				else if (!mod_state[Max(0, m - 1)])
					pos = Max(0, m - 1);
				else if (!mod_state[m + 1])
					pos = m + 1;
				else
					pos = m;
				...
				for (i = 0; i < decoy.fragments.size(); i++)
				{
					...
					if (decoy.fragments[i].type & fTypeY)
						decoy.fragments[i].mz += C_shift / c;
					else
						decoy.fragments[i].mz += N_shift / c;
				}
			}
			else
			{
				...
				for (i = 1; i < seq.size() - 1; i++)
					inv_seq[i] = seq[seq.size() - i - 1];
				for (i = 0; i < seq.size(); i++)
					if (Abs(inv_seq[i] - seq[i]) > 1200.0 * GeneratorAccuracy)
						break;
				if (i == seq.size())
					inv_seq[1] += 12.0, inv_seq[seq.size() - 2] -= 12.0;
		...
		void generate()
		{
			...
			if (AddLosses)
				y_loss_scores(H2O_scores, aas, false), y_loss_scores(NH3_scores, aas, true), to_exp(H2O_scores), to_exp(NH3_scores);
			for (i = cnt = 0; i < gen.size(); i++)
				if (gen[i].mz >= MinFrMz && gen[i].mz <= MaxFrMz)
					if ((gen[i].type & fTypeY) && seq.size() - gen[i].index >= MinFrAAs)
						cnt++;
			...
			for (i = cnt = 0; i < gen.size(); i++)
				if (gen[i].mz >= MinFrMz && gen[i].mz <= MaxFrMz)
					if ((gen[i].type & fTypeY) && seq.size() - gen[i].index >= MinFrAAs)
					{
						...
						if (gen[i].loss == loss_none)
							target.fragments[cnt].height = scores[gen[i].index];
						else if (gen[i].loss == loss_H2O)
							target.fragments[cnt].height = scores[gen[i].index] * H2O_scores[gen[i].index];
						else
							target.fragments[cnt].height = scores[gen[i].index] * NH3_scores[gen[i].index];
	...
	void generate_all()
	{
		...
		if (Threads > 1)
		{
			...
			for (i = 0; i < Threads; i++)
				threads.push_back(std::thread(&Library::generate_spectra, this));
			for (i = 0; i < Threads; i++)
				threads[i].join();
		}
		else
	...
	void smp_decode_spectra(std::vector<std::vector<float>> *_spectra, std::vector<int> *_decoder, int max_frc, std::vector<Lock> *_locks)
	{
		...
		for (i = 0; i < entries.size(); i++)
			if (locks[i].set())
			{
				...
				ions.resize(tot);
				memset(&(ions[0]), 0, tot * sizeof(Ion));
				...
				std::sort(ions.begin(), ions.end(), [&](const Ion &x, const Ion &y)
						  { return x.mz > y.mz; });
				if (ions[MinGenFrNum - 1].mz - E <= E)
					continue;
				double margin = Min(ions[0].mz * 0.001, ions[MinGenFrNum - 1].mz - E);
				filtered.clear();
				for (auto &ion : ions)
					if (ion.mz > margin && (ion.type == fTypeY ? (L + 3 - ion.index) : ion.index) >= MinFrAAs)
						filtered.push_back(ion);
				auto frs = generate_fragments(get_sequence(e.name), filtered, e.target.charge);
				for (j = cnt = 0; j < frs.size(); j++)
					if (frs[j].mz >= MinFrMz && frs[j].mz <= MaxFrMz)
						cnt++;
	...
	void generate_predictions()
	{
		...
		if (Verbose >= 1)
			Time(), std::cout << "Encoding peptides for spectra and RTs prediction\n";
...

		{
			std::vector<std::thread> threads;
			for (i = 0; i < Threads; i++)
				threads.push_back(std::thread(&Library::smp_generate_codes, this, &codes, &dict, &locks));
			for (i = 0; i < Threads; i++)
				threads[i].join();
			for (auto &L : locks)
				L.free();
		...
		for (auto &c : codes)
			if (!c.first.size())
				skipped++;
		if (skipped)
			cout << "WARNING: skipping " << skipped << " precursors; unrecognised modifications?\n";

		auto rt_codes = codes;
		std::sort(codes.begin(), codes.end(), [](auto &x, auto &y)
				  { return x.first < y.first; });
		for (auto &c : rt_codes)
			if (c.first.size())
				c.first[0] = 0;
		std::sort(rt_codes.begin(), rt_codes.end(), [](auto &x, auto &y)
				  { return x.first < y.first; });
		...
		rt:
			...
			flag = (i == 0);
			if (!flag)
				if (curr->first != rt_code->first)
					flag = true;
			if (!flag)
				curr->first.clear();
			else
				last_rt = i, rt_code = curr;
		...
		{
			if (Verbose >= 1)
				Time(), std::cout << "Predicting spectra\n";
			P.predict(spectra, codes, 1, Verbose >= 3);
			if (Verbose >= 1)
				Time(), std::cout << "Predicting RTs\n";
		...
		if (Verbose >= 1)
			Time(), std::cout << "Decoding predicted spectra\n";
		{
			std::vector<std::thread> threads;
			for (i = 0; i < Threads; i++)
				threads.push_back(std::thread(&Library::smp_decode_spectra, this, &spectra, &fr_decoder, 3, &locks));
			for (i = 0; i < Threads; i++)
				threads[i].join();
		}

		if (Verbose >= 1)
			Time(), std::cout << "Decoding RTs\n";
		for (i = 0; i < entries.size(); i++)
		{
	...
	public:
		...
		{
			runs.clear();
			runs.resize(Max(n_s, ms_files.size()));
		...
		{ // files currently must be the same as ms_files
			if (Verbose >= 1)
				Time(), dsout << "Reading quantification information: " << (n_s = files.size()) << " files\n";
			clear();
			...
			for (int i = 0; i < n_s; i++)
			{
				...
				if (_quants == NULL || !QuantInMem)
				{
					...
					if (in.fail() || temp_folder.size())
						in = std::ifstream(location_to_file_name(files[i]) + std::string(".quant"), std::ifstream::binary);
				...
				if (Q->lib_size != lib_n)
					dsout << "ERROR: different library used to generate the .quant file\n";
				...
				if (_quants == NULL || !QuantInMem)
					std::vector<QuantEntry>().swap(Q->entries);
		...
		void quantify()
		{
			...
			if (Verbose >= 1)
				Time(), dsout << "Quantifying peptides\n";
			...
			for (auto it = map.begin(); it != map.end(); it++)
			{
				...
				fr_score.clear();
				for (auto jt = (*v).begin(); jt != (*v).end(); jt++)
					if (jt->second.pr.qvalue <= MaxQuantQvalue)
					{
						for (i = 0; i < jt->second.fr_n; i++)
						{
							int ind = jt->second.fr[i].index;
							if (ind >= fr_score.size())
								fr_score.resize(ind + 1, std::pair<float, int>(0.0, 0));
				...
				for (auto &s : fr_score)
					if (s.second)
						s.first /= (double)s.second;

				if (ExcludeSharedFragments)
				{ // exclude (from quantification) fragments shared by peptides in the same elution group
					...
					for (int pos = ce.first; pos < ce.first + ce.second; pos++)
					{
						...
						if (next != pr_index && lib->entries[next].target.charge == lib->entries[pr_index].target.charge)
						{ // difference only in the label
							auto &lf = lib_e;
							...
							for (i = 0; i < lf.fragments.size(); i++)
							{
								...
								if (flag)
									fr_score[i].first -= 2.0;
							}
						}
					}
				}

				if (RestrictFragments)
				{ // exclude fragments from quantification based on their library annotation
					auto &frs = lib_e.fragments;
					assert(fr_score.size() <= frs.size());
					for (j = 0; j < fr_score.size(); j++)
						if (frs[j].type & fExclude)
							fr_score[j].first -= INF;
				}

				fr_ordered.resize(fr_score.size());
				fr_ordered.assign(fr_score.begin(), fr_score.end());
				std::sort(fr_ordered.begin(), fr_ordered.end());
				for (pos = fr_ordered.size() - 3; pos < fr_ordered.size() - 1; pos++)
					if (fr_ordered[pos].first > -1.0 + E)
						break;
				double margin = Max(-INF / 2.0, fr_ordered[pos].first - E);
				if (NoFragmentSelectionForQuant)
					margin = -INF / 2.0;
				if (GenFrExclusionInfo)
					for (j = 0; j < fr_score.size(); j++)
						if (fr_score[j].second)
						{
							auto &fr = lib_e.fragments[j];
							if (fr_score[j].first >= margin)
								fr.type &= ~fExclude;
							else
								fr.type |= fExclude;
						}
				...
				for (auto jt = (*v).begin(); jt != (*v).end(); jt++)
				{
					...
					if (jt->second.pr.quantity > E)
						jt->second.pr.quality /= jt->second.pr.quantity;
					jt->second.pr.norm = jt->second.pr.level = jt->second.pr.quantity;
				}
			}

			if (TranslatePeaks)
			{ // calculate ratios between labelled and unlabelled peptides; requires TranslatePeaks to ensure co-elution
				for (auto it = map.begin(); it != map.end(); it++)
				{
					auto v = &(it->second);
					int pr_index = it->first;
					...
					if (lib->entries[pr_index].entry_flags & fFromFasta)
						continue; // only spectral library entries
					...
					for (int pos = ce.first; pos < ce.first + ce.second; pos++)
					{
						int next = lib->co_elution[pos];
						if (next != pr_index)
							if (lib->entries[next].target.charge == lib->entries[pr_index].target.charge)
							{
										...
										for (auto jt = (*v).begin(); jt != (*v).end(); jt++)
										{
											...
											if (kt->first == jt->first)
											{
												...
												for (i = 0; i < kt->second.fr_n; i++)
												{
													...
													if (ind >= fr_score.size())
														fr_score.resize(ind + 1, std::pair<float, int>(0.0, 0));
													if (fr_score[ind].second != lib->entries[next].target.fragments[ind].ion_code())
														fr_score[i].first = -INF;
													else if (kt->second.fr[i].corr < fr_score[ind].first)
														fr_score[ind].first = kt->second.fr[i].corr;
												}
												for (i = 0; i < fr_score.size(); i++)
												{
													...
													if (i < exclude.size())
														if (exclude[i])
															fr_score[i].first -= INF * 0.5;
												}
												std::sort(fr_score.begin(), fr_score.end());
												if (fr_score.size())
													if (fr_score[fr_score.size() - 1].first > E)
													{
														int fcnt = 0;
														...
														{
															if (ifr < fr_score.size() - 1)
																if (fr_score[ifr].first < 0.8)
																	break;
															int fr = fr_score[ifr].second;
															for (i = 0; i < jt->second.fr_n; i++)
																if (jt->second.fr[i].index == fr)
																{
																	f1f[fcnt] += jt->second.fr[i].quantity[qFiltered];
																	break;
																}
															for (i = 0; i < kt->second.fr_n; i++)
																if (kt->second.fr[i].index == fr)
																{
														...
														fratio.resize(fr_n);
														for (i = fcnt = 0; i < fr_n; i++)
															if (f1f[i] > E)
																fratio[fcnt++] = f2f[i] / f1f[i];
														if (fcnt <= 2)
														{
															for (i = 0; i < fr_n; i++)
																f1 += f1f[i], f2 += f2f[i];
															if (f1 > E)
																jt->second.pr.ratio += (jt->second.pr.quantity * f2) / f1;
														}
														else
														{
															fratio.resize(fcnt);
															std::sort(fratio.begin(), fratio.end());
					...
					for (auto jt = (*v).begin(); jt != (*v).end(); jt++)
						jt->second.pr.ratio = (jt->second.pr.ratio > E ? (jt->second.pr.quantity / jt->second.pr.ratio) : -1.0);
				}
			}

			if (ms_files.size() <= 1 || IndividualReports)
				return;
			...
			for (auto it = map.begin(); it != map.end(); it++)
			{
				...
				{
					int index = jt->first;
					if (jt->second.pr.qvalue <= NormalisationQvalue)
						sums[index] += jt->second.pr.quantity;
				}
			}
			for (i = k = 0; i < sums.size(); i++)
				if (sums[i] > E)
					av += sums[i], k++;
			if (k)
				av /= (double)k;
			...
			for (auto it = map.begin(); it != map.end(); it++, i++)
			{
				...
				for (auto jt = (*v).begin(); jt != (*v).end(); jt++)
				{
					int index = jt->first;
					if (jt->second.pr.qvalue <= NormalisationQvalue)
						x[index] = jt->second.pr.level, k++, m++;
				}
				if (k >= 2)
				{
					double u = mean(&(x[0]), x.size());
					if (u > E)
						score[i] = sqrt(var(&(x[0]), x.size())) / u;
					else
						score[i] = INF;
				}
				else
			...
			for (auto it = map.begin(); it != map.end(); it++, i++)
			{
				...
				for (auto jt = (*v).begin(); jt != (*v).end(); jt++)
				{
					int index = jt->first;
					if (jt->second.pr.qvalue <= NormalisationQvalue)
						sums[index] += jt->second.pr.level;
				}
			}

			for (i = k = 0, av = 0.0; i < sums.size(); i++)
				if (sums[i] > E)
					av += sums[i], k++;
			if (k)
				av /= (double)k;
			...
			if (LocalNormalisation)
				...
				{ // iter = 0: RT-dependent, iter = 1: signal-dependent normalisation
					if (iter == 0 && NoRTDepNorm)
						continue;
					if (iter == 1 && NoSigDepNorm)
						continue;
					...
					for (auto it = map.begin(); it != map.end(); it++, i++)
					{
						...
						for (auto jt = (*v).begin(); jt != (*v).end(); jt++)
							if (jt->second.pr.qvalue <= NormalisationQvalue)
								norm_totals[i] += jt->second.pr.norm, cnt++;
						if (cnt)
							norm_shares[i] = 1.0 / (double)cnt;
					}
					...
					for (k = 0; k < ms_files.size(); k++)
					{
						...
						for (auto it = runs[k].begin(); it != runs[k].end(); it++)
						{
							...
							if (jt->pr.qvalue <= NormalisationQvalue && norm_shares[i] > E)
								if (norm_shares[i] * (double)ms_files.size() <= 2.0)
						...
						for (auto it = runs[k].begin(); it != runs[k].end(); it++)
						{
							...
							if (high > norm_ind.size())
								high = norm_ind.size(), low = Max(0, high - 2 * LocNormRadius);
							if (norm_saved_ratios[Min(ind, norm_ind.size() - 1)] > E)
								ratio = norm_saved_ratios[Min(ind, norm_ind.size() - 1)];
							else if (high - low <= Max(16, LocNormRadius / 16))
								ratio = 1.0;
							else
							{
								norm_ratios.clear();
								...
								if (norm_ratios.size() & 1)
									ratio = norm_ratios[norm_ratios.size() / 2];
								else
									ratio = 0.5 * (norm_ratios[(norm_ratios.size() / 2) - 1] + norm_ratios[norm_ratios.size() / 2]);
	...
	void extract_proteins()
	{
		...
		for (auto &pg : protein_ids)
		{
			...
			while (std::getline(list, word, ';'))
			{
				...
				for (auto &pr : pg.precursors)
					ins.first->precursors.insert(pr);
			}
		}
		...
		{
			if (Verbose >= 1 && (!FastaSearch || GuideLibrary))
				Time(), dsout << "Finding proteotypic peptides (assuming that the list of UniProt ids provided for each peptide is complete)\n";
			for (auto &e : entries)
				if (protein_ids[e.pid_index].proteins.size() <= 1)
					e.proteotypic = true;
		}
	}
	...
	{
		if (Verbose >= 1)
			Time(), dsout << "Assembling elution groups\n";
	...
	{
		int i, egt = 0, peg;
		for (i = 0; i < elution_groups.size(); i++)
			if (elution_groups[i] > egt)
				egt = elution_groups[i];
		egt++;
		co_elution_index.resize(egt, std::pair<int, int>(0, 1));
		std::set<std::pair<int, int>> ce;
		for (i = 0; i < elution_groups.size(); i++)
			ce.insert(std::pair<int, int>(elution_groups[i], i));
		co_elution.resize(ce.size());
		i = peg = 0;
		if (co_elution_index.size())
			co_elution_index[0].second = 0;
		for (auto it = ce.begin(); it != ce.end(); it++, i++)
		{
			co_elution[i] = it->second;
			if (it->first != peg)
				co_elution_index[it->first].first = i;
			else
				co_elution_index[it->first].second++;
			peg = it->first;
		}
		ce.clear();
	}
	...
	bool load_sptxt(const char *file_name, double acc = 0.000005)
	{
					...
					else if (!memcmp(&(line[0]), "Comment", 7))
					{
						...
						if (ppos != std::string::npos)
						{
							...
							if (rtpos != std::string::npos)
								pr_rt = to_double(&(line[rtpos + 3])), no_rt = false;
							else
							{
								rtpos = line.find("RetentionTime=");
								if (rtpos != std::string::npos)
									pr_rt = to_double(&(line[rtpos + 14])), no_rt = false;
							}
							if (no_rt)
								no_rt_cnt++;
							...
							if (mpos != std::string::npos)
								if (line[mpos + 5] != '0')
								{
									...
									for (i = 0; i < mods; i++)
									{
										...
										mod_name.clear();
										pos = line.find(',', pos + 1) + 1;
										for (j = pos; line[j] && line[j] != '/' && line[j] != ' '; j++)
											mod_name += line[j];
										if (i < mods - 1)
											if (line[j] != '/')
											{
										...
										if (nterm)
											name += '(' + mod_name + ')';
										for (; curr_aa <= mod_pos; curr_aa++)
											name += aas[curr_aa];
										if (!nterm)
											name += '(' + mod_name + ')';
									}
									for (; curr_aa < aas.size(); curr_aa++)
										name += aas[curr_aa];
								...
								if (apos != std::string::npos)
								{
									...
									if (Abs(err / mz) < acc)
									{
										...
										if (is_i == std::string::npos || is_i > apos)
										{
											...
											if (type == fTypeY)
												num = peptide_length(pep_name) - num;
											int frc = 1;
											auto frcpos = ann.find_first_of('^');
											if (frcpos != std::string::npos && frcpos < apos)
												frc = std::stoi(&(ann[frcpos + 1]));
											int loss = loss_none;
		...
		if (no_rt_cnt)
			dsout << "WARNING: " << no_rt_cnt << " precursors without retention time information\n";
		...
		for (auto it = map.begin(); it != map.end(); it++, i++)
		{
			...
			if (entries[i].target.iRT < iRT_min)
				iRT_min = entries[i].target.iRT;
			if (entries[i].target.iRT > iRT_max)
				iRT_max = entries[i].target.iRT;
	...
	bool load(const char *file_name)
	{
		...
		if (Verbose >= 1)
			Time(), dsout << "Loading spectral library " << name << "\n";
		...
		if (get_extension(name) == std::string(".speclib"))
		{
			...
			{
				if (PGLevelSet == 2 && genes.size() >= 2)
					PGLevel = 2;
				else if (PGLevelSet == 1 && names.size() >= 2)
					PGLevel = 1;
			}
			if (fasta_names.size())
				if (Verbose >= 1)
					Time(), dsout << "Library annotated with sequence database(s): " << fasta_names << "\n";
			if (!fasta_files.size() && (genes.size() >= 2 || names.size() >= 2))
				library_protein_stats();
			if (!elution_groups.size())
			...
			for (auto &e : entries)
			{
				...
				if (gc)
					for (auto &f : e.target.fragments)
						if (!(int)f.charge)
							gen_charges = true, gc = 0;
			}
		}
		...
		else
		{
			...
			char delim = '\t';
			if (std::string(file_name).find(".csv") != std::string::npos)
				delim = ',';												   // not for OpenSWATH libraries
			...
			while (std::getline(csv, line))
			{
				...
				for (; end < line.size(); end++)
				{
					if (line[end] == delim && !in_quote)
					{
						...
						{
							if (nw > words.size())
								words.resize(words.size() * 2 + 1);
							words[nw].resize(en - st);
							for (i = st; i < en; i++)
								words[nw][i - st] = line[i];
						}
						else
							words[nw].clear();
						start = end + 1, nw++;
					...
					for (i = 0; i < nw; i++)
					{
						for (auto jt = library_headers.begin(); jt != library_headers.end(); jt++)
							if (jt->find(words[i]) != std::string::npos)
							{
								...
								if (colInd[ind] < 0)
									colInd[ind] = i;
								else if (words[colInd[ind]].size() < words[i].size())
									colInd[ind] = i;
								break;
							}
					}
					for (i = 0; i < libCols; i++)
						if (colInd[i] < 0 && i < libPID)
						{
							if (i == 0)
							{
								for (int j = 0; j < nw; j++)
									if (trim(words[j]) == std::string("ModifiedPeptideSequence"))
									{
										colInd[0] = j;
										break;
									}
							...
							if (Verbose >= 1)
								dsout << "WARNING: cannot find column " + library_headers[i] << "\n";
							csv.close();
							return false;
						}
					if (colInd[libPID] < 0)
					...
					if (colInd[libFrCharge] >= 0)
					{
						...
						if (colInd[libFrType] >= 0)
						{
							...
							if (colInd[libFrNumber] >= 0)
							{
								...
								if (colInd[libFrLoss] >= 0)
									fragment_loss_info = true;
								else
									dsout << "WARNING: no neutral loss information found in the library - assuming fragments without losses\n";
							}
							else
								dsout << "WARNING: no fragment number information found in the library\n";
						}
						else
							dsout << "WARNING: no fragment type information found in the library\n";
					}
					else
						dsout << "WARNING: no fragment charge information found in the library - assuming fragments with charge 1\n";
					continue;
				}
				...
				if (p.charge <= 0)
				{
					p.charge = 1;
					if (!fcw)
						fcw = true, std::cout << "WARNING: fragment charge 0 specified - assuming 1\n";
				}
				if (fragment_type)
				{
					...
					if (wt.size())
					{
						if (wt[0] == 'y')
							...
							ftw = true, p.type = 0, dsout << "WARNING: unknown fragment type " << wt << "\n";
					}
					else if (!ftmw)
						ftmw = true, p.type = 0, dsout << "WARNING: fragment type missing for row number " << cnt << "\n";
					if (p.type && fragment_num_info)
					{
						p.index = std::stoi(words[colInd[libFrNumber]]);
						if (p.type != fTypeB)
							p.index = peptide_length(words[colInd[libPr]]) - p.index;
				...
				if (colInd[libIsDecoy] >= 0)
				{
					...
					if (w.length())
						if (w[0] == '1' || w[0] == 't' || w[0] == 'T')
							decoy_fragment = true;
				}
				if (decoy_fragment && FastaSearch)
					continue; // do not use supplied decoys for the guide library
				if (decoy_fragment)
					gen_decoys = false;
				...
				if (id != prev_id || !prev_id.length())
				{
					...
					if (colInd[libPN] >= 0)
						ins->second.prot->names = words[colInd[libPN]];
					if (colInd[libGenes] >= 0)
						ins->second.prot->genes = words[colInd[libGenes]];
					if (colInd[libPT] >= 0 && words[colInd[libPT]].size())
					{
						char pt = words[colInd[libPT]][0];
						if (pt == 'T' || pt == 't' || pt == '1' || pt == 'Y' || pt == 'y')
							ins->second.proteotypic = true;
						else
							ins->second.proteotypic = false;
					}
				}
				...
				if (colInd[libQ] >= 0)
					pep->lib_qvalue = to_double(words[colInd[libQ]]);
				pep->charge = charge;
				pep->fragments.push_back(p);
			}
			csv.close();
			...
			if (!gen_decoys)
			{
				...
				for (auto &e : entries)
				{
					if (!e.target.fragments.size() && e.decoy.fragments.size())
						dwt++;
					if (e.target.fragments.size() && !e.decoy.fragments.size())
						twd++;
				}
				if (dwt)
					dsout << "WARNING: " << dwt << " decoy precursors don't have matching target precursors; each decoy supplied in the library must correspond to a target with the same modified sequence and charge\n";
				if (twd)
					dsout << "WARNING: " << twd << " target precursors don't have matching decoy precursors; is this intended?\n";
			}
		}

		int ps = proteins.size(), pis = protein_ids.size();
		if (ps)
			if (!proteins[0].id.size())
				ps--;
		if (pis)
			if (!protein_ids[0].ids.size())
				pis--;

		elution_group_index();

		if (Verbose >= 1)
			Time(), dsout << "Spectral library loaded: "
						  << ps << " protein isoforms, "
	...
	void export_prosit()
	{
		...
		for (auto &e : entries)
			prs.insert(std::pair<std::pair<std::string, int>, float>(std::pair<std::string, int>(to_prosit(e.name), e.target.charge), e.target.mz));
		for (auto &s : prs)
			out << s.first.first << ",30," << s.first.second << '\n';
		out.close();
		if (Verbose >= 1)
			Time(), dsout << "Prosit input saved to " << prosit_file << "\n";
	}
	...
	void load_protein_annotations(Fasta &fasta)
	{
		...
		for (int i = 0; i < entries.size(); i++)
		{
			...
			if (OverwriteLibraryPGs || (entries[i].entry_flags & fFromFasta))
			{
				...
				else
				{
					...
					if (pos != fasta.dict.end())
						if (pos->first == key.first)
						{ // checks needed when !(entries[i].entry_flags & fFromFasta)
							std::string ids;
							for (auto &seq : pos->second)
							{
								...
								for (int shift = 0; shift <= sequence.size() - stripped.size(); shift++)
								{
									...
									if (found != std::string::npos)
									{
										...
										if (found == 0)
											l_cut = true;
										else if (found == 1 && NMetExcision && (sequence[0] == 'M' || sequence[0] == 'X'))
											l_cut = true;
										else
											l_cut = do_cut(sequence[found - 1], sequence[found]);
										if (l_cut)
										{
											if (found >= sequence.size() - stripped.size())
												r_cut = true;
											else
												r_cut = do_cut(sequence[found + stripped.size() - 1], sequence[found + stripped.size()]);
										}
										if (l_cut && r_cut)
										{
											if (!ids.size())
												ids = fasta.ids[seq];
											else if (!name_included(ids, fasta.ids[seq]))
												ids += std::string(";") + fasta.ids[seq];
											break;
										}
									}
									else
		...
		extract_proteins();
		if (Verbose >= 1)
			Time(), dsout << entries.size() << " precursors generated\n";
	...
	{
		if (Verbose >= 1)
			Time(), dsout << "Processing FASTA\n";

		int i = entries.size(), cnt = 0;
		if (!i && !InSilicoRTPrediction)
			iRT_min = -1.0, iRT_max = 1.0;
	...
	void library_protein_stats()
	{
		...
		if (ns)
			if (!names[0].size())
			{
				ns--;
				if (Verbose >= 1)
					Time(), dsout << "Protein names missing for some isoforms\n";
			}
		if (gs)
		{
			if (!genes[0].size())
				gs--;
			if (Verbose >= 1)
				Time(), dsout << "Gene names missing for some isoforms\n";
		}
		if (Verbose >= 1)
			Time(), dsout << "Library contains " << ns << " proteins, and " << gs << " genes\n";
	}
	...
	void annotate()
	{
		...
		for (auto &p : proteins)
			name.insert(p.name), gene.insert(p.gene);
	...
	{
		for (auto &pid : pgs)
			pid.annotate(proteins, names, genes);
	}

	void save(const std::string &file_name, std::vector<int> *ref, bool searched, bool decoys = false)
	{
		if (Verbose >= 1)
			Time(), dsout << "Saving spectral library to " << file_name << "\n";
		std::ofstream out(file_name, std::ofstream::out);
		if (out.fail())
		{
			dsout << "ERROR: cannot write to " << file_name << ". Check if the destination folder is write-protected or the file is in use\n";
			return;
		}
		...
		{
			cnt++;
			if (searched && it.best_run < 0 && ((FastaSearch && (it.entry_flags & fFromFasta)) || !SaveOriginalLib))
				continue;
			if (!searched && it.target.fragments.size() < MinF)
				continue;
			if (ref != NULL)
			{
			...
			bool recognise = false;
			std::vector<Ion> ions;
			for (auto &f : it.target.fragments)
				if (!(f.type & 3) || f.charge <= 0)
				{
					recognise = true;
					if (!rec_info)
					{
						rec_info = true;
						if (Verbose >= 1)
							Time(), dsout << "Some precursors lack fragment annotation; annotating...\n";
					}
					break;
				}
			if (recognise)
			...
			else
			{
				...
				for (int i = 0; i < ions.size(); i++)
					ions[i].init(frs[i]), ions[i].type = frs[i].type;
			}
			auto &pep = it.target;
			...
			for (int dc = 0; dc <= 1; dc++)
			{
				...
				for (int fr = 0; fr < pep.fragments.size(); fr++)
				{
					...
					if (fr_charge)
					{
						...
						if (ExportRecFragOnly && !dc)
							fr_mz = ions[fr].mz;
					}
					else
					{ // unrecognised
						fr_type = fr_loss = fr_num = 0;
						if (ExportRecFragOnly)
							continue;
					}
					...
					if (pep.fragments[fr].height < MinGenFrInt)
						continue;
					int pg = (InferPGs && searched) ? it.pg_index : it.pid_index;
					auto fr_name = name + std::string("_") + std::to_string(char_from_type[fr_type & 3]) + std::string("_") + std::to_string(fr_charge) + std::string("_") + std::to_string(fr_loss) + std::string("_") + std::to_string(fr_num);
		...
		{
			if (!decoys_written)
				Time(), dsout << targets_written << " precursors saved\n";
			else
				Time(), dsout << targets_written << " target and " << decoys_written << " decoy precursors saved\n";
		}
		if (skipped)
			dsout << "WARNING: " << skipped << " precursors with unrecognised library fragments were skipped\n";
		if (empty)
			dsout << "WARNING: " << empty << " precursors without any fragments annotated were skipped\n";
	}

	bool save(const std::string &file_name)
	{
		if (Verbose >= 1)
			Time(), dsout << "Saving the library to " << file_name << "\n";
	...
	void init(bool decoys)
	{
		...
		for (auto it = entries.begin(); it != entries.end(); it++)
			if (it->lock.set())
			{
				if (it->target.charge <= 0)
					it->target.charge = 2, corrected_charge = true;
				if (gen_decoys && decoys)
					it->generate_decoy();
				if (it->decoy.charge <= 0 && decoys)
					it->decoy.charge = 2, corrected_charge = true;
				else if (UseIsotopes && gen_charges && !(it->entry_flags & fFromFasta))
					it->annotate_charges();
				it->init();
			}
		if (corrected_charge)
			dsout << "WARNING: at least one library precursor had non-positive charge; corrected to charge = 2\n";
	}
	...
	void initialise(bool decoys)
	{
		...
		skipped = 0;
		if (Verbose >= 1)
			Time(), dsout << "Initialising library\n";
		if (Threads > 1)
		{
			std::vector<std::thread> threads;
			for (i = 0; i < Threads; i++)
				threads.push_back(std::thread(&Library::init, this, decoys));
			for (i = 0; i < Threads; i++)
				threads[i].join();
		}
		else
		...
		for (i = 0; i < entries.size(); i++)
			entries[i].lock.free();
		if (skipped)
			dsout << "WARNING: " << skipped
				  << " library precursors were skipped due to unrecognised fragments when generating decoys.\n";
		if (ExportLibrary)
			save(out_lib_file, NULL, false, ExportDecoys);
	}

	void infer_proteins()
	{ // filters precursors and proteins at q_cutoff level and infers protein groups
		int i;
		if (PGsInferred || !protein_ids.size() || !proteins.size())
			return;
		protein_groups.clear();

		if (Verbose >= 1)
			Time(), dsout << "Assembling protein groups\n";
		...
		for (auto it = info.map.begin(); it != info.map.end(); it++)
		{
			...
			for (auto jt = v->begin(); jt != v->end(); jt++)
			{
				...
				if (pr->qvalue <= ProteinIDQvalue)
				{
					...
					bool first = true;
					id_class.clear();
					id_class.resize(pid.proteins.size(), 0);
					int i = -1, max_class = -1;
					for (auto &p : pid.proteins)
					{
						i++;
						if (SwissProtPriority)
							if (proteins[p].swissprot)
								id_class[i] += 1;
						auto pos = std::lower_bound(info.proteins[s].begin(), info.proteins[s].end(), p);
						if (pos != info.proteins[s].end())
							if (*pos == p)
								id_class[i] += 10;
						if (PGLevel == 0)
							if (!proteins[p].id.size())
								id_class[i] -= 100;
						if (PGLevel == 1)
							if (!proteins[p].name.size())
								id_class[i] -= 100;
						if (PGLevel == 2)
							if (!proteins[p].gene.size())
								id_class[i] -= 100;
						if (id_class[i] > max_class)
							max_class = id_class[i];
						if (id_class[i] > max_protein_score[p])
							max_protein_score[p] = id_class[i];
					}
		...
		for (auto &pr : pgs)
		{
			int max = -INF;
			for (auto &prot : pr)
				if (max_protein_score[prot] > max)
					max = max_protein_score[prot];
			for (auto &prot : pr)
				if (max_protein_score[prot] < max)
					pr.erase(prot);
		}
		...
		for (i = 0; i < entries.size(); i++)
		{
			...
			if (!pgs[i].size() && ins.second)
				ins.first->proteins = pid.proteins;
			for (auto &p : pgs[i])
				ins.first->proteins.insert(p);
			ins.first->precursors.push_back(i);
		}
		protein_groups.clear();
		...
		bool annotate = false;
		if (names.size())
			if (names[names.size() - 1] != "" && names[names.size() - 1] != " ")
				annotate = true;
		if (genes.size())
			if (genes[genes.size() - 1] != "" && genes[genes.size() - 1] != " ")
				annotate = true;
		if (annotate)
			annotate_pgs(protein_groups);
	}
	...
	int generate_gene_groups()
	{
		...
		gg_index.clear();
		gg_index.resize(entries.size(), 0);
		auto &prot = (InferPGs ? protein_groups : protein_ids);

		for (auto &pg : prot)
			gg.insert(pg.genes);
		ggs.clear();
		ggs.insert(ggs.begin(), gg.begin(), gg.end());
		gg.clear();
		if (!ggs.size())
			ggs.resize(1);
		int n = ggs.size();
		...
		for (int i = 0; i < n; i++)
			gene_groups[i].ids = ggs[i];
		for (int i = 0; i < gg_index.size(); i++)
			gene_groups[gg_index[i]].precursors.push_back(i);
		return n;
	}
			...
			if (locks[cnt++].set())
			{
				...
				for (i = 0; i < prs_n; i++)
				{
					...
					if (pos != info.map.end())
					{
						...
						for (auto jt = v->begin(); jt != v->end(); jt++)
						{
							...
							if (unique && !entries[prp->index].proteotypic)
								continue;
							if (prp->qvalue <= q_cutoff && prp->norm > E)
								quantities[s * prs_n + i] = log(prp->norm), prn[i]++;
						}
					}
				}
				...
				for (i = 0; i < n_s; i++)
				{
					...
					for (k = 0; k < prs_n; k++)
						if (pi[k] > max)
							max = pi[k];
					if (max > lfq_mar)
						reference[i] = max, indices.push_back(i);
					else
						reference[i] = -INF;
				}
				...
				for (i = 0; i < ni; i++)
				{
					for (j = i + 1; j < ni; j++)
					{
						...
						for (k = 0; k < prs_n; k++)
							if (pi[k] > lfq_mar && pj[k] > lfq_mar)
								ratios.push_back(pi[k] - pj[k]);
						if (ratios.size())
						{
							double median;
							if (ratios.size() >= 2)
							{
								...
								if (ratios.size() & 1)
									...
									median = 0.5 * (ratios[(ratios.size() / 2) - 1] + ratios[ratios.size() / 2]);
							}
							else
								median = ratios[0];
				...
				for (i = 0; i < prs_n; i++)
				{
					...
					if (pos != info.map.end())
					{
						...
						for (auto jt = v->begin(); jt != v->end(); jt++)
						{
							...
							{
								if (!unique)
									prp->max_lfq = exp(quantities[s]);
								else
									prp->max_lfq_unique = exp(quantities[s]);
							}
						}
					}
				}
	...
	void group_qvalues(int stage = 0)
	{
		...
		if (Verbose >= 1 && stage == 0)
			Time(), dsout << "Calculating q-values for protein and gene groups\n";
		...
		for (i = 0; i < entries.size(); i++)
		{
			...
			for (s = 0; s < n_s; s++)
				if (info.detectable_precursors[s * info.n_det_pr_block + ind] & (1 << shift))
					prs[s * n_p + p]++;
		}
		...
		for (auto it = info.map.begin(); it != info.map.end(); it++)
		{
			...
			for (auto jt = v->begin(); jt != v->end(); jt++)
			{
				...
				if (prs[s * n_p + p] < E)
					dsout << "ERROR: undetectable precursor id reported in the .quant file\n";
		...
		for (s = 0; s < n_s; s++)
		{
			for (int at = 0; at <= 1; at++)
			{
				...
				for (i = 0; i < n_p; i++)
				{
					...
					if (pos.second)
					{
						if (pos.first != cs_qv.begin() && std::prev(pos.first)->second < pair.second)
							pos.first->second = std::prev(pos.first)->second;
						else
							for (auto jt = std::next(pos.first); jt != cs_qv.end() && jt->second > pair.second; jt++)
								jt->second = pair.second;
					}
				}
	...
	void quantify_proteins(int N, double q_cutoff, int stage = 0)
	{ // top N method for protein quantification
		if (InferPGs && !stage)
			infer_proteins();
		auto &prot = (InferPGs ? protein_groups : protein_ids);
		...
		if (Verbose >= 1 && stage == 0)
			Time(), dsout << "Quantifying proteins\n";

		if (stage == 1)
			n = generate_gene_groups();
		if (!n)
		{
			if (stage < 1)
				quantify_proteins(N, q_cutoff, stage + 1);
			return;
		}
		...
		for (pass = 0; pass <= 3; pass++)
		{
			for (auto it = info.map.begin(); it != info.map.end(); it++)
			{
				...
				for (auto jt = v->begin(); jt != v->end(); jt++)
				{
					...
					if (pass == 0)
						pr->max_lfq = pr->max_lfq_unique = 0.0;
					int ggi = (stage == 1) ? gg_index[pr->index] : 0;
					if (stage == 1)
						if (!gene_groups[gg_index[pr->index]].ids.size())
						{
							if (pass == 0)
								pr->gene_quantity = pr->gene_norm = 0.0;
							continue;
						}
					...
					if (pass == 0)
					{
						if (q<*pos_q && * pos_q> q_cutoff)
							*pos_q = Max(q_cutoff, q);
					}
					else if (pass == 1)
					{
						if (q <= *pos_q)
					...
					{
						if (q <= *pos_q && quantity >= pos_l[N - 1])
							quant[index] += pr->quantity, norm[index] += pr->norm;
					}
					else if (pass == 3)
					{
						if (stage == 1)
							pr->gene_quantity = quant[index], pr->gene_norm = norm[index];
						else
							pr->pg_quantity = quant[index], pr->pg_norm = norm[index];
					}
				}
			}
		}
		...
		{ // gene groups
			...
			for (i = 0; i < Threads; i++)
				thr.push_back(std::thread(&Library::max_lfq_quant, this, &proteins, &locks, q_cutoff, false));
			for (i = 0; i < Threads; i++)
				thr[i].join();
		}
		...
		{ // unique genes
			...
			std::vector<std::thread> thr;
			for (i = 0; i < Threads; i++)
				thr.push_back(std::thread(&Library::max_lfq_quant, this, &proteins, &locks, q_cutoff, true));
			for (i = 0; i < Threads; i++)
				thr[i].join();
		}
	...
	void report_pr_matrix(const std::string &file_name)
	{ // must only be called after the regular report() below
		if (Verbose >= 1)
			Time(), dsout << "Saving precursor levels matrix\n";
		std::ofstream out(file_name, std::ofstream::out);
		if (out.fail())
		{
			dsout << "ERROR: cannot write to " << file_name << ". Check if the destination folder is write-protected or the file is in use\n";
		...
		for (auto it = info.map.begin(); it != info.map.end(); it++)
		{
			...
			for (auto jt = (*v).begin(); jt != (*v).end(); jt++)
			{
				...
				if (q <= MatrixQValue)
					quants.push_back(std::pair<int, float>(jt->first, jt->second.pr.norm));
			}
			if (!quants.size())
				continue;
			...
			out << prot[pg].ids.c_str() << '\t'
				...
				<< prot[pg].names.c_str() << '\t'
				<< prot[pg].genes.c_str() << '\t';
			if (prot[pg].proteins.size())
				out << proteins[*(prot[pg].proteins.begin())].description.c_str() << '\t';
			else
				out << '\t';
		...
		if (Verbose >= 1)
			Time(), dsout << "Precursor levels matrix (" << MatrixQValue * 100.0 << "% precursor and protein group FDR) saved to " << file_name << ".\n";
	}

	void report_pg_matrix(const std::string &file_name, bool genes = false, bool unique = false)
	{ // must only be called after the regular report() below
		if (Verbose >= 1)
		{
			if (!genes)
				Time(), dsout << "Saving protein group levels matrix\n";
			else if (!unique)
				Time(), dsout << "Saving gene group levels matrix\n";
			else
				Time(), dsout << "Saving unique genes levels matrix\n";
		}
		std::ofstream out(file_name, std::ofstream::out);
		if (out.fail())
		{
		...
		for (auto it = info.map.begin(); it != info.map.end(); it++)
		{
			...
			if (genes && unique)
				if (!entry->proteotypic)
					continue;

			for (auto jt = (*v).begin(); jt != (*v).end(); jt++)
			{
				if (PG[shift + jt->first] > E)
					continue;
				if (!genes)
					q = Max(jt->second.pr.qvalue, jt->second.pr.pg_qvalue), qt = jt->second.pr.pg_norm;
				else if (!unique)
					q = Max(jt->second.pr.qvalue, jt->second.pr.gg_qvalue), qt = jt->second.pr.max_lfq;
				else
					q = Max(jt->second.pr.qvalue, jt->second.pr.protein_qvalue), qt = jt->second.pr.max_lfq_unique;
				if (q <= MatrixQValue)
					PG[shift + jt->first] = qt;
			}
		}
		...
		for (i = 0; i < n_pg; i++)
		{
			...
			out << prot[i].ids.c_str() << '\t'
				...
				<< prot[i].names.c_str() << '\t'
				<< prot[i].genes.c_str();
			if (prot[i].proteins.size())
				out << '\t' << proteins[*(prot[i].proteins.begin())].description.c_str();
			else
				out << '\t';
		...
		{
			if (!genes)
				Time(), dsout << "Protein group levels matrix (" << MatrixQValue * 100.0 << "% precursor FDR and protein group FDR) saved to " << file_name << ".\n";
			else if (!unique)
				Time(), dsout << "Gene groups levels matrix (" << MatrixQValue * 100.0 << "% precursor FDR and gene group FDR) saved to " << file_name << ".\n";
			else
				Time(), dsout << "Unique genes levels matrix (" << MatrixQValue * 100.0 << "% precursor FDR and unique protein FDR) saved to " << file_name << ".\n";
		}
	}
	...
	void report(const std::string &file_name)
	{
		...
		for (int i = 0; i < runs.size(); i++)
			runs[i] = remove_extension(get_file_name(ms_files[i]));
		...
		for (auto it = info.map.begin(); it != info.map.end(); it++)
		{
			...
			for (auto jt = (*v).begin(); jt != (*v).end(); jt++)
			{
				if (jt->second.pr.qvalue > ReportQValue)
					continue;
				if (jt->second.pr.pg_qvalue > ReportProteinQValue)
					continue;
				...
				if (jt->second.pr.gene_quantity > E)
				{
					...
					if (jt->second.pr.max_lfq > E)
						out << jt->second.pr.max_lfq << '\t';
					else
						out << '\t';
					if (jt->second.pr.max_lfq_unique > E)
						out << jt->second.pr.max_lfq_unique << '\t';
					else
						out << '\t';
				}
				else
				...
				if (jt->second.pr.ratio >= 0.0)
					out << jt->second.pr.ratio << '\t';
				else
					out << "\t";
				...
				if (ExtendedReport)
				{
					...
					for (int fr = 0; fr < jt->second.fr_n; fr++)
						out << jt->second.fr[fr].quantity[qTotal] << ";";
					out << '\t';
					for (int fr = 0; fr < jt->second.fr_n; fr++)
						out << jt->second.fr[fr].quantity[qFiltered] << ";";
					out << '\t';
					for (int fr = 0; fr < jt->second.fr_n; fr++)
						out << jt->second.fr[fr].corr << ";";
					out << '\t' << jt->second.pr.apex;

					if (ReportLibraryInfo)
					{
						if ((entry->entry_flags & fFromFasta) && !entry->target.fragments.size())
							entry->generate();
						out << '\t' << entry->target.mz << '\t';
						for (int fr = 0; fr < jt->second.fr_n; fr++)
						{
							...
							if (cfr.loss != loss_none)
								out << '-' << name_from_loss[cfr.loss].c_str();
							out << '^' << (int)cfr.charge << '/' << cfr.mz << ';';
						}
					}
#ifdef TDFREADER
					if (MobilityReport)
						out << '\t' << jt->second.pr.one_over_K0;
#endif
				}
#if REPORT_SCORES
				for (int i = 0; i < pN; i++)
					out << '\t' << jt->second.pr.scores[i];
				for (int i = 0; i < pN; i++)
					out << '\t' << jt->second.pr.decoy_scores[i];
#endif
				out << "\n";
			}
		}
		...
		if (DecoyReport)
			for (auto it = info.decoy_map.begin(); it != info.decoy_map.end(); it++)
			{
				...
				for (auto jt = (*v).begin(); jt != (*v).end(); jt++)
				{
					if (jt->second.qvalue > ReportQValue)
						continue;
		...
		out.close();
		if (Verbose >= 1)
			Time(), dsout << "Report saved to " << file_name << ".\n";
	...
	void gene_report(const std::string &file_name)
	{
		...
		if (!size || !gg_index.size())
			return;

		if (Verbose >= 1)
			Time(), dsout << "Writing gene report\n";
		for (auto it = info.map.begin(); it != info.map.end(); it++)
		{
			auto v = &(it->second);
			auto entry = &(entries[it->first]);
		...
		for (int i = 0; i < gene_groups.size(); i++)
			for (int j = 0; j < s; j++)
			{
				int index = i * s + j;
				if (gq[index] > E)
		...
		out.close();
		if (Verbose >= 1)
			Time(), dsout << "Gene report saved to " << file_name << ".\n";
	}
	...
	void stats_report(const std::string &file_name)
	{
		...
		if (Verbose >= 1)
			Time(), dsout << "Stats report saved to " << file_name << "\n";
	}
	...
	void tic_report(const std::string &file_name)
	{
		...
		out.close();
		if (Verbose >= 1)
			Time(), dsout << "TICs saved to " << file_name << "\n";
	}
	...
	void XIC_report(const std::string &file_name)
	{
		...
		for (auto &xic : XICs)
		{
			...
			for (in = 0; in <= 1; in++)
			{
				out << ms_files[xic.run] << '\t'
					...
					<< (in ^ 1) << '\t';
				if (xic.level == 1)
					out << xic.pr_mz << "\tNA\tNA\tNA\tNA";
				else
					out << xic.fr_mz << '\t' << char_from_type[xic.fr.type & 3] << '\t' << ((int)xic.fr.charge) << '\t'
						<< ((xic.fr.type & fTypeB) ? ((int)xic.fr.index) : aas.size() - ((int)xic.fr.index)) << '\t' << name_from_loss[xic.fr.loss].c_str();

				if (!in)
				{
					for (i = 0; i < W; i++)
						out << '\t' << xic.peaks[i].first;
					out << '\n';
				}
				if (in)
				{
					for (i = 0; i < W; i++)
						out << '\t' << xic.peaks[i].second;
					out << '\n';
				}
			}
			cnt++;
		...
		out.close();
		if (Verbose >= 1)
			Time(), dsout << cnt << " XICs saved to " << file_name << "\n";
	}
...
public:
	...
	void gen_decoy_list()
	{
		...
		for (auto &e : entries)
		{
			...
			if (pos == cs_qv.end())
				e.pr.profile_qvalue = 1.0;
			else
				e.pr.profile_qvalue = pos->second;
		}
	}
	...
	Profile(std::vector<std::string> &files, int lib_size = 0)
	{
		...
		if (decoy_list)
			dq.resize(MaxLibSize, 1.0), tq.resize(MaxLibSize, 1.0);
		...
		for (int i = 0; i < files.size(); i++)
		{
			...
			for (auto it = Q->entries.begin(); it != Q->entries.end(); it++)
			{
				...
				if (pos >= entries.size())
					entries.resize(pos + 1 + pos / 2);
				if (entries[pos].index < 0)
					entries[pos] = *it;
				else if (it->pr.qvalue < entries[pos].pr.qvalue)
					entries[pos].pr = it->pr;

				if (decoy_list)
				{
					float sc = Max(it->pr.qvalue, it->pr.lib_qvalue) + it->pr.lib_qvalue;
					if (pos >= tq.size())
						tq.resize(pos + 1 + pos / 2, 1.0);
					if (sc <= ReportQValue && sc < tq[pos])
						tq[pos] = sc;
				}
			}
			if (decoy_list)
			{
				for (auto it = Q->decoys.begin(); it != Q->decoys.end(); it++)
				{
					...
					if (pos >= dq.size())
						dq.resize(pos + 1 + pos / 2, 1.0);
					if (sc <= ReportQValue && sc < dq[pos])
						dq[pos] = sc;
				}
			}
			if (!QuantInMem)
				std::vector<QuantEntry>().swap(Q->entries), std::vector<DecoyEntry>().swap(Q->decoys);
		}

		if (decoy_list)
			gen_decoy_list();
		std::vector<float>().swap(tq);
		std::vector<float>().swap(dq);
	}
	...
	Profile(Library *lib)
	{
		...
		for (auto it = info.map.begin(); it != info.map.end(); it++)
		{
			...
			for (auto jt = v->begin(); jt != v->end(); jt++)
			{
				...
				if (entries[pos].index < 0)
					entries[pos] = qr;
				else if (qr.pr.qvalue < entries[pos].pr.qvalue)
					entries[pos] = qr;
...
void annotate_library(Library &lib, Fasta &fasta)
{
	for (auto &e : lib.proteins)
	{
		...
		if (pos != fasta.proteins.end())
			if (pos->id == e.id)
				e.name = pos->name, e.gene = pos->gene, e.description = pos->description, e.swissprot = pos->swissprot;
	}
	lib.annotate();
	lib.annotate_pgs(lib.protein_ids);
	for (auto &le : lib.entries)
	{
		if (lib.protein_ids[le.pid_index].proteins.size() == 1)
			le.proteotypic = true;
		else
		{
			if (PGLevel == 0)
				le.proteotypic = false;
			else if (PGLevel == 1)
				le.proteotypic = (lib.protein_ids[le.pid_index].name_indices.size() == 1);
			else if (PGLevel == 2)
				le.proteotypic = (lib.protein_ids[le.pid_index].gene_indices.size() == 1);
		}
	}
}
...
void bagging_score(int thread_id, std::vector<NN> *net, int in, std::vector<float *> *input, std::vector<float> *nn_sc)
{
	...
	for (int n = 0; n * Threads + thread_id < bagging; n++)
	{
		...
		for (i = 0; i < input->size(); i += cnt)
		{
			...
			for (j = 0; j < cnt; j++)
				(*nn_sc)[(i + j) * 2 * bagging + ind] = result->data[2 * j], (*nn_sc)[(i + j) * 2 * bagging + ind + 1] = result->data[2 * j + 1];
			free(dataset);
		}
	}
}
...
public:
	...
	void run(int epochs)
	{
		...
		for (i = 0; i < net.size(); i++)
			net[i].maxIters = (size[Min(i, size.size() - 1)] * epochs) / net[i].batchSize;

		if (net.size() > 1 && Threads > 1)
		{
			std::vector<std::thread> thr;
			for (i = 0; i < Threads; i++)
				thr.push_back(std::thread(train, i, &net));
			for (i = 0; i < Threads; i++)
				thr[i].join();
		}
		else
	...
	void predict(std::vector<float *> &data, std::vector<float *> &prediction)
	{
		...
		if (bagging > 1 && Threads > 1)
		{
			std::vector<std::thread> thr;
			for (i = 0; i < Threads; i++)
				thr.push_back(std::thread(&bagging_score, i, &net, in, &data, &nn_sc));
			for (i = 0; i < Threads; i++)
				thr[i].join();
		}
		else
			bagging_score(0, &net, in, &data, &nn_sc);
		...
		for (i = 0; i < data.size(); i++)
		{
			...
			int cnt = 0;
			for (j = 0; j < bagging; j++)
				if (nn_sc[i * bagging * 2 + j] >= 0.0)
					sc_one += nn_sc[i * bagging * 2 + j], sc_two += nn_sc[i * bagging * 2 + j + 1], cnt++;
			if (cnt)
				prediction[i][0] = sc_one / (double)cnt, prediction[i][1] = sc_two / (double)cnt;
			else
				prediction[i][0] = prediction[i][1] = -1.0;
		}
	}
...
void learn_from_library(const std::string &file_name)
{
	...
	Library lib;
	if (!lib.load(&(file_name[0])))
		Throw("Cannot load the library");
	if (Verbose >= 1)
		Time(), dsout << "Learning peptide characteristics\n";
	...
	for (auto &e : lib.entries)
	{
		...
		if (gen_acc > GeneratorAccuracy + E)
			continue;
		...
		if (InSilicoRTPrediction)
		{
			...
			for (i = 0; i < M; i++)
				if (Abs(count[i]) > E)
					TL_RT.push_back(T(pos_RT, i, (double)count[i]));
			b_RT.push_back(e.target.iRT);
			pos_RT++;
		}

		y_index.resize(n = aas.size());
		for (i = 0; i < y_index.size(); i++)
			y_index[i] = -1;
		for (i = 0; i < frs.size(); i++)
			if (frs[i].charge)
			{
				auto &fr = frs[i];
				if (fr.charge == 1 && (fr.type & fTypeY))
					if (fr.loss == loss_none)
						y_index[fr.index] = i;
			}
		...
		if (AddLosses)
			for (i = 0; i < frs.size(); i++)
				if (frs[i].charge)
				{
					auto &fr = frs[i];
					if (fr.charge == 1 && (fr.type & fTypeY))
						if (y_index[fr.index] >= 0)
							if (fr.loss == loss_H2O || fr.loss == loss_NH3)
							{
								...
								for (j = fr.index; j < aas.length(); j++)
									TL_l.push_back(T(pos_l, y_loss_aa(aas[j], fr.loss == loss_NH3), 1.0));
								pos_l++;
							}
				}
		if (AddLosses && !pos_l)
		{
			AddLosses = false;
			if (Verbose >= 1)
				Time(), dsout << "No H2O/NH3 neutral losses in the library, losses will not be used\n";
		}
		...
		for (i = 2; i < y_index.size(); i++)
			if (y_index[i] >= 0 && y_index[i - 1] >= 0)
			{
				...
				for (j = 0; j < aas.length(); j++)
					TL_y.push_back(T(pos_y, y_delta_composition_index(aas[j]), 1.0));
		...
		for (int i = 0; i < frs.size(); i++)
			if (frs[i].charge)
			{
				...
				if (fr.charge == 1 && fr.loss == loss_none)
					if (fr.type & fTypeY)
						actual_y[fr.index] = e.target.fragments[i].height;
			}
		y_scores(predicted_y, e.target.charge, aas);
		for (i = 2; i < aas.size(); i++)
			if (actual_y[i] > E && actual_y[i - 1] > E)
				s2_y += Sqr(log(actual_y[i] / actual_y[i - 1]) - (predicted_y[i] - predicted_y[i - 1])), s2_cnt++;
		to_exp(predicted_y);
#if (HASH > 0)
...
public:
	...
	{
		for (int i = 1; i < pN; i++)
			weights[i] = guide_weights[i] = 0.0;
		weights[0] = guide_weights[0] = 1.0;
		default_weights = true;
	}

	void copy_weights(float *dst, float *src)
	{
		for (int i = 0; i < pN; i++)
			dst[i] = src[i];
	}
	...
	void check_weights()
	{
		...
		if (weights[pdRT] > 0.0)
			weights[pdRT] = 0.0;
		if (guide_weights[pdRT] > 0.0)
			guide_weights[pdRT] = 0.0;
		for (int i = 0; i < TopF; i++)
		{
			if (weights[pAcc + i] > 0.0)
				weights[pAcc + i] = 0.0;
			if (guide_weights[pAcc + i] > 0.0)
				guide_weights[pAcc + i] = 0.0;
		}
	}
	...
	Run(int _run_index)
	{
		...
		for (int i = 0; i < 20; i++)
			pars.push_back(p_none); // pAAs
#endif
		pars.push_back(p_none); // pRT
		for (int i = 0; i < TopF; i++)
			pars.push_back(p_end); // pAcc
		for (int i = 1; i < auxF; i++)
			pars.push_back(p_none); // pRef
		for (int i = 0; i < TopF; i++)
			pars.push_back(i < TopF - 1 ? p_end : p_none); // pSig
		for (int i = 0; i < auxF; i++)
			pars.push_back(p_none); // pCorr
		for (int i = 0; i < TopF; i++)
			pars.push_back(p_none); // pShadowCorr
		for (int i = 0; i < nnW; i++)
			pars.push_back(p_none); // pShape
#if Q1
		for (int i = 0; i < qL; i++)
			pars.push_back(p_learn); // pQLeft
		for (int i = 0; i < qL; i++)
			pars.push_back(p_learn); // pQRight
		for (int i = 0; i < qL; i++)
			pars.push_back(p_learn); // pQPos
		for (int i = 0; i < qL; i++)
			pars.push_back(p_learn); // PQNFCorr
		for (int i = 0; i < qL; i++)
			pars.push_back(p_learn); // pQCorr
#endif
		assert(pars.size() == pN);
		...
		if (Calibrate && !QuantOnly)
			MassAccuracy = MassAccuracyMs1 = CalibrationMassAccuracy;
		MassCorrection.resize(1 + MassCalBinsMax * 2, 0.0);
		MassCorrectionMs1.resize(1 + MassCalBinsMax * 2, 0.0);
		MassCalSplit.resize(MassCalBinsMax + 1, 0.0);
		MassCalSplitMs1.resize(MassCalBinsMax + 1, 0.0);
		MassCalCenter.resize(MassCalBinsMax + 1, 0.0);
		MassCalCenterMs1.resize(MassCalBinsMax + 1, 0.0);
	...
	{
		if (ms2h[i].window_high > ms2h[j].window_low - E && ms2h[i].window_low < ms2h[j].window_low)
			return true;
		if (ms2h[j].window_high > ms2h[i].window_low - E && ms2h[j].window_low < ms2h[i].window_low)
			return true;
		return false;
	}
	...
	inline double predicted_mz_ms1(double *t, double mz, double rt)
	{
		...
		if (rt <= MassCalCenterMs1[0])
			...
			for (int i = 1; i < MassCalBinsMs1; i++)
	...
	template <bool seek>
	inline double cscore(float *scores, bool guide = false)
	{
		if (seek)
			return scalar(scores, guide ? guide_weights : weights, par_seek, pN);
		else
			return scalar(scores, guide ? guide_weights : weights, par_learn, pN);
	}
...
#define WinCenter(x) (0.5 * (ms2h[x].window_low + ms2h[x].window_high))
	...
	void q1_profiles(float *profiles, float *coo, int apex, float *fmz, int QS, int m)
	{
		...
		for (i = 0; i < QS; i++)
			coo[i] = -INF;
		for (j = 0; j < m; j++)
			profiles[j * QW + QS] = ms2h[apex].level<false>(fmz[j], MassAccuracy);
		coo[QS] = wc;
					...
					for (i = QS + 1; i < QW; i++)
						if (w < coo[i])
						{
							for (int j = QW - 1; j > i; j--)
							{
								coo[j] = coo[j - 1];
								for (k = 0; k < m; k++)
									profiles[k * QW + j] = profiles[k * QW + j - 1];
							}
							coo[i] = w;
							for (k = 0; k < m; k++)
								profiles[k * QW + i] = ms2h[pos].level<false>(fmz[k], MassAccuracy);
							break;
						}
				if (w - wc < 0.0)
					for (i = QS - 1; i >= 0; i--)
	...
	inline XIC ms1_XIC(float mz, float RT, int S)
	{
		...
		if (index >= n)
			index--;
		if (index > 0)
			if (RT - ms1h[index - 1].RT < ms1h[index].RT - RT)
				index--;
		...
		for (int j = start; j <= Min(index + S, n - 1); j++)
			xic.peaks[j - start].first = ms1h[j].RT, xic.peaks[j - start].second = ms1h[j].level<false>(mz, MassAccuracyMs1);
		return xic;
	}
	...
	inline XIC ms2_XIC(float pr_mz, float fr_mz, int apex, int S)
	{
		...
		for (j = apex, cnt = 0; j >= 0; j--)
		{
			if (cnt >= S + 1)
				break;
			if (ms2h[j].has(pr_mz))
				xic.peaks[S - cnt].first = scan_RT[j], xic.peaks[S - cnt].second = ms2h[j].level<false>(query_mz, MassAccuracy), cnt++;
		}
		for (j = apex + 1, cnt = 0; j < n; j++)
		{
			if (cnt >= S)
				break;
			if (ms2h[j].has(pr_mz))
				xic.peaks[S + cnt + 1].first = scan_RT[j], xic.peaks[S + cnt + 1].second = ms2h[j].level<false>(query_mz, MassAccuracy), cnt++;
		}
		return xic;
	}

	double ms1_area(double rt_from, double rt_to, double mz)
	{ // integrate MS1 signal
		if (!ms1h.size())
			return 0.0;
	...
	void ms1_profile(float *ms1, float mz, int S, float RT)
	{
		...
		int start = Max(index - S, 0);
		for (int j = start; j <= Min(index + S, n - 1); j++)
			ms1[j - start] = ms1h[j].level<false>(mz, MassAccuracyMs1);
	}
	...
	void ms1_top_profiles(std::vector<float> &ms1, std::vector<float> &masses, std::vector<Peak> &temp, int S, float RT, float low, float high, int N)
	{
		...
		temp.resize(ms1h[index].n);
		memcpy(&(temp[0]), ms1h[index].peaks, ms1h[index].n * sizeof(Peak));
		std::sort(temp.begin(), temp.end(), [](const Peak &left, const Peak &right)
				  { return left.height > right.height; });
		...
		for (i = k = 0; i < temp.size(); i++)
			if (temp[i].mz >= low && temp[i].mz < high)
				k++;
		N = Min(N, k);
		masses.clear(), masses.resize(N), ms1.clear();
		ms1.resize(W * N);
		...
		for (i = k = 0; i < temp.size(); i++)
		{
			...
			int start = Max(index - S, 0);
			for (j = start; j <= Min(index + S, n - 1); j++)
				ms1[k * W + j - start] = ms1h[j].level<false>(mz, MassAccuracyMs1);
			masses[k] = mz;
			k++;
		}
	}
	...
	void ms1_top_peaks(int thread_id, std::vector<Peak> *_top, std::vector<Lock> *_locks, int N = 500)
	{
		...
		for (int i = 0; i < ms1h.size(); i++)
			if (locks[i].set())
				ms1h[i].top_peaks(&(top[i * N]), N, lower_selection_bound, upper_selection_bound, temp);
	}

	void ms1_features(int thread_id, std::vector<std::vector<Peak>> *_features, std::vector<Peak> *_top, std::vector<Lock> *_locks,
					  int window, float accuracy = 12.0 / 1000000.0, int N = 500)
	{ // features must be empty before calling this function
		...
		for (i = S; i < ms1h.size() - S; i++)
			if (locks[i].set())
			{
				...
				for (k = 0; k < Min(N, ms1h[i].size()); k++)
				{
					...
					if (mz <= lower_selection_bound || mz >= upper_selection_bound)
						continue;
					...
					for (j = 0; j < W; j++)
					{
						...
						for (l = ind[j]; l < N; l++)
						{
							if (top[jjN + l].mz <= low)
								continue;
							if (top[jjN + l].mz >= high)
								break;
							if (top[jjN + l].height > ch[j * N + k])
								ch[j * N + k] = top[jjN + l].height;
						}
						ind[j] = l;
					}
				}
				...
				for (k = 0; k < Min(N, ms1h[i].size()); k++)
				{
					...
					for (j = 1; j < W - 1; j++)
						elution[j] = 0.5 * ch[j * N + k] + 0.25 * (ch[(j - 1) * N + k] + ch[(j + 1) * N + k]);

					bool skip_flag = false, inc_flag = false;
					float curr_evidence = elution[S];
					for (pos = S - Max(S / 3, 1); pos <= S + Max(S / 3, 1); pos++)
						if (elution[pos] > curr_evidence)
						{
							skip_flag = true;
							break;
						}
					...
					if (!inc_flag)
						continue;
					if (curr_evidence < max_evidence * PeakApexEvidence)
						continue;
	...
	int get_charge(int scan, float acc, const Peak &p)
	{
		...
		for (i = 7; i >= 1; i--)
		{
			...
			{
				charges[i] += l1;
				if ((l2 = Min(ms1h[scan].level<false>(p.mz + (2.0 * C13delta) / (double)i, acc), l1)) >= margin)
					charges[i] += l2;
			}
		}
	...
	int get_shadow(int scan, std::vector<float> &elution, int charge, float acc, const Peak &p, float min_corr = 0.7)
	{
		...
		for (int c = 1; c <= 4; c++)
			if (c != charge)
				if (ms1h[scan].level<false>(p.mz - C13delta / (double)c, acc) >= margin)
					return c;
		return 0;
	}
			...
			if (locks[i].set())
			{
				...
				if (pos != scan_RT.end() && pos != scan_RT.begin())
				{
					...
					for (auto &feature : features[i])
					{
						...
						for (start = apex; start >= 1; start--)
							if (ms2h[start].has(mz))
								break;
						for (stop = apex; stop < ms2h.size() - 1; stop++)
							if (ms2h[stop].has(mz))
								break;
						if (start < 1)
							start++;
						if (stop >= ms2h.size() - 1)
							stop--;
						if (rt - ms2h[start].RT < ms2h[stop].RT - rt)
							apex = start;
						else
							apex = stop;
						scan_index[S] = apex;

						for (start = apex - 1; start >= 0 && low >= 0; start--)
							if (ms2h[start].has(mz))
								scan_index[low--] = start;
						for (stop = apex + 1; stop < ms2h.size() && high < W; stop++)
							if (ms2h[stop].has(mz))
								scan_index[high++] = stop;
						low++;
						len = high - low;
						if (len < 2)
							continue;
						...
						for (j = low; j < high; j++)
							x[j] = ms1h[i + j - S].level<false>(mz, ms1_accuracy);
						...
						for (auto &p : peaks)
							...
							{
								for (j = low; j < high; j++)
									y[j] = ms2h[scan_index[j]].level<false>(p.mz, accuracy);
								float c = corr(&(elution[low]), &(y[low]), len);
								if (c >= corr_threshold)
									fs.peaks.push_back(Trace(p.mz, p.height, c)), score += c;
							}
						if (fs.peaks.size() >= min_fragments)
						{
							...
							std::sort(fs.peaks.begin(), fs.peaks.end(), [](const Trace &left, const Trace &right)
									  { return left.score > right.score; });
							if (fs.peaks.size() > SCSPeaks)
								fs.peaks.resize(SCSPeaks);
							...
							if (QuantMode == 0)
							{
								...
								{
									if (elution[start] < valley)
										valley = elution[start], vpos = start;
									else if (valley < elution[S] / 3.0 && valley < elution[start] / 2.0)
									{
										start = vpos;
										break;
									}
									if (Max(x[start], x[start + 1]) < boundary)
										break;
								}
								for (stop = vpos = S + 1, valley = elution[S]; stop < W; stop++)
								{
									if (elution[stop] < valley)
										valley = elution[stop], vpos = stop;
									else if (valley < elution[S] / 3.0 && valley < elution[stop] / 2.0)
									{
										stop = vpos;
										break;
									}
									if (Max(x[stop], x[stop - 1]) < boundary)
										break;
								}
								for (j = Max(start, S - R); j <= Min(stop, S + R); j++)
									quant += x[j];
							}
							else
								for (j = S - R; j <= S + R; j++)
	...
	public:
		...
		void build_index()
		{
			...
			if (Abs(dmz) < Min(Abs(run->tandem_max - mz), Abs(mz - run->tandem_min)) * 0.5)
				qmz += dmz;

			auto scan_index = &(run->ScanIndex[thread_id]);
			auto chrom_index = &(run->ChromIndex[thread_id]);
			for (i = 0, k = 0; i < n; i++)
				if (run->ms2h[i].has(qmz))
					k++;
			scan_index->resize(k);
			for (i = 0, k = 0; i < n; i++)
				if (run->ms2h[i].has(qmz))
			...
			if (!QuantOnly)
			{
				if (WindowRadius)
					S = WindowRadius;
				else if (run->PeakWidth > E)
					S = Max(1, int(ScanScale * run->PeakWidth));
				else
					S = Max(1, k / ScanFactor);
			}
		}
		...
		public:
			...
			Searcher(Precursor *precursor)
			{
				...
				if ((le.entry_flags & fFromFasta) && !le.target.fragments.size())
					le.generate();
				...
				for (auto &fr : pr->pep->fragments)
				{
					if (UseIsotopes)
						assert(fr.charge > 0);
					if (fr.mz > run->tandem_min && fr.mz < run->tandem_max)
						fragments->push_back(fr), fri->push_back(i);
					i++;
				}
				if (fragments->size() < MinSearchFrNum)
					fragments->clear(), fri->clear();
				m = fragments->size();
			}

			void chromatogram(int from = 0, int to = 0)
			{ // extract full chromatograms for the top 6 fragments
				if (!m)
					return;
				if (!to)
					to = (*scan_index).size();
				...
				for (k = from; k < to; k++)
				{
					...
					if (use_ms1)
					{
						...
						if (ms1_i < run->ms1h.size())
						{
							...
							if (force_ms1)
								if (ms1_left < MinPeakHeight && ms1_right < MinPeakHeight)
									continue;
							if (get_ms1)
								ms1[ind] = ((rt - run->ms1h[ms1_i - 1].RT) * ms1_right + (run->ms1h[ms1_i].RT - rt) * ms1_left) / Max(E, run->ms1h[ms1_i].RT - run->ms1h[ms1_i - 1].RT);
						}
					}

					if (run->RT_windowed_search)
					{ // main search phase only
						if (run->scan_RT[(*scan_index)[Min(to - 1, k + W)]] < RT_min)
							continue;
						if (run->scan_RT[(*scan_index)[Max(from, k - W)]] > RT_max)
							break;
					}
					...
					for (fr = 0; fr < Min(m, TopF); fr++)
					{
						...
						if (ms2[l * fr + ind] < MinPeakHeight)
							ms2[l * fr + ind] = 0.0;
					}
				}
			}

			void quant_chromatogram(int from, int to)
			{ // extract chromatogram in the vicinity of the peak - for the quantify() function
				if (!m)
					return;
				...
				for (pos = std::distance(run->ms1_RT.begin(), ms1_ptr); pos < run->ms1_RT.size() && (run->ms1h[pos].window_low >= mz || run->ms1h[pos].window_high <= mz); pos++)
					;
				if (pos == run->ms1_RT.size())
					ms1_right_RT = INF;
				else
				{
				...
				for (pos = std::distance(run->ms1_RT.begin(), ms1_ptr); pos >= 0 && (run->ms1h[pos].window_low >= mz || run->ms1h[pos].window_high <= mz || run->ms1_RT[pos] > rt); pos--)
					;
				if (pos == -1)
					ms1_left_RT = 0.0;
				else
				...
				for (k = from; k < to; k++)
				{
					...
					for (fr = 0; fr < m; fr++)
					{
						...
						if (k == center && info->size() && fr == (*info)[0].best_fragment && (!pr->pep->no_cal || !MassCalFilter))
						{
							...
							{
								if (ms1_right_RT - ms1_left_RT > E)
									(*info)[0].mass_delta_ms1 = ((rt - ms1_left_RT) * ms1_right_mz + (ms1_right_RT - rt) * ms1_left_mz) / (ms1_right_RT - ms1_left_RT) - mz;
								else
									(*info)[0].mass_delta_ms1 = 0.5 * (ms1_right_mz + ms1_left_mz) - mz;
							}
							else
								(*info)[0].mass_delta_ms1 = 0.0;
						}
					}
				}
			}

			void extra_chromatogram(bool full_spectrum, int from = 0, int to = 0)
			{ // obtain extra information in the vicinity of the putative elution peaks
				if (!m || !pr->info.size())
					return;
				if (!to)
					to = scan_index->size();
				...
				if (iso)
					run->MS2_H[pr->thread_id].resize(l * m);
				run->Shadow[pr->thread_id].resize(l * m);
				run->NF[pr->thread_id].resize(l);
				if (TightMassAcc)
					run->MS2_tight_one[pr->thread_id].resize(l * m), run->MS2_tight_two[pr->thread_id].resize(l * m);
				if (!run->ms1h.size())
					get_ms1 = false;
				if (get_ms1)
					run->MS1[pr->thread_id].resize(l * 9);
				...
				if (iso)
					ms2_H = &(run->MS2_H[pr->thread_id][0]);
				shadow = &(run->Shadow[pr->thread_id][0]);
				if (TightMassAcc)
					ms2_tight_one = &(run->MS2_tight_one[pr->thread_id][0]), ms2_tight_two = &(run->MS2_tight_two[pr->thread_id][0]);
				for (i = 0; i < l * m; i++)
					ms2_min[i] = shadow[i] = 0.0;
				if (TightMassAcc)
					for (i = 0; i < l * m; i++)
						ms2_tight_one[i] = ms2_tight_two[i] = 0.0;
				if (iso)
					for (i = 0; i < l * m; i++)
						ms2_H[i] = 0.0;
				if (get_ms1)
				...
				for (auto &p : (*info)[0].peaks)
				{
					for (k = p.peak - S; k <= p.peak + S; k++)
					{
						...
						{ // main search phase only
							...
							if (run->scan_RT[(*scan_index)[Min(to - 1, k + W)]] < pred_RT - margin)
								continue;
							if (run->scan_RT[(*scan_index)[Max(from, k - W)]] > pred_RT + margin)
								break;
						}
						...
						if (get_ms1)
						{
							...
							if (rt > ms1_right_RT)
							{
								...
								for (pos = std::distance(run->ms1_RT.begin(), ms1_ptr); pos < run->ms1_RT.size() && (run->ms1h[pos].window_low >= mz || run->ms1h[pos].window_high <= mz); pos++)
									;
								if (pos == run->ms1_RT.size())
									ms1_right_RT = INF, ms1[ind] = ms1_right[0], ms1[ind + l] = ms1_right[1], ms1[ind + 2 * l] = ms1_right[2];
								else
								{
									for (int j = 0; j < 9; j++)
										ms1_left[j] = ms1_right[j];
									...
									if (ms1_right[0] >= MinPeakHeight)
									{
										...
										ms1_right[1] = ms1_right[2] = 0.0;

									if (UseIsotopes)
										for (int isotope = 1; isotope < 3; isotope++)
										{
											int ii = 3 * isotope;
											...
											if (ms1_right[ii] >= MinPeakHeight)
											{
												...
												ms1_right[ii + 1] = ms1_right[ii + 2] = 0.0;
										}
								}
							}
							for (int p = 0; p < 9; p++)
								ms1[ind + p * l] = ((rt - ms1_left_RT) * ms1_right[p] + (ms1_right_RT - rt) * ms1_left[p]) / Max(E, ms1_right_RT - ms1_left_RT);
						}
						...
						for (fr = 0; fr < (full_spectrum ? m : Min(m, TopF)); fr++)
						{
							...
							int low = 0, high = run->ms2h[i].size();
							if (fr >= TopF)
								ms2[l * fr + ind] = run->ms2h[i].level<false>(low, high, query_mz, run->MassAccuracy);
							...
							if (iso && ms2[l * fr + ind] >= MinPeakHeight)
							{
								...
								int hi = -1;
								if (run->ms2h[i].has(hmz))
									hi = i;
								if (i < run->ms2h.size() - 1 && hi < 0)
									if (run->ms2h[i + 1].has(hmz))
										hi = i + 1;
								if (i > 0 && hi < 0)
									if (run->ms2h[i - 1].has(hmz))
										hi = i - 1;
								if (hi >= 0)
									ms2_H[l * fr + ind] = run->ms2h[hi].level<false>(query_mz, run->MassAccuracy);
							}
						}
			...
			{
				ms1_elution_profile.clear();
				ms1_elution_profile.resize(2 * ElutionProfileRadius + 1, std::pair<float, float>(0.0, 0.0));
				if (!run->ms1h.size())
					return;
				...
				while (true)
				{
					...
					if (high <= low + 1)
						break;
					if (run->ms1h[pos].RT > rt)
						high = pos;
					else
						low = pos;
				}
				if (pos < run->ms1h.size() - 1)
					if (rt - run->ms1h[pos].RT > run->ms1h[pos + 1].RT - rt)
						pos++;
			...
			noninline void peaks(int from = 0, int to = 0, float bcs = -INF, float tcs = -INF)
			{
				...
				if (p >= 2)
					for (k = from + S + 1, total_corr_sum = 0.0; k < to - S - 1; k++)
					{
						int ind = k - from;
						best_fr = next_fr = -1;
						...
						{
							bool flag = false;
							if (ms2[fr * l + ind] >= MinPeakHeight)
								i++, flag = true;
							else if (ms2[fr * l + ind - 1] >= MinPeakHeight || ms2[fr * l + ind + 1] >= MinPeakHeight)
								flag = true;
							if (flag)
							{
								if (!j)
						...
						if (j == 2)
						{
							...
							if (s2 > s1)
								index[1] = best_fr, index[0] = best_fr = next_fr;
							else
								index[0] = best_fr, index[1] = next_fr;
						}
						else
						{
							best_fr = -1;
							...
							for (fr = added = 0, max = E; fr < p; fr++)
							{
								...
								if (s > E)
								{
									...
									for (int f = 0; f < p; f++)
									{
										...
										if (s > score[f])
										{
											for (int g = added - 1; g > f; g--)
												index[g] = index[g - 1], score[g] = score[g - 1];
											index[f] = fr;
											score[f] = s;
											break;
										}
						...
						for (next = 0; next < p; next++)
						{
							...
							if (run->full_spectrum && ForceQ1Apex)
							{
								...
								if (run->ms2h[(*scan_index)[k] - 1].level<false>(query_mz, run->MassAccuracy) <= ms2[best_fr * l + ind] && run->ms2h[(*scan_index)[k] + 1].level<false>(query_mz, run->MassAccuracy) <= ms2[best_fr * l + ind])
								{
								}
								else
									continue;
				...
				if (bcs > -INF / 2.0)
					best = pr->info[0].best_corr_sum = bcs, pr->info[0].total_corr_sum = tcs;

				double min_score = best - MaxCorrDiff;
				for (i = pos = 0; i < n_peaks; i++)
					if (corr_sum_list[i] >= min_score)
						pos++;
			...
			noninline void score(int peak, int from = 0, int to = 0)
			{
				...
				for (fr = 0; fr < Min(m, auxF); fr++)
				{
					...
					if (fr < TopF)
					{
						if (TightMassAcc)
						{
							sc[pTightCorrOne] += (tu = corr(elution, &(ms2_tight_one[fr * l + ind - S]), W));
							if (tu > u)
								u = tu;
							sc[pTightCorrTwo] += (tu = corr(elution, &(ms2_tight_two[fr * l + ind - S]), W));
							if (tu > u)
								u = tu;
						}
						sc[pTimeCorr] += u;
						...
						if (Abs(peak_mz) > E)
							sc[pAcc + fr] = (Abs(peak_mz - query_mz) / ((*fragments)[fr].mz * run->MassAccuracy)) * u;
					}
					sc[pCorr + fr] += u;
				}
				...
				if (UseQ1 && run->full_spectrum && (run->scanning || ForceQ1))
				{
					...
					for (i = 0; i < Min(m, TopF); i++)
						fmz[i] = run->predicted_mz(&(run->MassCorrection[0]), (*fragments)[i].mz, rt);
					fmz[Min(m, TopF)] = run->predicted_mz(&(run->MassCorrection[0]), mz, rt);
					run->q1_profiles(qprofile, ws, apex, fmz, QS, Min(m, TopF) + 1);
					for (i = 0; i < qL; i++)
						sc[pQPos + i] = centroid_coo(qprofile + best_fr * QW + QS - QSL[i], ws + QS - QSL[i], 2 * QSL[i] + 1) - mz - run->Q1Correction[0] - run->Q1Correction[1] * mz;
					for (fr = 0; fr < Min(m, TopF); fr++)
					{
						if (fr != best_fr)
							for (i = 0; i < qL; i++)
								sc[pQCorr + i] += corr(qprofile + fr * QW + QS - QSL[i], qprofile + best_fr * QW + QS - QSL[i], 2 * QSL[i] + 1);
						for (i = 0; i < qL; i++)
						{
					...
					for (i = 0; i < qL; i++)
						sc[pQNFCorr + i] += corr(qprofile + Min(m, TopF) * QW + QS - QSL[i], qprofile + best_fr * QW + QS - QSL[i], 2 * QSL[i] + 1);
				}
#endif
...
#define LogEnhance(x, cap) x -= log(Max(E, -x + ((double)cap)));
		...
		void search()
		{
			if (!(flags & fInit))
			{
				...
				flags |= fInit;
				if (info.size())
					for (int peak = 0; peak < info[0].peaks.size(); peak++)
						searcher.score(peak);
			}
			if (info.size())
				for (int peak = 0; peak < info[0].peaks.size(); peak++)
		...
		noninline void quantify(int peak, int stage, bool filter = false)
		{
			...
			if (stage)
			{
				if (filter && info[0].qvalue > QFilter)
					return;
				auto &le = run->lib->entries[pep->index];
				if (!pep->fragments.size())
					le.generate();
			...
			if (QuantAdjacentBins && run->scanning && stage >= 2)
			{
				...
				for (pos = low; pos <= high; pos++)
				{
					...
					for (int inc = -1; inc <= 1; inc++)
						...
						{
							int adj = scan + inc;
							if (adj >= 0 && adj < run->ms2h.size())
								if (run->adjacent_windows(adj, scan))
								{
									for (fr = 0; fr < p; fr++)
									{
										...
										if (inc == -1)
											lower[ind] += run->ms2h[adj].level<false>(query_mz, run->MassAccuracy);
										else
											upper[ind] += run->ms2h[adj].level<false>(query_mz, run->MassAccuracy);
									}
								}
						}
			...
			e = elution[k - low] * 0.5;
			for (pos = info[0].peak_width = 0; pos < len; pos++)
				if (elution[pos] >= e)
					info[0].peak_width++;
			if (flags & fTranslated)
			{ // translate peak boundaries
				int eg = run->lib->elution_groups[pep->index];
				if (run->best_egs[eg].second <= -INF)
					return;
				int tr_index = run->best_egs[eg].first;
				auto &tr_pr = run->entries[tr_index].target;
				assert(tr_pr.pep->index != pep->index);

				for (start = k - low - 1; start > 0; start--)
					if (run->ms2h[(*searcher.scan_index)[start]].RT < tr_pr.info[0].RT_start - E)
					{
						start = Min(k - low - 1, start + 1);
						break;
					}
				for (stop = k - low + 1; stop < high - low; stop++)
					if (run->ms2h[(*searcher.scan_index)[stop]].RT > tr_pr.info[0].RT_stop + E)
					{
			...
			else
			{
				...
				if (QuantMode == 0 && stage >= 2)
				{
					...
					for (start = vpos = k - low - 1, valley = elution[k - low]; start > 0; start--)
					{
						if (elution[start] < valley)
							valley = elution[start], vpos = start;
						else if (valley < elution[k - low] / 3.0 && valley < elution[start] / 2.0)
						{
							start = vpos;
							break;
						}
						if (Max(searcher.ms2[best_fr * len + start], searcher.ms2[best_fr * len + start + 1]) < boundary)
							break;
					}
					for (stop = vpos = k - low + 1, valley = elution[k - low]; stop < high - low; stop++)
					{
						if (elution[stop] < valley)
							valley = elution[stop], vpos = stop;
						else if (valley < elution[k - low] / 3.0 && valley < elution[stop] / 2.0)
						{
							stop = vpos;
							break;
						}
						if (Max(searcher.ms2[best_fr * len + stop], searcher.ms2[best_fr * len + stop - 1]) < boundary)
							break;
					}
				}
			}
			...
			for (fr = 0; fr < p; fr++)
			{
				...
				for (int q = 0; q < qN; q++)
					info[0].quant[fr].quantity[q] = 0.0;
				scaled[fr] = 0.0;
			}
			for (pos = k - low - 1, w = 0.0; pos <= k - low + 1; pos++)
				w += Sqr(elution[pos]);
			assert(w > E);
			...
			for (fr = 0; fr < p; fr++)
			{
				...
				if (NoIfsRemoval)
					info[0].quant[fr].quantity[qFiltered] = info[0].quant[fr].quantity[qTotal];
				else
				{
			...
			if (!NoIfsRemoval)
			{
				...
				for (fr = 0; fr < p; fr++)
				{
					...
					if (info[0].quant[fr].corr < 0.8)
					{
						...
						for (pos = k - low - 1; pos >= 0; pos--)
						{
							if (elution[pos] < e)
								...
								radj = Min(radj, searcher.ms2[fr * len + pos] / Max(E, searcher.ms2[best_fr * len + pos]));
						}
					...
					for (pos = start; pos <= stop; pos++)
					{
						float value;
						if (!QuantFitProfiles)
							value = searcher.ms2[fr * len + pos];
						else
							value = run->ms2h[(*searcher.scan_index)[low + pos]].level(query_mz, run->MassAccuracy, r * searcher.ms2[best_fr * len + pos]);
						info[0].quant[fr].quantity[qFiltered] += Min(value, FilterFactor * r * elution[pos]);
					}
				}
			}
			for (fr = 0, info[0].quantity = 0.0; fr < p; fr++)
				info[0].quantity += info[0].quant[fr].quantity[qFiltered];

#if ELUTION_PROFILE
			searcher.get_ms1_elution_profile(ms1_elution_profile, info[0].RT);
#endif

			if (info[0].qvalue <= 0.01 && (e = searcher.ms2[best_fr * len + k - low] * 0.5) >= MinPeakHeight)
			{ // for run stats
				...
				for (pos = k, flag = 0; pos < high; pos++)
				{
					...
					if (searcher.ms2[best_fr * len + i + 1] >= e)
						mul = 1.0;
					else
					{
			...
			if (stage == 2 && SaveXICs)
			{
				...
				for (int i = 0; i < xg.fr.size(); i++)
					xg.fr[i].init(pep->fragments[i]);
				...
				for (auto &fr : pep->fragments)
				{
					...
					for (int i = 0; i < xg.N; i++)
						xg.values[pos++] = xic.peaks[i].second;
					cnt++;
				}
				while (!run->lock.set())
				{
		...
		noninline void intensities(int apex, std::vector<Ion> &gen)
		{ // get fragment intensities
			...
			for (i = 0; i < l; i++)
				x[i] = run->ms2h[(*scan_index)[low + i]].level<false>(query_mz, run->MassAccuracy);
			smooth(elution, x, l);
			...
			for (fr = 0; fr < gen.size(); fr++)
				actual_mz[fr] = err_mz[fr] = 0.0;
			for (fr = cnt = 0, max = 0.0; fr < gen.size(); fr++)
			{
				float fmz = gen[fr].mz, amz = 0.0;
				if (fmz < MinFrMz || fmz > MaxFrMz)
					continue;
				if (fmz <= run->tandem_min || fmz >= run->tandem_max)
					continue;
				...
				if (u >= best_height * MinRelFrHeight && u >= MinGenFrInt)
				{
					...
					if (gen[fr].type & fTypeB)
						c -= FrCorrBSeriesPenalty;
					if (c >= MinGenFrCorr && (c >= MinRareFrCorr || ((gen[fr].charge == 1 || pep->charge >= 3) && gen[fr].loss == loss_none)))
					{
						bool skip = false;
						for (j = 0; j < cnt; j++)
							if (Abs(amz - actual_mz[j]) < E)
							{
								skip = true;
								if (Abs(amz - query_mz) < err_mz[j])
									new_fr[j].mz = fmz, err_mz[j] = Abs(amz - query_mz);
							}
						if (skip)
							continue;
			...
			if (cnt < MinOutFrNum)
				run->lib->entries[pep->index].best_run = -1;
			else
			{
		...
		void seek()
		{
			...
			for (peak = 0; peak < info[0].peaks.size(); peak++)
			{
				...
				if (SaveCandidatePSMs && !run->suspend_output && run->curr_iter == nnIter - 1)
				{
					...
					for (int i = 0; i < pN; i++)
						run->run_out << '\t' << info[0].scoring[peak * pN + i];
					run->run_out << '\n';
					run->lock.free();
				}
				if (score > info[0].cscore)
				{
					if (run->RT_windowed_search && Abs(run->scan_RT[info[0].peaks[peak].apex] - calc_spline(run->RT_coeff, run->RT_points, pep->iRT)) > run->RT_window)
						continue;
			...
			if (found)
			{
				...
				for (k = 0; k < pN; k++)
					info[0].scores[k] = info[0].scoring[info[0].best_peak * pN + k];
		...
		void find(int _thread_id, bool _free, bool skip)
		{
			if (lock.set())
			{
				...
				else if ((flags & fFound) && !(flags & fDecoy))
					quantify(info[0].peak_pos, 2);
				if (_free)
					free();
			}
		}
		...
		noninline void translate()
		{
			...
			int tr_index = run->best_egs[eg].first;
			auto &tr_pr = run->entries[tr_index].target;
			if (tr_pr.pep->index == pep->index)
				return;
			if (tr_pr.info[0].qvalue > RubbishFilter)
				return;

			build_index();
			Searcher searcher(this);
			int p = Min(TopF, searcher.m), S = searcher.S, W = searcher.W;
			if (searcher.scan_index->size() <= W + 3)
				return;
			...
			{
				k = (low + high) >> 1;
				if (run->scan_RT[searcher.scan_index->at(k)] <= target_RT)
					low = k;
				else
					high = k;
			}
			k = low;
			if (run->scan_RT[searcher.scan_index->at(k + 1)] - target_RT < target_RT - run->scan_RT[searcher.scan_index->at(k)])
				k++;

			if (Abs(k - info[0].peak_pos) <= Max(S / 2, 1))
				return;
			...
			if (info.size())
			{
				...
				for (int peak = 0; peak < info[0].peaks.size(); peak++)
					searcher.score(peak, min, max), score_RT(peak);
				seek();
				flags |= fTranslated;
			}
		}
	...
	void extract_spectra(std::vector<Feature> &spectra, std::vector<float> &masses, int N_ms1, int N_ms2, float min_corr = 0.7)
	{ // only partial support for overlapping windows
		spectra.clear();
		...
		for (window = 0; window < cycle_length; window++)
		{
			...
			if (WindowRadius)
				...
				S = Max(1, int(ScanScale * PeakWidth));
			else
				S = Max(1, k / ScanFactor);
			W = 2 * S + 1;
			...
			for (k = S; k < N - S; k++)
			{
				...
				for (j = 0; j < m; j++)
				{
					corrs[j] = 0.0;
					peak[j] = 0;
					if (ms2[j][k] > ms2[j][k - 1] && ms2[j][k] > ms2[j][k + 1])
						peak[j] = 1;
				}
				for (j = 0; j < m; j++)
					for (l = j + 1; l < m; l++)
				...
				for (j = best = 0, max = -INF; j < m; j++)
					if (peak[j] && corrs[j] > max)
						max = corrs[j], best = j;
				if (max < 0.5)
					continue;
				...
				for (j = 0; j < m; j++)
					corrs[j] = corr(&(elution[0]), &(ms2[j][k - S]), W);

				ms1_top_profiles(ms1, mzs, temp, S, scan_RT[si[k]], ms2h[si[k]].window_low, ms2h[si[k]].window_high, N_ms1);
				scores.resize(mzs.size());
				for (j = 0; j < mzs.size(); j++)
					scores[j].first = corr(&(elution[0]), &(ms1[j * W]), W), scores[j].second = mzs[j];
				...
				for (j = l = 0; j < m; j++)
					if (corrs[j] >= min_corr && ms2[j][k] > E)
						l++;
				if (l < 2)
					continue; // require at least two ions
				...
				for (auto &p : peaks)
					...
					{
						for (j = k - S; j <= k + S; j++)
							x[j - k + S] = ms2h[si[j]].level<false>(p.mz, MassAccuracy);
						float c = corr(&(elution[0]), &(x[0]), W);
						if (c >= min_corr)
							fs.peaks.push_back(Trace(p.mz, p.height, c));
					}
	...
	void write(char *file_name)
	{
		...
		long long pos = 0;
		for (int i = 0; i < ms1hc.size(); i++)
			ms1hc[i].peaks = (Peak *)pos, pos += ms1hc[i].n;
		for (int i = 0; i < ms2hc.size(); i++)
			ms2hc[i].peaks = (Peak *)pos, pos += ms2hc[i].n;

		out.write((char *)&(ms1hc[0]), size_ms1 * sizeof(Scan));
		out.write((char *)&(ms2hc[0]), size_ms2 * sizeof(Scan));
		for (int i = 0; i < size_ms1; i++)
			out.write((char *)ms1h[i].peaks, ms1h[i].n * sizeof(Peak));
		for (int i = 0; i < size_ms2; i++)
			out.write((char *)ms2h[i].peaks, ms2h[i].n * sizeof(Peak));
	...
	bool read(char *file_name)
	{
		...
		if (in.fail())
		{
			dsout << "ERROR: cannot open " << file_name << "\n";
			return false;
		}
		...
		if (size == 0)
		{
			dsout << "ERROR: no MS2 spectra in the file\n";
			return false;
		}
		if (size > 0)
		{ // 1st version of the .dia format
			const int Block = 5;
			...
			for (i = tot = 0; i < size; i++)
			{
				...
				if (ind == Block - 1 || i == size - 1)
				{
					...
					for (j = tot = 0; j <= ind; j++)
						memcpy(&(pl[tot]), &(S[j][0]), S[j].size() * sizeof(Peak)), ms2h[i - ind + j].peaks = &(pl[tot]), ms2h[i - ind + j].n = S[j].size(), tot += S[j].size();
				}
			}
			...
			for (i = tot = 0; i < size; i++)
			{
				...
				if (ind == Block - 1 || i == size - 1)
				{
					...
					auto &pl = peak_lists.back();
					for (j = tot = 0; j <= ind; j++)
						memcpy(&(pl[tot]), &(S[j][0]), S[j].size() * sizeof(Peak)), ms1h[i - ind + j].peaks = &(pl[tot]), ms1h[i - ind + j].n = S[j].size(), tot += S[j].size();
				}
			}
		}
		else if (size <= -2)
		{ // 2nd version of the .dia format
			...
			if (size_ms1)
				in.read((char *)&(ms1h[0]), size_ms1 * sizeof(Scan));
			if (size_ms2)
				in.read((char *)&(ms2h[0]), size_ms2 * sizeof(Scan));
			if (size_peaks)
				in.read((char *)&(peaks[0]), size_peaks * sizeof(Peak));

			if (size <= -3)
			{ // 3rd version of the .dia format
			...
			for (int i = 0; i < ms1h.size(); i++)
				ms1h[i].peaks = &(peaks[(long long)ms1h[i].peaks]);
			for (int i = 0; i < ms2h.size(); i++)
				ms2h[i].peaks = &(peaks[(long long)ms2h[i].peaks]);
		}
		else
		{
			dsout << "ERROR: unknown version of the .dia file format\n";
			return false;
		}
	...
	{ // called after read()
		...
		{
			if (Verbose >= 1)
				Time(), dsout << "Exporting window acquisition scheme\n";
			std::ofstream out(name + std::string(".txt"), std::ofstream::out);
			out << "lower_offset\tupper_offset\n";
			for (i = 0; i < Min(n_scans, MaxCycleLength); i++)
				out << ms2h[i].window_low << "\t" << ms2h[i].window_high << "\n";
			out.close();
		}
		...
		{ // handle overlapping windows
			...
			if (ms2h[i - 1].window_high < ms2h[i].window_high && ms2h[i - 1].window_low < ms2h[i].window_low)
				scan_cycle[i] = cycle;
			else
				scan_cycle[i] = ++cycle;

			if (ms2h[i].window_low < lower_selection_bound)
				lower_selection_bound = ms2h[i].window_low;
			if (ms2h[i].window_high > upper_selection_bound)
				upper_selection_bound = ms2h[i].window_high;
		}
		for (cycle_length = 0; cycle_length < scan_cycle.size(); cycle_length++)
			if (scan_cycle[cycle_length])
				break;

		if (ForceNormalSWATH)
			scanning = false;
		if (ForceScanningSWATH)
			scanning = true;
		if (Verbose >= 1 && scanning && !Convert)
			Time(), dsout << "Analysing as Scanning SWATH run\n";
		...
		{
			if (t_min < t_max)
				tandem_min = t_min, tandem_max = t_max;
			if (Verbose >= 3)
				Time(), dsout << "Detected MS/MS range: " << tandem_min << " - " << tandem_max << "\n";
		}

		ms1_RT.resize(ms1h.size());
		for (i = 0; i < ms1_RT.size(); i++)
			ms1_RT[i] = ms1h[i].RT;

		for (int i = 0; i < ms1h.size(); i++)
		{ // calculate ms1 windows
			if (!ms1h[i].size())
				ms1h[i].window_low = ms1h[i].window_high = 0.0;
			else
			{
		...
		while (true)
		{
			...
			for (next = start; next <= stop; next++)
			{
				if (first)
				{
					...
					if (total_spectra > 1)
						spectra->reserve(total_spectra + 2);
					lock.free();
				}
				else
				{
					if (!r.readFile(NULL, s[next - start], next))
					{
						finish = true;
						break;
					}
				}
				if (s[next - start].getScanNumber() == 0)
				{
					finish = true;
					break;
				}
			...
			if (next > start)
			{
				...
				peak_lists.push_back(std::vector<Peak>(tot));
				auto &pl = peak_lists.back();
				if (next - 1 > spectra->size())
					spectra->resize(next - 1);
				for (int i = start; i < next; i++)
				{
					...
					for (int k = 0; k < sc.n; k++)
						pl[pos + k].mz = sp.at(k).mz, pl[pos + k].height = sp.at(k).intensity;
					pos += sc.n;
	...
	void fill_spectrum(std::vector<int> &mobility, std::vector<long long> &spectrum, std::vector<long long> &temp, const std::vector<unsigned int> &buf, int N, int from)
	{
		...
		for (i = 1; i < n - 1; i++)
			spectrum[i] = temp[i - 1] + temp[i] + temp[i + 1];
		for (i = 1; i < n - 1; i++)
			if (spectrum[i] < spectrum[i - 1] || spectrum[i] < spectrum[i + 1] || spectrum[i] < 1 || mobility[i] < 0)
				spectrum[i] = 0;
	}
	...
	bool load_tdf(int thread_id, char *file_name, std::list<std::vector<Peak>> *_scan_list, std::vector<Lock> *_locks)
	{
...

		{
			...
			while (!query.eof())
			{
				...
				if (frame >= window_groups.size())
					window_groups.resize(frame + 1);
...

		{
			...
			while (!query.eof())
			{
				...
				if (id >= frames.size())
					frames.resize(id + 1);
				if (type > 0)
					frames[id].ms_level = 2, tot_ms2++;
				else
					frames[id].ms_level = 1, tot_ms1++;
		...
		if (window_groups.size() > locks.size())
			locks.resize(window_groups.size());
		...
		for (long long frame = 0; frame < frames.size(); frame++)
			if (locks[frame].set())
			{
				if (!frames[frame].ms_level)
					continue;
				if (!window_groups[frame] && frames[frame].ms_level != 1)
					continue;
				...
				if (frames[frame].ms_level == 1)
				{
					...
					if (!read)
					{
						std::cout << "ERROR: cannot read frame " << frame << "\n";
						tims_close(handle);
						return false;
					};
					...
					for (i = 0; i < spectrum.size(); i++)
						if (spectrum[i] >= margin)
							mzi.push_back((double)i), mobi.push_back(mobility[i]);
					...
					scan_list.push_back(peak_list);
					auto &peaks = scan_list.back();
					int ims1 = ms1h.size();
					ms1h.resize(ims1 + 1), ms1mob.resize(ims1 + 1);
					lock.free();
					...
					if (peaks.size())
						for (i = cnt = 0; i < spectrum.size(); i++)
							if (spectrum[i] >= margin)
							{
								...
								iref &= ~0xFF;
								iref |= (int)floor(Min(oneoverk0[cnt], 1.9999) * 128.0);
				...
				else
				{
					...
					for (int w = 0; w < windows.size(); w++)
						if (windows[w].group == wg)
						{
							...
							tims_index_to_mz(handle, frame, &(mzi[0]), &(mz[0]), mz.size());
							tims_scannum_to_oneoverk0(handle, frame, &(mobi[0]), &(oneoverk0[0]), oneoverk0.size());
							scn[0] = windows[w].from, scn[1] = windows[w].to + 1;
							tims_scannum_to_oneoverk0(handle, frame, scn, ok0, 2);
							...
							auto &peaks = scan_list.back();
							int ims2 = ms2h.size();
							ms2h.resize(ims2 + 1), ms2mob.resize(ims2 + 1);
							lock.free();
	...
	{ // Spectra in the file should be ordered by the acquisition time
		name = std::string(file_name);
		if (Verbose >= 1)
			Time(), dsout << "Loading run " << name << "\n";
		...
		if (ext == std::string(".wiff"))
		{
			try
			{
				...
				for (i = 0; i < n; i++)
				{
					...
					for (j = 0; j < S.n; j++)
						S.peaks[j].mz = *(fptr++), S.peaks[j].height = *(fptr++);
					ptr = (int *)fptr;
					pos += S.n;
		...
		if (ext == std::string(".tdf"))
		{
			...
			for (int i = 0; i < max_threads; i++)
				threads.push_back(std::thread(&Run::load_tdf, this, i, file_name, &scan_list, &locks));
			for (int i = 0; i < max_threads; i++)
				threads[i].join();
			...
			std::sort(ms1h.begin(), ms1h.end(), [&](const Scan &left, const Scan &right)
					  {
						  if (left.RT < right.RT)
							  return true;
						  if (left.RT <= right.RT && left.window_low < right.window_low)
							  return true;
						  return false;
					  });
...
#ifdef MSTOOLKIT
		{
			...
			int max_threads = Max(1, Threads - 1);
			for (int i = 0; i < max_threads; i++)
				threads.push_back(std::thread(&Run::load_raw, this, i, file_name, &spectra));
			for (int i = 0; i < max_threads; i++)
				threads[i].join();
			...
			if (ms1_cnt + ms2_cnt != total_spectra)
				dsout << "WARNING: spectra other than MS1 and MS2 detected\n";
	...
	void load_library(Library *_lib)
	{
		...
		for (auto it = lib->entries.begin(); it != lib->entries.end(); it++, pos++)
		{
			...
			for (i = 0; i < max; i++)
				if (ms2h[i].has(mz))
				{
					...
					{
						if (gen[fr].mz > tandem_min && gen[fr].mz < tandem_max)
							frn++;
						if (frn >= MinGenFrNum)
							break;
					}
		...
		entries.resize(cnt);
		if (Verbose >= 1)
			Time(), dsout << cnt << " library precursors are potentially detectable\n";
		cnt = pos = 0;
		for (auto it = lib->entries.begin(); it != lib->entries.end(); it++, pos++)
		{
			if (has[pos])
		...
		std::mt19937_64 gen(1);
		if (BatchMode)
			Batches = Min(MaxBatches, Max(1, entries.size() / MinBatch));
		if (Batches == 1)
		...
		{
			if (GuideLibrary)
				guide_batches = Min(MaxBatches / 2 + 1, Max(1, guide / MinBatch)), fasta_batches = Max(1, Batches - guide_batches), Batches = guide_batches + fasta_batches;
			std::vector<int> index(entries.size());
			for (pos = 0; pos < index.size(); pos++)
				index[pos] = pos;
			...
			for (pos = 0; pos < entries.size(); pos++)
			{
				...
				else
				{
					if (!(lib->entries[entries[pos].target.pep->index].entry_flags & fFromFasta))
						...
						entries[pos].batch = guide_batches + (index[pos] % fasta_batches);
				}
			}
		}
	}
	...
	void process_precursors(int thread_id, bool free, bool free_fragments, int min_batch, int max_batch)
	{ // skip before min_batch (only update RT scores), ignore after max_batch
		for (int i = 0; i < entries.size(); i++)
		{
			if (entries[i].batch > max_batch)
				continue;
			if (entries[i].lock.set())
			{
				if ((!QuantOnly && curr_iter < iN) || (entries[i].target.flags & fFound))
					entries[i].target.find(thread_id, free, entries[i].batch < min_batch);
				if (!QuantOnly && curr_iter < iN)
					entries[i].decoy.find(thread_id, free, entries[i].batch < min_batch);

				auto &le = lib->entries[entries[i].target.pep->index];
				if (free_fragments && (le.entry_flags & fFromFasta) && le.target.fragments.size() && !(le.entry_flags & fPredictedSpectrum))
					le.free();
				entries[i].lock.free();
			}
		}
	...
	void seek_precursors(bool clean, bool free, bool free_fragments, int min_batch, int max_batch)
	{
		...
		if (Threads > 1)
		{
			std::vector<std::thread> threads;
			for (i = 0; i < Threads; i++)
				threads.push_back(std::thread(&Run::process_precursors, this, i, free, free_fragments, min_batch, max_batch));
			for (i = 0; i < Threads; i++)
				threads[i].join();
		}
		else
			process_precursors(0, free, free_fragments, min_batch, max_batch);
		for (i = 0; i < entries.size(); i++)
		{
	...
	void quantify_decoy_precursors(int thread_id, int stage, bool filter)
	{
		for (int i = 0; i < entries.size(); i++)
		{
			if (entries[i].decoy.flags & fFound)
				if (entries[i].decoy.lock.set())
				{
					entries[i].decoy.thread_id = thread_id;
					entries[i].decoy.quantify(entries[i].decoy.info[0].peak_pos, stage, filter);
				}
		}
	}
	...
	void quantify_all(int stage, bool filter)
	{
		...
		if (Threads > 1)
		{
			std::vector<std::thread> threads;
			for (i = 0; i < Threads; i++)
				threads.push_back(std::thread(&Run::quantify_precursors, this, i, stage, filter));
			for (i = 0; i < Threads; i++)
				threads[i].join();
		}
		else
			quantify_precursors(0, stage, filter);
		quantify_decoy_precursors(0, stage, filter);
		for (i = 0; i < entries.size(); i++)
			entries[i].target.lock.free();
	}
	...
	void reset_precursors()
	{
		for (int i = 0; i < entries.size(); i++)
		{
			...
			if (entries[i].target.info.size())
				std::vector<Search>().swap(entries[i].target.info);
			if (entries[i].decoy.info.size())
				std::vector<Search>().swap(entries[i].decoy.info);
		}
	}
		...
		for (auto &e : entries)
			if (e.target.flags & fFound)
				...
				else
				{
					...
					{
						if (e.target.info[0].cscore < smin_g)
							smin_g = e.target.info[0].cscore;
					}
					else if (e.target.info[0].cscore < smin)
						smin = e.target.info[0].cscore;
					cnt++;
				}

		tot += cnt;
		if (cnt >= MinNonRubbish && tot > MinRubbishFactor * cnt)
		{ // this is likely indicative of library-free or Prosit library search
			if (Verbose >= 1)
				Time(), dsout << "Removing low confidence identifications\n";
			for (auto &e : entries)
				if (e.decoy.flags & fFound)
				{
					if (GuideLibrary && !(lib->entries[e.decoy.pep->index].entry_flags & fFromFasta))
					{
						if (e.decoy.info[0].cscore < smin_g)
							e.decoy.flags &= ~fFound;
					}
					else if (e.decoy.info[0].cscore < smin)
						e.decoy.flags &= ~fFound;
				}
		}
	}
	...
	void map_ids()
	{
		...
		for (i = 0; i < ms2h.size(); i++)
		{
			if (IDs[i].size() < 2)
				continue;
			std::sort(IDs[i].begin(), IDs[i].end(), [&](const int &left, const int &right)
					  { return entries[left].target.info[0].cscore > entries[right].target.info[0].cscore; });
		}
	}
	...
	inline void remove_ifs(int i, int start, Precursor &es)
	{
		...
		for (int next = start + 1; next < IDs[i].size(); next++)
		{
			...
			if (en.info[0].cscore > smax)
				continue;
			if (lib->elution_groups[en.pep->index] == lib->elution_groups[es.pep->index])
				continue; // same elution group => might actually share fragments

			float masses[TopF];
			for (fr = 0; fr < TopF; fr++)
				masses[fr] = 0.0;
			for (fr = 0, tot_corr = ifs_corr = 0.0; fr < Min(TopF, en.info[0].quant.size()); fr++)
				tot_corr += en.info[0].quant[fr].corr;
			for (int l = 0; l < Min(TopF, en.info[0].quant.size()); l++)
			{
				...
				for (int k = 0; k < Min(TopF, es.info[0].quant.size()); k++)
				{
					...
					else
					{
						if (!UseIsotopes)
							continue;
						if (Abs(nmz - es.pep->fragments[ki].mz - C13delta / (double)es.pep->fragments[ki].charge) >= error)
							continue;
	...
	void refine_ids()
	{
		...
		for (i = 0; i < ms2h.size(); i++)
		{
			...
			for (int start = 0; start < IDs[i].size() - 1; start++)
			{
				...
				{
					if (j == i || Sgn(ms2h[j].window_low - ms2h[i].window_low) != Sgn(j - i))
						continue;
					if ((scanning && Abs(j - i) <= MaxQ1Bins + 1) || ms2h[j].has(qmz, QuadrupoleError) || (UseIsotopes && (ms2h[j].has(qmz + delta, QuadrupoleError) || ms2h[j].has(qmz + 2.0 * delta, QuadrupoleError) || ms2h[j].has(qmz + 3.0 * delta, QuadrupoleError) || ms2h[j].has(qmz + 4.0 * delta, QuadrupoleError))))
						remove_ifs(j, -1, es);
				}

				if (StrictIntRemoval)
				{
					for (j = i + 1, cnt = 0; j < ms2h.size(); j++)
						if (ms2h[j].has(es.pep->mz))
						{
							remove_ifs(j, -1, es);
							if ((++cnt) >= es.S / 4)
								break;
	...
	void map_egs()
	{
		int i = 0;
		best_egs.clear();
		best_egs.resize(lib->co_elution_index.size(), std::pair<int, float>(0, -INF));
		for (auto &e : entries)
		{
			if (e.target.flags & fFound)
			{
				int index = lib->elution_groups[e.target.pep->index];
				if (e.target.info[0].cscore > best_egs[index].second)
					best_egs[index].first = i, best_egs[index].second = e.target.info[0].cscore;
			}
			i++;
		}
	}
	...
	void translate_all()
	{
		...
		if (Threads > 1)
		{
			std::vector<std::thread> threads;
			for (i = 0; i < Threads; i++)
				threads.push_back(std::thread(&Run::translate_peaks, this, i));
			for (i = 0; i < Threads; i++)
				threads[i].join();
		}
		else
	...
	void standardise()
	{
		...
		if (Verbose >= 2)
			Time(), dsout << "Standardising scores\n";
		pos = 0;
		for (i = 0; i < pN; i++)
			e[i] = s2[i] = 0.0;
		for (auto it = entries.begin(); it != entries.end(); it++)
		{
			...
			{
				for (i = 0; i < pN; i++)
					e[i] += it->target.info[0].scores[i];
				pos++;
			}
			if (it->decoy.flags & fFound)
			{
				for (i = 0; i < pN; i++)
					e[i] += it->decoy.info[0].scores[i];
				pos++;
			}
		}
		...
		for (auto it = entries.begin(); it != entries.end(); it++)
		{
			...
			{
				for (i = 0; i < pN; i++)
					s2[i] += Sqr(it->target.info[0].scores[i] - e[i]);
			}
			if (it->decoy.flags & fFound)
			{
				for (i = 0; i < pN; i++)
					s2[i] += Sqr(it->decoy.info[0].scores[i] - e[i]);
			}
		}
		mul = 1.0 / (double)(pos - 1);
		double scale = 1.0 / StandardisationScale;
		for (i = 0; i < pN; i++)
			s2[i] = sqrt(Max(E, s2[i] * mul)) * scale;

		for (auto it = entries.begin(); it != entries.end(); it++)
		{
			if (it->target.flags & fFound)
				for (i = 0; i < pN; i++)
					it->target.info[0].scores[i] = (it->target.info[0].scores[i] - e[i]) / s2[i];
			if (it->decoy.flags & fFound)
				for (i = 0; i < pN; i++)
					it->decoy.info[0].scores[i] = (it->decoy.info[0].scores[i] - e[i]) / s2[i];
		}
	}
	...
	noninline void fit_weights(int max_batch, bool all = false)
	{
		...
		if (!GuideLibrary || !GuideClassifier || curr_iter >= nnIter)
			all = true;
		...
		if (curr_iter >= nnIter)
		{
			...
			for (i = 0; i < index_t.size(); i++)
				index_t[i] = i;
			for (i = 0; i < index_d.size(); i++)
				index_d[i] = i;
			...
			for (auto it = entries.begin(); it != entries.end(); it++)
			{
				if (it->batch > max_batch)
					continue;
				if (it->target.flags & fFound)
					it->target.info[0].nn_index = index_t[i++];
				if (it->decoy.flags & fFound)
					it->decoy.info[0].nn_index = index_d[j++];
			}

			for (i = 0; i < nnBagging; i++)
				training[i].resize(maxTraining), training_classes[i].resize(maxTraining);

			for (i = j = 0; i < nnBagging; i++)
			{
				nn_mod[i] = i % nn_mod_n;
				if (nn_mod[i] == 0)
					nn_scale[i] = ((++j) % (nn_scale_max + 1 - nn_scale_min)) + nn_scale_min;
				else
					nn_scale[i] = nn_scale[i - 1];
				if (nn_mod[i] == 0)
					nn_shift[i] = gen() % nn_scale[i];
				else
					nn_shift[i] = nn_shift[i - 1];
			}
		}

		if (Verbose >= 2)
			Time(), dsout << "Optimising weights\n";
		...
		for (auto it = entries.begin(); it != entries.end(); it++)
		{
			...
			if (curr_iter >= nnIter && (!nnFilter || !it->target.pep->no_cal))
			{
				if (it->target.flags & fFound)
				{
					...
					{
						t50++;
						if (it->target.info[0].qvalue <= 0.1)
							t10++;
					}
					for (i = 0; i < nnBagging; i++)
						if (train_on_index(it->target.info[0].nn_index, i))
				...
				if (it->decoy.flags & fFound)
				{
					dall++;
					for (i = 0; i < nnBagging; i++)
						if (train_on_index(it->decoy.info[0].nn_index, i))
							training[i][nn_cnt[i]] = it->decoy.info[0].scores, training_classes[i][nn_cnt[i]] = decoy_nn, nn_cnt[i]++;
					nnCnt++;
				}
			}

			if (!(it->target.flags & fFound) || !(it->decoy.flags & fFound))
				continue;
			...
			if (LDA)
				for (i = 0; i < pN; i++)
					mt[i] += it->target.info[0].scores[i], md[i] += it->decoy.info[0].scores[i];
		...
		if (curr_iter < nnIter || !default_weights)
		{
			...
			if (all)
			{
				...
				for (i = 0; i < pN; i++)
					guide_weights[i] = weights[i];
			}
			else
				solve(guide_weights, A, b, pN);
	...
	try_nn:
		...
		{
			if (Verbose >= 1)
				Time(), dsout << "Too few confident identifications, neural network will not be used\n";
			goto finish;
		}
		if (Verbose >= 1)
			Time(), dsout << "Training the neural network: " << nnCnt - dall << " targets, " << dall << " decoys\n";

		{
			...
			if (TranslatePeaks)
			{
				...
				nn_validated = true;
				if (Verbose >= 1)
					Time(), dsout << "Translating peaks within elution groups\n";
				map_egs();
				translate_all();
	...
	finish:
		if (!all && non_guide_present)
			fit_weights(max_batch, true);
	}
		...
		for (int n = 0; n * Threads + thread_id < nnBagging; n++)
		{
			...
			for (auto it = entries.begin(); it != entries.end(); it++, i++)
			{
				...
				if (GuideLibrary && !all && (lib->entries[it->target.pep->index].entry_flags & fFromFasta))
					continue;
				if (it->target.flags & fFound)
					if (!translated || (it->target.flags & fTranslated))
				...
				if (cnt >= batch_size || (cnt > 0 && it + 1 == entries.end()))
				{
					...
					for (j = 0; j < cnt; j++)
					{
						...
						int pos = index[j] >> 1;
						if (!decoy)
							(*nn_sc)[pos * (2 * nnBagging) + ind] = result->data[2 * j];
						else
							(*nn_sc)[pos * (2 * nnBagging) + nnBagging + ind] = result->data[2 * j];
					}
					free(dataset);
					cnt = 0;
				}
	...
	void nn_scores(bool all, std::vector<NN> *net, bool translated = false)
	{
		for (auto it = entries.begin(); it != entries.end(); it++)
		{
			...
			if (it->target.flags & fFound)
				if (!translated || (it->target.flags & fTranslated))
					it->target.info[0].cscore = 0.0, it->target.info[0].nn_inc = 0;
			if (!translated)
				if (it->decoy.flags & fFound)
					it->decoy.info[0].cscore = 0.0, it->decoy.info[0].nn_inc = 0;
		}

		std::vector<float> nn_sc(entries.size() * nnBagging * 2, -1.0);
		std::vector<std::thread> thr;
		for (int i = 0; i < Threads; i++)
			thr.push_back(std::thread(&Run::nn_score, this, i, all, net, &nn_sc, translated));
		for (int i = 0; i < Threads; i++)
			thr[i].join();
		...
		{
			auto &e = entries[i];
			if (e.batch > curr_batch)
				continue;
			if (e.target.flags & fFound)
				if (!translated || (e.target.flags & fTranslated))
				{
					e.target.info[0].cscore = 0.0;
					int cnt = 0;
					for (int j = 0; j < nnBagging; j++)
						if (nn_sc[i * (nnBagging * 2) + j] >= 0.0)
							e.target.info[0].cscore += nn_sc[i * (nnBagging * 2) + j], cnt++;
					assert(cnt > 0);
			...
			if (e.decoy.flags & fFound)
				if (!translated)
				{
					...
					int cnt = 0;
					for (int j = 0; j < nnBagging; j++)
						if (nn_sc[i * (nnBagging * 2) + nnBagging + j] >= 0.0)
							e.decoy.info[0].cscore += nn_sc[i * (nnBagging * 2) + nnBagging + j], cnt++;
					assert(cnt > 0);
	...
	start:
		if (Verbose >= 2)
			Time(), dsout << "Calculating q-values\n";
		...
		for (auto it = entries.begin(); it != entries.end(); it++, i++)
		{
			...
			if (curr_iter < nnIter || !nn_trained)
			{
				...
				if (it->target.flags & fFound)
					it->target.info[0].cscore = !use_evidence ? cscore<false>(it->target.info[0].scores, guide) : it->target.info[0].scores[0];
				if (it->decoy.flags & fFound)
					it->decoy.info[0].cscore = !use_evidence ? cscore<false>(it->decoy.info[0].scores, guide) : it->decoy.info[0].scores[0];
			}

			if (it->target.flags & fFound)
				if (!(it->target.flags & fTranslated))
				{
		...
		for (auto it = entries.begin(); it != entries.end(); it++)
		{
			...
			if (it->target.flags & fFound)
			{
				...
				if (n_better <= iRTTopPrecursors && (!GuideLibrary || !(lib->entries[it->target.pep->index].entry_flags & fFromFasta)))
					if (it->target.info[0].cscore < iRT_cscore)
						iRT_cscore = it->target.info[0].cscore;
				if (n_better <= iRTRefTopPrecursors && it->target.info[0].cscore < iRT_ref_score)
					iRT_ref_score = it->target.info[0].cscore;
				...
				else
				{
					...
					if (pos.second)
					{
						if (pos.first != guide_cs_qv.begin() && std::prev(pos.first)->second < pair.second)
							pos.first->second = std::prev(pos.first)->second;
						else
							for (auto jt = std::next(pos.first); jt != guide_cs_qv.end() && jt->second > pair.second; jt++)
								jt->second = pair.second;
					}
		...
		for (auto it = entries.begin(); it != entries.end(); it++)
		{
			...
			if (curr_iter >= iN - 1)
				if (it->decoy.flags & fFound)
				{
					...
					if (it->decoy.info[0].qvalue > 1.0)
						it->decoy.info[0].qvalue = 1.0;
			...
			if (!(it->target.flags & fFound))
				continue;
			auto pos = (!GuideLibrary || (lib->entries[it->target.pep->index].entry_flags & fFromFasta)) ? cs_qv.lower_bound(it->target.info[0].cscore) : guide_cs_qv.lower_bound(it->target.info[0].cscore);
			it->target.info[0].qvalue = pos->second;
			if (it->target.info[0].qvalue > 1.0)
				it->target.info[0].qvalue = 1.0;
			if (it->target.info[0].qvalue <= 0.01)
				ids++;
			...
			if (it->target.info[0].qvalue <= 0.01)
				...
				continue;
			if (it->target.info[0].qvalue <= 0.001)
				Ids01++;
		}
		RS.precursors = Ids1;
		...
		{
			if (Verbose < 3)
				Time(), dsout << "Number of IDs at 0.01 FDR: " << ids << "\n";
			if (Verbose >= 3)
				Time(), dsout << "Number of IDs at 50%, 10%, 1%, 0.1% FDR: "
							  << Ids50 << ", " << Ids10 << ", " << Ids1 << ", " << Ids01 << "\n";
		}
		if (!nn_validated)
			if (curr_iter >= nnIter && nn_trained && (Ids10 < linear_classifier_ids10 || (ids < linear_classifier_ids1 && linear_classifier_ids1 >= 50 && ((double)ids / (double)linear_classifier_ids1) < ((double)linear_classifier_ids10 / (double)Max(1, Ids10))) || Ids10 < 100))
			{
				if (Verbose >= 1)
					Time(), dsout << "Too low number of IDs with NNs: reverting to the linear classifier\n";
				nn_trained = false;
				goto start;
			}

		if (Ids10 < 500 && Ids1 < 100 && !all_iters && !use_evidence)
			reset_weights();
		...
		if (curr_iter == nnIter - 1)
			linear_classifier_ids1 = Ids1, linear_classifier_ids10 = Ids10;
		return ids;
	}
	...
	int update_classifier(bool fit, bool clean, bool free, bool free_fragments, int min_batch, int max_batch)
	{
		...
		if (LCAllScores && curr_iter == nnIter - 1)
		{
			...
			for (int i = pShape; i < pN; i++)
				par_learn[i] = false; // remove pShape scores to avoid potential linear dependency
		}
		if (fit)
			fit_weights(max_batch);
	...
	{
		if (Verbose >= 2)
			Time(), dsout << "Calibrating retention times\n";
		...
		if (!RemoveMassAccOutliers)
		{
...
#if Q1
			if (calibrate_q1)
				q1_diff.clear(), q1_diff_mz.clear();
#endif
			...
			for (auto it = entries.begin(); it != entries.end(); it++)
			{
				...
				if (it->target.info[0].qvalue <= MassCalQvalue && Abs(it->target.info[0].mass_delta) > E)
				{
					mass_cnt++;
					if (calibrate)
						rt_coo.push_back(it->target.info[0].RT);
					if (Abs(it->target.info[0].mass_delta_ms1) > E)
						mass_cnt_ms1++;
				}
				if (GuideLibrary && (lib->entries[it->target.pep->index].entry_flags & fFromFasta))
					continue;
...
#if Q1
						if (calibrate_q1)
							q1_diff.push_back(it->target.info[0].q1_shift), q1_diff_mz.push_back(it->target.pep->mz);
#endif
					}
					if (gen_ref && it->target.info[0].cscore >= iRT_ref_score)
						rt_ref.push_back(it->target.pep->index);
				}
			}
			std::sort(rt_stats.begin(), rt_stats.end(), [&](const auto &lhs, const auto &rhs)
					  { return entries[lhs].target.pep->iRT < entries[rhs].target.pep->iRT; });
			rt_data.resize(rt_stats.size());
			for (int i = 0; i < rt_stats.size(); i++)
				rt_data[i] = std::pair<float, float>(entries[rt_stats[i]].target.pep->iRT, entries[rt_stats[i]].target.info[0].RT);
			if (Verbose >= 2)
				Time(), dsout << rt_data.size() << " precursors used for iRT estimation.\n";
			...
			if (set_RT_window)
			{
				...
				for (int i = 0; i < rt_stats.size(); i++)
				{
					if (!GuideLibrary || (lib->entries[entries[rt_stats[i]].target.pep->index].entry_flags & fFromFasta))
						rt_delta[i] = -Abs(entries[rt_stats[i]].target.info[0].RT - calc_spline(RT_coeff, RT_points, entries[rt_stats[i]].target.pep->iRT));
					else
						rt_delta[i] = -Abs(entries[rt_stats[i]].target.info[0].RT - calc_spline(RT_coeff, RT_points, entries[rt_stats[i]].target.pep->sRT));
				}
				std::sort(rt_delta.begin(), rt_delta.end());
				std::sort(rt_stats.begin(), rt_stats.end(), [&](const auto &lhs, const auto &rhs)
						  { return entries[lhs].target.info[0].RT < entries[rhs].target.info[0].RT; });
				...
				if (!RT_windowed_search && Verbose >= 3)
				{
					Warning("not enough confidently identified precursors for RT-windowed search");
				}
				else if (Verbose >= 1)
					Time(), dsout << "RT window set to " << RT_window << "\n";
			}
			...
			if (set_scan_window)
			{
				...
				if (Verbose >= 1)
					Time(), dsout << "Peak width: " << PeakWidth << "\n";

				int window_size = Max(1, int(ScanScale * PeakWidth));
				if (Verbose >= 1)
					Time(), dsout << "Scan window radius set to " << window_size << "\n";
				if (!IndividualWindows)
					WindowRadius = window_size, InferWindow = false;
			...
			if (calibrate_q1)
			{
				...
				if (q1_diff.size() >= 500 && Q1CalLinear)
				{
					...
					b.clear(), k = 0;
					for (int i = 0; i < q1_diff.size(); i++)
						if (q1_diff[i] >= dmin && q1_diff[i] <= dmax)
						{
				...
				for (int i = (q1_diff.size() / 5); i < q1_diff.size() - (q1_diff.size() / 5); i++)
					diff += q1_diff[i], k++;
				Q1Correction[0] = diff / (float)Max(1, k);

				if (Verbose >= 1)
					Time(), dsout << "Q1 correction: " << Q1Correction[0] << "Th + " << Q1Correction[1] << "(m/z)"
								  << "\n";
			}
#endif
		}
		...
		if (calibrate)
		{
			...
			if (mass_cnt >= min_mass_cnt || RemoveMassAccOutliers)
			{
				...
				if (!RemoveMassAccOutliers)
				{
					...
					for (int i = 0; i < MassCalBins; i++)
						MassCalSplit[i] = rt_coo[Min(split * (double)i, mass_cnt - 1)];
					MassCalSplit[MassCalBins] = rt_coo[mass_cnt - 1];
					for (int i = 0; i < MassCalBins; i++)
						MassCalCenter[i] = 0.5 * (MassCalSplit[i] + MassCalSplit[i + 1]);
					for (int i = 1; i < MassCalBins; i++)
						if (MassCalCenter[i] - MassCalCenter[i - 1] < E)
						{
							for (int k = i - 1; k < MassCalBins - 1; k++)
								MassCalCenter[k] = MassCalCenter[k + 1];
							MassCalBins--;
						}
				}
				...
				for (auto it = entries.begin(); it != entries.end(); it++)
				{
					if (!(it->target.flags & fFound))
						continue;
					if (it->target.info[0].qvalue <= MassCalQvalue && Abs(it->target.info[0].mass_delta) > E && (!RemoveMassAccOutliers || Abs(it->target.info[0].mass_delta_mz + it->target.info[0].mass_delta - predicted_mz(&(MassCorrection[0]), it->target.info[0].mass_delta_mz, it->target.info[0].RT)) < MassAccOutlier * it->target.info[0].mass_delta_mz))
					{
				...
				SQR.compute(A);
				auto X = SQR.solve(B);
				for (int i = 0; i < MassCorrection.size(); i++)
					MassCorrection[i] = X(i);

				if (Verbose >= 3)
				{
					Time(), dsout << "Mass correction transform (" << mass_cnt << " precursors): ";
					for (int i = 0; i < MassCorrection.size(); i++)
						dsout << MassCorrection[i] << " ";
					dsout << "\n";
				}
				...
				for (auto it = entries.begin(); it != entries.end(); it++)
				{
					...
					if (it->target.info[0].qvalue <= MassCalQvalue && Abs(it->target.info[0].mass_delta) > E)
						b_r.push_back(Abs(it->target.info[0].mass_delta_mz + it->target.info[0].mass_delta -
									  ...
									  it->target.info[0].mass_delta_mz);
				}

				if (Verbose >= 3)
					Time(), dsout << "M/z SD: " << sqrt(var(b_r)) * 1000000.0 << " ppm\n";
				std::sort(b_r.begin(), b_r.end());
				MassAccuracy = b_r[0.7 * (double)b_r.size()];
				RS.MS2_acc_corrected = b_r[0.5 * (double)b_r.size()];
				if (Verbose >= 2)
					Time(), dsout << "Top 70% mass accuracy: " << MassAccuracy * 1000000.0 << " ppm\n";
				acc_calibrated = true;
				...
				if (Verbose >= 2)
					Time(), dsout << "Top 70% mass accuracy without correction: " << acc_no_cal * 1000000.0 << "ppm\n";
				if (acc_no_cal < MassAccuracy)
				{
					if (Verbose >= 2)
						Time(), dsout << "No mass correction required\n";
					MassAccuracy = acc_no_cal;
					for (int i = 0; i < MassCorrection.size(); i++)
						MassCorrection[i] = 0.0;
				}
			}
			else if (Verbose >= 1)
				Time(), dsout << "Cannot perform mass calibration, too few confidently identified precursors\n";

			if (mass_cnt_ms1 >= min_mass_cnt || RemoveMassAccOutliers)
			{
				if (!RemoveMassAccOutliers)
					b.resize(mass_cnt_ms1);
				typedef Eigen::Triplet<double> T;
				std::vector<T> TL, TL_ms1;
				...
				if (!RemoveMassAccOutliers)
				{
					...
					for (int i = 0; i < MassCalBinsMs1; i++)
						MassCalSplitMs1[i] = rt_coo[Min(split * (double)i, mass_cnt_ms1 - 1)];
					MassCalSplitMs1[MassCalBinsMs1] = rt_coo[mass_cnt_ms1 - 1];
					for (int i = 0; i < MassCalBinsMs1; i++)
						MassCalCenterMs1[i] = 0.5 * (MassCalSplitMs1[i] + MassCalSplitMs1[i + 1]);
					for (int i = 1; i < MassCalBinsMs1; i++)
						if (MassCalCenterMs1[i] - MassCalCenterMs1[i - 1] < E)
						{
							for (int k = i - 1; k < MassCalBinsMs1 - 1; k++)
								MassCalCenterMs1[k] = MassCalCenterMs1[k + 1];
							MassCalBinsMs1--;
						}
				}
				...
				for (auto it = entries.begin(); it != entries.end(); it++)
				{
					if (!(it->target.flags & fFound))
						continue;
					if (it->target.info[0].qvalue <= MassCalQvalue && Abs(it->target.info[0].mass_delta) > E && Abs(it->target.info[0].mass_delta_ms1) > E && (!RemoveMassAccOutliers || Abs(it->target.pep->mz + it->target.info[0].mass_delta_ms1 - predicted_mz_ms1(&(MassCorrectionMs1[0]), it->target.pep->mz, it->target.info[0].RT)) < MassAccMs1Outlier * it->target.pep->mz))
					{
				...
				if (mass_cnt_ms1 < min_mass_cnt / 2)
					return;
				...
				SQR.compute(A);
				auto X = SQR.solve(B);
				for (int i = 0; i < MassCorrectionMs1.size(); i++)
					MassCorrectionMs1[i] = X(i);

				if (Verbose >= 3)
				{
					Time(), dsout << "MS1 mass correction transform (" << mass_cnt_ms1 << " precursors): ";
					for (int i = 0; i < MassCorrectionMs1.size(); i++)
						dsout << MassCorrectionMs1[i] << " ";
					dsout << "\n";
				}
				...
				if (!acc_calibrated || RemoveMassAccOutliers)
					MassAccuracyMs1 *= 5.0;
				if (Verbose >= 2)
					Time(), dsout << "Top 70% MS1 mass accuracy: " << acc * 1000000.0 << " ppm\n";
				...
				if (Verbose >= 2)
					Time(), dsout << "Top 70% MS1 mass accuracy without correction: " << acc_no_cal * 1000000.0 << "ppm\n";
				if (acc_no_cal < acc)
				{
					if (Verbose >= 2)
						Time(), dsout << "No MS1 mass correction required\n";
					MassAccuracyMs1 = acc_no_cal;
					if (!acc_calibrated || RemoveMassAccOutliers)
						MassAccuracyMs1 *= 5.0;
					for (int i = 0; i < MassCorrectionMs1.size(); i++)
						MassCorrectionMs1[i] = 0.0;
				}
				if (Verbose >= 2 && MassAccuracyMs1 > Min(acc, acc_no_cal) + E)
					Time(), dsout << "Recommended extraction MS1 mass accuracy: "
								  << MassAccuracyMs1 * 1000000.0 << " ppm\n";
			}
			else if (Verbose >= 1)
				Time(), dsout << "Cannot perform MS1 mass calibration, too few confidently identified precursors\n";
			mz_calibrated = true;
		}
	}
	...
	std::vector<int> calculate_protein_qvalues()
	{
		...
		if (Verbose >= 1)
			Time(), dsout << "Calculating protein q-values\n";
		...
		if (non_proteotypic)
			dsout << "WARNING: " << non_proteotypic << " precursors were wrongly annotated in the library as proteotypic\n";
						...
						for (auto &p : pid.proteins)
							if (lib->proteins[p].id.size())
							{
								if (std::find(prot_ids.begin(), prot_ids.end(), p) == prot_ids.end())
								{
									...
									if (e.target.info[0].qvalue <= 0.01)
										identified[p] = true;
								}
							}
					if (PGLevel)
		...
		if (Verbose >= 1)
			Time(), dsout << "Number of " << (PGLevel == 0 ? "protein isoforms" : (PGLevel == 1 ? "proteins" : "genes")) << " identified at 1% FDR: "
		...
		for (auto &e : entries)
		{
			...
			if (e.target.flags & fFound)
			{
				...
				if (PGLevel == 0)
					for (auto &p : pid.proteins)
						if (q[0][p] < e.target.info[0].protein_qvalue)
							e.target.info[0].protein_qvalue = q[0][p];
				if (PGLevel)
					for (auto &p : pid.proteins)
					{
						pi = (PGLevel == 1 ? lib->proteins[p].name_index : lib->proteins[p].gene_index);
						if (q[0][pi] < e.target.info[0].protein_qvalue)
							e.target.info[0].protein_qvalue = q[0][pi];
					}
			}
		}
		...
		for (i = 0; i < lib->proteins.size(); i++)
		{
			...
			if (PGLevel == 0 && q[0][i] <= ProteinIDQvalue)
				ids.push_back(i);
			else if (PGLevel == 1 && q[0][p.name_index] <= ProteinIDQvalue)
				ids.push_back(i);
			else if (PGLevel == 2 && q[0][p.gene_index] <= ProteinIDQvalue)
				ids.push_back(i);
		}
	...
	{
		if (!RTWindowedSearch && (!RefCal || (!InferWindow && !Calibrate)))
			return false;
		if (Verbose >= 1)
			Time(), dsout << "Calibrating retention times using a set of reference precursors.\n";
	...
	void mgf(int N_ms1, int N_ms2, int min_fr, float min_corr, int window, double acc, double acc_ms1)
	{
		window = Max(window, 1);
		if (Verbose >= 1)
			Time(), std::cout << "Extracting spectra from the raw data: scan window radius = " << window << ", mass accuracy = "
		...
		if (Threads > 1)
		{
			std::vector<std::thread> threads;
			for (i = 0; i < Threads; i++)
				threads.push_back(std::thread(&Run::ms1_top_peaks, this, i, &top, &locks, N_ms1));
			for (i = 0; i < Threads; i++)
				threads[i].join();
		}
		else
			ms1_top_peaks(0, &top, &locks, N_ms1);
		...
		if (Threads > 1)
		{
			std::vector<std::thread> threads;
			for (i = 0; i < Threads; i++)
				threads.push_back(std::thread(&Run::ms1_features, this, i, &features, &top, &locks, window, acc_ms1, N_ms1));
			for (i = 0; i < Threads; i++)
				threads[i].join();
		}
		else
			ms1_features(0, &features, &top, &locks, window, acc_ms1, N_ms1);
		for (i = 0; i < locks.size(); i++)
			locks[i].free();
		...
		if (Threads > 1)
		{
			std::vector<std::thread> threads;
			for (i = 0; i < Threads; i++)
				threads.push_back(std::thread(&Run::feature_spectra, this, i, &spectra, &features, &locks, window, acc_ms1, acc, N_ms2, min_fr, min_corr));
			for (i = 0; i < Threads; i++)
				threads[i].join();
		}
		else
			feature_spectra(0, &spectra, &features, &locks, window, acc_ms1, acc, N_ms2, min_fr, min_corr);
		for (i = 0; i < locks.size(); i++)
			locks[i].free();
		spectra.sort();
		...
		if (out_dir.size())
			mgf_name = out_dir + '\\' + get_file_name(mgf_name);
		else if (temp_folder.size())
			mgf_name = temp_folder + '\\' + get_file_name(mgf_name);
		std::ofstream out(mgf_name, std::ofstream::out);
		if (out.fail())
			std::cout << "ERROR: cannot write to " << mgf_name << "\n";
		else
		{
			i = 0;
			for (auto &s : spectra)
			...
			if (out.fail())
				...
				Time(), std::cout << spectra.size() << " spectra exported to " << mgf_name << "\n";
		}
	}
	...
	{
		if (Verbose >= 1)
			Time(), std::cout << "Extracting targeted masses\n";
	...
	void extraction(std::string file)
	{
		...
		if (Verbose >= 1)
			Time(), dsout << "Extracting " << N << " fragment ions specified\n";
		for (i = 0; i < N; i++)
		{
		...
		if (Verbose >= 1)
			Time(), dsout << "Extracted chromatograms saved to " << file << '\n';
		if (Verbose >= 3)
		{
	...
	void process(std::vector<QuantEntry> *result = NULL, float q_filtering = 0.01)
	{
...

		{
			...
			calibrate:
				...
				for (curr_batch = 0; curr_batch < Batches; curr_batch++)
				{
					if (curr_batch >= 5 && curr_batch < Batches - 1)
					{
						...
						if (Verbose >= 2)
							Time(), dsout << "Processing batches #" << first_batch + 1 << "-" << curr_batch + 1 << " out of " << Batches << " \n";
					}
					else if (Verbose >= 2)
					{
						if (Batches == 1)
							...
							Time(), dsout << "Processing batch #" << curr_batch + 1 << " out of " << Batches << " \n";
					}
					...
					if (recalibrate && ids >= MinCalRec && do_calibrate)
						break;
				}
				if (recalibrate)
				{
					...
					if (Verbose >= 2)
						Time(), dsout << "Recalibrating with mass accuracy " << MassAccuracy << ", " << MassAccuracyMs1 << " (MS2, MS1)\n";
				...
				{
					if (Verbose >= 3)
						Time(), dsout << "Refining mass correction\n";
				...
				if (ForceMassAcc)
				{
					...
					if (Verbose >= 2)
						Time(), dsout << "Using mass accuracy " << MassAccuracy << ", " << MassAccuracyMs1 << " (MS2, MS1)\n";
				}

				if (do_reset)
					reset_precursors();
				else
					start_batch = curr_batch;
			}
			...
			for (curr_batch = start_batch; curr_batch < Batches; curr_batch++)
			{
				...
				update(false, false, false, false, false);
				if (ids >= MinClassifier)
					break;
				if (acc_calibrated && curr_batch >= cal_batch)
					break;
			}
			...
			if (acc_calibrated && !ForceMassAcc)
			{
				...
				{
					MassAccuracy *= 1.2;
					if (MassAccuracy > start_acc * 10.0)
						break;
					if (Verbose >= 3)
						Time(), dsout << "Trying mass accuracy " << MassAccuracy * 1000000.0 << " ppm\n";
					reset_precursors();
					reset_weights();
					update_classifier(true, false, true, false, 0, curr_batch);
					ids = Ids10;
					if (ids > best_ids)
						best_acc = MassAccuracy, best_ids = ids, best1 = Ids1, best50 = Ids50, fail = 0;
					else
						fail++;
					if (fail >= 3)
						break;
				...
				if (Verbose >= 1)
					Time(), dsout << "Optimised mass accuracy: " << MassAccuracy * 1000000.0 << " ppm\n";
				reset_precursors();
				...
				if (ids < best_ids)
				{
					...
					if (Verbose >= 3)
						Time(), dsout << "Resetting weights and preventing the linear classifier from using the full set of weights in the future\n";
				}
				else
					update(false, false, false, false, false);
			...
			if (!IndividualMassAcc)
				ForceMassAcc = true;

			if (Extract.size())
				extraction(ms_files[run_index] + std::string(".extracted.txt"));
			...
			for (; curr_iter < nnIter - 1; curr_iter++)
			{
				...
				{
					if (Verbose >= 3)
						Time(), dsout << "Trying the other linear classifier\n";
					LDA ^= 1, switched++;
					int last10 = Ids10;
				...
				else
				{
					copy_weights(weights, best_weights), copy_weights(guide_weights, best_guide_weights);
					if (Verbose >= 3)
						Time(), dsout << "Reverting weights\n";
					if (curr_iter == CalibrationIter + 2 && !par_limit)
					{
						par_limit = true;
						if (Verbose >= 3)
							Time(), dsout << "Preventing the linear classifier from using the full set of weights in the future\n";
					}
					else
						fail++;
					update_classifier(true, false, false, false, 0, curr_batch);
					if (Ids10 < best_ids)
					{
						...
						best_ids = Ids10, update(false, false, false, false, false);
					if (fail >= 1)
					{
						curr_iter = nnIter - 1;
						break;
			...
			if (SaveCandidatePSMs)
			{
				...
				std::string floc;
				if (temp_folder.size())
					floc = location_to_file_name(name) + std::string(".psms.tsv");
				else
					floc = name + std::string(".psms.tsv");
			...
			if (SaveCandidatePSMs && !suspend_output)
				run_out.close(), std::vector<char>().swap(buf);
			update(false, false, false, false, false);
			...
			{
				if (Verbose >= 1)
					Time(), dsout << "Removing interfering precursors\n";
	...
	report:
		...
		// added by sj
		std::ofstream out("diann_info.tsv");

		out << "decoy\tpr_id\tpr_mz\trt_start\trt\trt_stop\tfg_mz";
		for (int i = 0; i < pN; i++)
			out << '\t' << "score_" << i;
		out << "\n";
		out.precision(10.0);
		for (auto &e : entries)
		{
			std::string pr_id;
			float pr_mz, quant;
			float rt_start, rt, rt_stop;
			if (e.target.flags & fFound)
			{
				pr_id = (lib->entries[e.target.pep->index].name).c_str();
				pr_mz = e.target.pep->mz;
				rt_start = e.target.info[0].RT_start;
				rt = e.target.info[0].RT;
				rt_stop = e.target.info[0].RT_stop;
				out << 0 << '\t' << pr_id << '\t' << pr_mz << '\t';
				out << rt_start << '\t' << rt << '\t' << rt_stop << '\t';
				for (int fr = 0; fr < e.target.info[0].quant.size(); fr++)
				{
					auto &cfr = e.target.pep->fragments[fr];
					out << cfr.mz;
					if (fr != e.target.info[0].quant.size() - 1)
						out << ";";
				}
				for (int i = 0; i < pN; i++)
					out << '\t' << e.target.info[0].scores[i];
				out << '\n';
			}
		}
		for (auto &e : entries)
		{
			...
			if (e.decoy.flags & fFound)
			{
				pr_id = (lib->entries[e.decoy.pep->index].name).c_str();
				pr_mz = e.decoy.pep->mz;
				rt_start = e.decoy.info[0].RT_start;
				rt = e.decoy.info[0].RT;
				rt_stop = e.decoy.info[0].RT_stop;
				out << 1 << '\t' << pr_id << '\t' << pr_mz << '\t';
				out << rt_start << '\t' << rt << '\t' << rt_stop << '\t';
				for (int fr = 0; fr < e.decoy.info[0].quant.size(); fr++)
				{
					auto &cfr = e.decoy.pep->fragments[fr];
					out << cfr.mz;
					if (fr != e.decoy.info[0].quant.size() - 1)
						out << ";";
				}
				for (int i = 0; i < pN; i++)
					out << '\t' << e.decoy.info[0].scores[i];
				out << '\n';
			}
		}
		out.close();
		...
		// added by sj
		std::ofstream sj_out("cal_params.tsv");

		sj_out << "MassCorrectionMs1_v" << '\t';
		for (auto i = 0; i < MassCorrectionMs1.size(); i++)
			sj_out << MassCorrectionMs1[i] << '\t';
		sj_out << '\n';

		sj_out << "MassCalCenterMs1_v" << '\t';
		for (auto i = 0; i < MassCalCenterMs1.size(); i++)
			sj_out << MassCalCenterMs1[i] << '\t';
		sj_out << '\n';

		sj_out << "MassCalBinsMs1" << '\t' << MassCalBinsMs1 << '\n';
		sj_out << "MassAccuracyMs1" << '\t' << MassAccuracyMs1 << '\n';

		sj_out << "MassCorrection_v" << '\t';
		for (auto i = 0; i < MassCorrection.size(); i++)
			sj_out << MassCorrection[i] << '\t';
		sj_out << '\n';

		sj_out << "MassCalCenter_v" << '\t';
		for (auto i = 0; i < MassCalCenter.size(); i++)
			sj_out << MassCalCenter[i] << '\t';
		sj_out << '\n';

		sj_out << "MassCalBins" << '\t' << MassCalBins << '\n';
		sj_out << "MassAccuracy" << '\t' << MassAccuracy;
		...
		if (args.find("--!list-peptides") != std::string::npos)
		{
			...
			for (auto &e : entries)
			{
				...
				if (e.target.flags & fFound)
					if (e.target.info[0].qvalue <= 0.01)
						abundance = e.target.info[0].quantity;
				p.first = get_aas(lib->entries[e.target.pep->index].name);
				p.second = abundance;
				auto ins = pep.insert(p);
				if (!ins.second)
					if (ins.first->second < abundance)
						ins.first->second = abundance;
			}
		...
		{
			float sum = 0;
			for (auto j = 0; j < ms1h[i].n; j++)
				sum += ms1h[i].peaks[j].height;
			RS.MS1_tot += sum;
		}
		if (ms2h.size())
		{
			...
			for (i = 0; i < ms2h.size(); i++)
			{
				...
				float sum = 0;
				for (auto j = 0; j < ms2h[i].n; j++)
					sum += ms2h[i].peaks[j].height;
				RS.TIC[bin] += sum;
				RS.MS2_tot += sum;
			}
		}
		...
		DecoyEntry de;
		for (i = 0; i < pN; i++)
			quant.weights[i] = weights[i], quant.guide_weights[i] = guide_weights[i];
		...
		if (result != NULL)
		{
			...
			for (auto it = entries.begin(); it != entries.end(); it++)
				if (it->target.flags & fFound)
					if (it->target.info[0].qvalue <= q_threshold)
						tot_entries++;
			result->clear();
			result->reserve(tot_entries);
		}
		for (auto it = entries.begin(); it != entries.end(); it++)
		{
			...
			if (result == NULL)
				if (it->decoy.flags & fFound)
					if (it->decoy.info[0].qvalue <= QFilter)
					{
			...
			if (qe.pr.decoy_found)
				qe.pr.decoy_qvalue = it->decoy.info[0].qvalue;
			else
				qe.pr.decoy_qvalue = 1.0;
			qe.pr.profile_qvalue = 1.0;

			qe.fr_n = it->target.info[0].quant.size();
			for (i = 0; i < qe.fr_n; i++)
				qe.fr[i] = it->target.info[0].quant[i];
			if (qe.fr_n < TopF)
				memset(&(qe.fr[qe.fr_n]), 0, (TopF - qe.fr_n) * sizeof(Fragment));
#if REPORT_SCORES
			for (i = 0; i < pN; i++)
				qe.pr.scores[i] = it->target.info[0].scores[i];
			if (qe.pr.decoy_found)
			{
				for (i = 0; i < pN; i++)
					qe.pr.decoy_scores[i] = it->decoy.info[0].scores[i];
			}
			else
				for (i = 0; i < pN; i++)
					qe.pr.decoy_scores[i] = 0.0;
#endif
#if ELUTION_PROFILE
			assert(it->target.ms1_elution_profile.size() == 2 * ElutionProfileRadius + 1);
			for (i = 0; i <= 2 * ElutionProfileRadius; i++)
				qe.ms1_elution_profile[i] = it->target.ms1_elution_profile[i];
#endif
			if (result == NULL)
		...
		else
		{
			...
			if (out.fail())
				dsout << "ERROR: cannot save the .quant file to " << out_quant << ". Check if the destination folder is write-protected or the file is in use\n";
			else
			{
				quant.write(out);
				...
				out.close();
				if (Verbose >= 1)
					Time(), dsout << "Quantification information saved to " << out_quant << ".\n\n";
#ifdef _MSC_VER
				if (args.find("--!mmap-quant") != std::string::npos)
				{
					...
					for (i = 0; i < 1000; i++)
						if (mapping_name[i] == 0)
							break;
					mapping_name.resize(i);
					if (Verbose >= 1)
						Time(), dsout << "Creating a file mapping " << mapping_name << " with the quantification information\n";
					auto map = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, fsize, mapping_name.c_str());
					if (map == NULL)
						dsout << "ERROR: Cannot create file mapping\n";
					else
					{
						auto buf = (LPTSTR)MapViewOfFile(map, FILE_MAP_ALL_ACCESS, 0, 0, fsize);
						if (buf == NULL)
							dsout << "ERROR: Cannot access file mapping\n";
						else
						{
		...
		if (IndividualReports)
		{
			...
			if (lib->genes.size())
				lib->gene_report(ms_files[run_index] + std::string(".genes.tsv"));
			dsout << '\n';
		}
		...
		if (SaveXICs)
		{
			...
			if (out.fail())
				dsout << "ERROR: cannot save the .xic file to " << out_xic << ". Check if the destination folder is write-protected or the file is in use\n";
			else
			{
				...
				for (auto &x : XIC_list)
					tot += x.size_in_bytes(); // now tot stores the total header size
				int header_size = tot;
	...
	noninline void update_library()
	{ // update fragment intensities in the spectral library
		int cnt = 0;
		for (auto &e : entries)
		{
			auto &le = lib->entries[e.target.pep->index];
			if (le.best_run != run_index || le.qvalue > ReportQValue || le.pg_qvalue > ReportProteinQValue)
				continue;
		...
		if (Verbose >= 1)
			Time(), dsout << cnt << " precursors added to the library\n";
	}
								...
								int n_ms2, float *ms2_rt, float *ms2_low, float *ms2_high, int *ms2_peak_n, float *ms2_peaks)
{
	...
	if (acc_ms2 > E)
	{
		...
		if (acc_ms1 > E)
			GlobalMassAccuracyMs1 = acc_ms1;
	}
	if (win_size)
		WindowRadius = win_size, InferWindow = false;
	...
	long long pos;
	for (int i = pos = 0; i < n_ms1; i++)
		run.ms1h[i].peaks = pos + ((Peak *)ms1_peaks), pos += ms1_peak_n[i], run.ms1h[i].n = ms1_peak_n[i], run.ms1h[i].RT = ms1_rt[i],
		run.ms1h[i].window_low = run.ms1h[i].window_high = 0.0;
	for (int i = pos = 0; i < n_ms2; i++)
		run.ms2h[i].peaks = pos + ((Peak *)ms2_peaks), pos += ms2_peak_n[i], run.ms2h[i].n = ms2_peak_n[i], run.ms2h[i].RT = ms2_rt[i],
...
int analyse_unit_test(int threads)
{
	...
	if (lib.fail())
	{
		dsout << "Cannot open the spectral library file\n";
		return -1;
	}
	lib.seekg(0, lib.end);
	...
	for (int i = 0; i < run.ms1h.size(); i++)
		ms1_rt[i] = run.ms1h[i].RT, ms1_peak_n[i] = run.ms1h[i].n;
	for (int i = 0; i < run.ms2h.size(); i++)
		ms2_rt[i] = run.ms2h[i].RT, ms2_peak_n[i] = run.ms2h[i].n,
...
int main(int argc, char *argv[])
{
	...
	dsout << "DIA-NN 1.7.16 (Data Independent Acquisition by Neural Networks)\nCompiled on " << __DATE__ << " " << __TIME__ << "\nCurrent date and time: " << std::ctime(&curr_time);
#ifdef _MSC_VER
	cpu_info(dsout);
#endif
	...
	if (Verbose >= 1)
		dsout << ms_files.size() << " files will be processed\n";
	if (Convert)
	{
		for (auto it = ms_files.begin(); it != ms_files.end(); it++)
		{
			...
			if (out_dir.size())
				dia_name = out_dir + '\\' + get_file_name(dia_name);
	...
	{
		if (learn_lib_file.size() && !ReportOnly)
			learn_from_library(learn_lib_file);
		if (!fasta.load(fasta_files, fasta_filter_files))
			Throw("Cannot load FASTA");
	}
	...
	if (fasta_files.size())
	{
		...
		for (int i = 1; i < fasta_files.size(); i++)
			all_fastas += std::string("; ") + fasta_files[i];
	...
	if (FastaSearch && fasta_files.size())
	{
		if (lib_file.size() && lib.load(&(lib_file[0])))
		{
			...
			if (learn_lib_file.size())
				for (auto &e : lib.entries)
					e.target.sRT = (InSilicoRTPrediction ? predict_irt(e.name) : default_iRT);
		}
		lib.load(fasta);
	}
	else
	{
		if (!lib_file.size())
			return 0;
		if (!lib.load(&(lib_file[0])))
			Throw("Cannot load spectral library");
		if (ExportProsit && !fasta_files.size())
		{
			if (Verbose >= 1)
				Time(), dsout << "Preparing Prosit input from the spectral library provided\n";
			lib.export_prosit();
		}
	}
	if (!FastaSearch && fasta_files.size())
	{
		if (lib.protein_ids.size() > 1 && !Reannotate)
			fasta.load_proteins(fasta_files);
		else if (!fasta.load(fasta_files, fasta_filter_files))
			Throw("Cannot load FASTA");
	}
	if (fasta_files.size())
	{
		...
		{
			if (Verbose >= 1 && !FastaSearch)
				Time(), dsout << "Annotating library proteins with information from the FASTA database\n";
		...
		else
		{
			if (Verbose >= 1)
			{
				...
				{
					PG pg;
					lib.protein_ids.clear();
					lib.protein_ids.push_back(pg);
					lib.names.clear();
					lib.genes.clear();
					lib.proteins.clear();
					for (auto &e : lib.entries)
	...
	if (!ReportOnly)
		lib.initialise(!QuantOnly);
	if (!FastaSearch && !lib.from_speclib && lib.gen_decoys && !PredictorSaved)
		lib.save(lib_file + std::string(".speclib"));

	Library ref;
	if (ref_file.size())
	{
		if (!ref.load(&(ref_file[0])))
			ref_file.clear();
		else
			ref.initialise(true);
	}
	...
	if (RTProfiling)
		if (Verbose >= 1)
			Time(), dsout << "First pass: collecting retention time information\n";
	for (auto it = ms_files.begin(); it != ms_files.end(); it++)
	{
		if ((UseRTInfo && RTProfiling) || UseQuant)
		{
			...
			if (in.fail() || temp_folder.size())
				in = std::ifstream(location_to_file_name(*it) + std::string(".quant"), std::ifstream::binary);
			if (in.is_open())
			{
				if (IndividualReports)
				{
					...
					if (lib.genes.size())
						lib.gene_report((*it) + std::string(".genes.tsv"));
				}
				in.close();
				continue;
			}
		...
		if (Verbose >= 1)
			Time(), dsout << "File #" << std::distance(ms_files.begin(), it) + 1 << "/" << ms_files.size() << "\n";

		Run run(std::distance(ms_files.begin(), it));
		if (!run.load(&((*it)[0])))
			dsout << "ERROR: cannot load the file, skipping\n", failed_files.insert(*it);
		if (ref_file.size())
			run.reference_run(&ref);
		run.load_library(&lib);
		run.process();
...
quant_only:
...

	{
		...
		for (auto it = ms_files.begin(); it != ms_files.end(); it++)
		{
			...
			if (Verbose >= 1)
				Time(), dsout << "Second pass: file #" << std::distance(ms_files.begin(), it) + 1 << "/" << ms_files.size() << "\n";
...
gen_spec_lib:
	...
	if (!ms_files.size())
		goto remove;
	if (Verbose >= 1)
		Time(), dsout << "Cross-run analysis\n";
	if (QuantInMem)
		lib.info.load(&(lib), ms_files, &quants);
	else
	...
	if (out_file.size() && !IndividualReports)
	{
		...
		if (Visualise.size())
			lib.XIC_report(remove_extension(out_file) + std::string(".XIC.tsv"));
	}
	if (out_gene_file.size() && !IndividualReports)
		lib.gene_report(out_gene_file);

	if (out_lib_file.size() && ms_files.size())
		if (FastaSearch || GenSpecLib)
		{ // generating spectral library
			MaxF = INF;
			if (Verbose >= 1)
				Time(), dsout << "Generating spectral library:\n";
			...
			// determine the best run for each precursor and collect information from this run
			{
				...
				for (auto jt = profile.entries.begin(); jt != profile.entries.end(); jt++)
					if (jt->index >= 0)
					{
						...
						{
							lib.entries[jt->pr.index].qvalue = jt->pr.qvalue;
							if (ProfileQValue && jt->pr.profile_qvalue > jt->pr.qvalue)
								lib.entries[jt->pr.index].qvalue = jt->pr.profile_qvalue;
							if (lib.entries[jt->pr.index].qvalue > ReportQValue)
								lib.entries[jt->pr.index].best_run = -1;
							else
							{
								lib.entries[jt->pr.index].best_run = jt->pr.run_index;
			...
			for (auto it = ms_files.begin(); it != ms_files.end(); it++)
			{
				...
				if (!QuantInMem)
				{
					...
					Q = &(quants[std::distance(ms_files.begin(), it)]);
			...
			if (Verbose >= 1)
				Time(), dsout << "Loading the generated library and saving it in the .speclib format\n";
			Library new_lib;
			new_lib.load(&(out_lib_file[0]));
			Fasta fst;
			fst.load_proteins(fasta_files);
...
remove:
	...
	{
		if (Verbose >= 1)
			Time(), dsout << "Removing .quant files\n";
		for (auto &file : ms_files)
			try
			{
				std::experimental::filesystem::remove(file + std::string(".quant"));
			}
			catch (std::exception &e)
			{
			}
	...
	if ((out_file.size() && ms_files.size()) || out_lib_file.size())
	{
		...
		if (out.fail())
			dsout << "ERROR: cannot write log to " << log_name << "\n";
		else
		{
			out << dsout.s.rdbuf();
			out.close();
